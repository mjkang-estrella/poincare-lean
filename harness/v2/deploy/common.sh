...
load_config() {
  ...
  unset POINCARE_LEANSTRAL_BACKLOG_TARGET
  ...
  POINCARE_LEANSTRAL_BACKLOG_TARGET=${POINCARE_LEANSTRAL_BACKLOG_TARGET:-$POINCARE_MAX_LEANSTRAL_JOBS}
  ...
  export POINCARE_MAX_LEANSTRAL_JOBS POINCARE_LEANSTRAL_BACKLOG_TARGET POINCARE_INTEGRATION_BATCH_SIZE
  ...
  require_uint_range POINCARE_LEANSTRAL_BACKLOG_TARGET \
    "$POINCARE_LEANSTRAL_BACKLOG_TARGET" 1 "$POINCARE_MAX_LEANSTRAL_JOBS"
...
PY
  ...
  POINCARE_CONFIG_FINGERPRINT=$("$HARNESS_PI_PYTHON" -S -P -B - \
    ...
    "$POINCARE_MAX_LEANSTRAL_JOBS" "$POINCARE_LEANSTRAL_BACKLOG_TARGET" \
...
target = int(os.environ["POINCARE_LEANSTRAL_BACKLOG_TARGET"])
ceiling = int(os.environ["POINCARE_MAX_LEANSTRAL_JOBS"])
execution_backlog = counts["queued"] + counts["executing"]
payload = dict(counts)
payload.update(
    {
        "ceiling": ceiling,
        ...
        "underfilled": max(0, target - execution_backlog),
    }
)
