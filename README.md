...
- `Poincare/RicciFlowInterface.lean` exposes the finite-extinction and
  ...
  Their canonical-statement payload constructors now also round-trip from the
  ...
  and ordinary remaining-dependency and aggregate proof-dependency projection
...
- `Poincare/TopologyExtraction.lean` defines typed no-constructor interfaces for
  ...
  loop/path/quotient variants replace the raw compactification
  ...
  homeomorphisms.  Its loop/path/quotient variants replace the raw
  compactification `SimplyConnectedSpace` input with
  `OnePointThreeSpaceLoopNullhomotopyStatement`,
  `OnePointThreeSpacePathHomotopyStatement`, and
