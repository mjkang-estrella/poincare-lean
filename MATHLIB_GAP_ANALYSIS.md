...
zero-curvature Ricci-flow right-hand side, plus a checked pointwise route for
the zero equation verification itself. The same explicit inputs now build zero
Ricci-flow data and an analytic equation-boundary package.
...
the fixed-flow analytic derivation statement from that payload,
