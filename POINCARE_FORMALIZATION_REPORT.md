...
| Pin dependency finite-extinction payload contracts | `Poincare/DependencyProjections.lean` now proves equality contracts for the surgery-package-enriched finite-extinction statement payload and the simplified dependency-level finite-extinction statement payload, pinning both routes to the named surgery package payload and package-level finite-extinction projections | Achieved as dependency payload accounting, not the unconditional proof |
