...
| Pin projection certificate statement payload routes | `Poincare/CanonicalBridges.lean` now pins the ordinary projection certificate's literal and aggregate canonical-statement payload projections directly to the finite-extinction plus theorem-shaped topology-extraction route, and pins the certified extraction-derivation projection certificate's matching statement payload projections directly to the finite-extinction plus extractor/derivation route at both remaining-dependency and aggregate proof-dependency surfaces | Achieved as statement-payload route accounting, not the unconditional proof |
