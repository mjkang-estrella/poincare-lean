...
/-- **The untraced contracted Lichnerowicz formula, trace term simplified**:
`δRic(u,w) = div δΓ(u,w) − ½ (∇²(tr_g H))(u,w)`. Combining the contracted Lichnerowicz
formula `δRic = div δΓ − ∇(tr δΓ)` with the collapse of the trace term to half the
covariant Hessian of `tr_g H`. The untraced analogue of the scalar
`ricciDeriv_raised_trace_eq_divTrace_sub_half_curvedLaplacian`; the `div δΓ` term remains
for the `∇²H` Bochner expansion (roadmap item 3). -/
theorem ricciDeriv_eq_div_sub_half_hessian
    {G H : E → E →L[ℝ] E →L[ℝ] ℝ} {x : E}
    (hGd : ∀ y : E, DifferentiableAt ℝ G y)
    (hGsymm : ∀ (y : E) (p q : E), G y p q = G y q p)
    (hinv : ∀ y : E, (G y).IsInvertible)
    ...
    ricciDeriv G H x u w
      = deltaGammaDivergence G H x u w
        - (1 / 2 : ℝ) * covTensor1Deriv G
            (fun y ↦ fderiv ℝ (tensorMetricTrace G H) y) x u w := by
  rw [ricciDeriv_eq_deltaGamma_contractions hΓsymm,
    deltaGammaContractionDeriv_eq_half_hessian hGd hGsymm hinv hHd hHsymm hVd
      hTr2 u w]
...
/-- **The `δΓ`-trace-derivative is half the covariant Hessian of `tr_g H`** (named form):
`∇_u(tr δΓ)(w) = ½ (Hess_g(tr_g H))(u,w)`, the clean named-`covariantHessian` form of
`deltaGammaContractionDeriv_eq_half_hessian` via `covTensor1Deriv_fderiv_eq_covariantHessian`.
The trace term of the untraced Ricci variation as a genuine covariant Hessian (roadmap item 3). -/
theorem deltaGammaContractionDeriv_eq_half_covariantHessian
    {G H : E → E →L[ℝ] E →L[ℝ] ℝ} {x : E}
    (hGd : ∀ y : E, DifferentiableAt ℝ G y)
    (hGsymm : ∀ (y : E) (p q : E), G y p q = G y q p)
    (hinv : ∀ y : E, (G y).IsInvertible)
  ...
  rw [deltaGammaContractionDeriv_eq_half_hessian hGd hGsymm hinv hHd hHsymm hVd
      hTr2 u w,
    covTensor1Deriv_fderiv_eq_covariantHessian hGsymm hinv
...
/-- **The `δΓ`-divergence in Lichnerowicz half-Koszul form**: substituting
`g_covDeltaGammaDeriv_lichnerowicz`, `div δΓ(u,w) = Σᵢ [ D_{bᵢ}(½(∇H(u,w,♯bⁱ) +
∇H(w,u,♯bⁱ) − ∇H(♯bⁱ,u,w))) − three Christoffel corrections ]`. The divergence term of the
untraced Ricci variation with the Koszul `δΓ = ½∇H` substituted under one `fderiv` — the
clean entry to the `∇²H` Bochner form (roadmap item 3). -/
theorem deltaGammaDivergence_lichnerowicz_form
    {G H : E → E →L[ℝ] E →L[ℝ] ℝ} {x : E} {u : E}
    (hGd : ∀ y : E, DifferentiableAt ℝ G y)
    (hGsymm : ∀ (y : E) (a b : E), G y a b = G y b a)
    (hinv : ∀ y : E, (G y).IsInvertible)
    ...
    (w : E) :
    deltaGammaDivergence G H x u w
      = ∑ i, ((fderiv ℝ (fun y ↦ (1 / 2 : ℝ) *
              (covTensor2Deriv G H y u w
                ((G x).inverse (LinearMap.toContinuousLinearMap
                  ((Module.finBasis ℝ E).coord i)))
              + covTensor2Deriv G H y w u
                ((G x).inverse (LinearMap.toContinuousLinearMap
                  ((Module.finBasis ℝ E).coord i)))
              - covTensor2Deriv G H y
                ((G x).inverse (LinearMap.toContinuousLinearMap
                  ((Module.finBasis ℝ E).coord i))) u w)) x)
              ((Module.finBasis ℝ E) i)
          - G x (christoffelDerivOp G H x
              (christoffelClosedOp G x ((Module.finBasis ℝ E) i) u) w)
              ((G x).inverse (LinearMap.toContinuousLinearMap
  ...
  rw [deltaGammaDivergence_eq_g_paired hGsymm hinv]
  refine Finset.sum_congr rfl fun i _ ↦ ?_
  exact g_covDeltaGammaDeriv_lichnerowicz hGd hGsymm hinv hHsymm hVd
    ((Module.finBasis ℝ E) i) w
    ((G x).inverse (LinearMap.toContinuousLinearMap
      ((Module.finBasis ℝ E).coord i)))
...
/-- **The `δΓ`-divergence in `∇²H` Bochner form**: substituting `fderiv_covTensor2Deriv_eq`
into the three Lichnerowicz `∇H` derivatives, each becomes a second covariant derivative
`∇²H` plus three Christoffel corrections. The full untraced second-order Bochner expansion of
the divergence term of the Ricci variation (roadmap item 3). -/
theorem deltaGammaDivergence_sndDeriv_bochner_form
    {G H : E → E →L[ℝ] E →L[ℝ] ℝ} {x : E} {u : E}
    (hGd : ∀ y : E, DifferentiableAt ℝ G y)
    (hGsymm : ∀ (y : E) (a b : E), G y a b = G y b a)
    (hinv : ∀ y : E, (G y).IsInvertible)
    ...
    deltaGammaDivergence G H x u w
      = ∑ i, ((1 / 2 : ℝ) *
          ...
          - G x (christoffelDerivOp G H x u w) (christoffelClosedOp G x ((Module.finBasis ℝ E) i) ((G x).inverse (LinearMap.toContinuousLinearMap ((Module.finBasis ℝ E).coord i))))) := by
  rw [deltaGammaDivergence_lichnerowicz_split hGd hGsymm hinv hHsymm hHd hH2 hΓd hVd w]
  refine Finset.sum_congr rfl fun i _ ↦ ?_
  rw [fderiv_covTensor2Deriv_eq, fderiv_covTensor2Deriv_eq,
    fderiv_covTensor2Deriv_eq]
...
/-- **Expansion of the 2nd-slot correction difference**: applying the product rule to both
orientations and using linearity of `H x` in its middle slot,
`corr₁(v') − corr₁(v) = H x (D_{v'}Γ_v p − D_v Γ_{v'} p) q + [(D_{v'}H)(Γ_v p, q) − (D_v H)(Γ_{v'} p, q)]`.
The `H·∂Γ` term carries the antisymmetric Christoffel derivative (curvature); the `∂H·Γ` term will
pair with the surviving slot corrections (roadmap item 3, curvature-commutation frontier). -/
theorem fderiv_corr1_antisymm_expand
    {G H : E → E →L[ℝ] E →L[ℝ] ℝ} {x : E} (v' v p q : E)
    (hH : DifferentiableAt ℝ H x)
    (hΓ : DifferentiableAt ℝ (fun y ↦ christoffelClosedOp G y v p) x)
    (hΓ' : DifferentiableAt ℝ (fun y ↦ christoffelClosedOp G y v' p) x) :
    (fderiv ℝ (fun y ↦ H y (christoffelClosedOp G y v p) q) x) v'
      - (fderiv ℝ (fun y ↦ H y (christoffelClosedOp G y v' p) q) x) v
    = H x (fderiv ℝ (fun y ↦ christoffelClosedOp G y v p) x v'
            - fderiv ℝ (fun y ↦ christoffelClosedOp G y v' p) x v) q
      + ((fderiv ℝ H x v') (christoffelClosedOp G x v p) q
         - (fderiv ℝ H x v) (christoffelClosedOp G x v' p) q) := by
  rw [fderiv_tensor_corr_field hH hΓ v' q, fderiv_tensor_corr_field hH hΓ' v q,
...
/-- **The 3rd-slot correction difference in curvature form**: the companion of
`fderiv_corr1_antisymm_curvature` for the last slot,
`corr₂(v') − corr₂(v) = H x p ((Rm v' v) q − Γ_{v'}∘Γ_v q + Γ_v∘Γ_{v'} q) + [∂H·Γ antisym]`
(roadmap item 3, curvature-commutation frontier). -/
theorem fderiv_corr2_antisymm_curvature
    {G H : E → E →L[ℝ] E →L[ℝ] ℝ} {x : E} {v' v : E} (p q : E)
    (hH : DifferentiableAt ℝ H x)
    (hΓ : DifferentiableAt ℝ (fun y ↦ christoffelClosedOp G y v) x)
    (hΓ' : DifferentiableAt ℝ (fun y ↦ christoffelClosedOp G y v') x) :
    ...
    = H x p ((coordCurvatureOp G x v' v) q
      ...
      + ((fderiv ℝ H x v') p (christoffelClosedOp G x v q)
         - (fderiv ℝ H x v) p (christoffelClosedOp G x v' q)) := by
  rw [fderiv_corr2_antisymm_expand v' v p q hH
      (hΓ.clm_apply (differentiableAt_const q))
      (hΓ'.clm_apply (differentiableAt_const q)),
...
/-- **The transposed rough Laplacian (Weitzenböck reordering)**: solving the connection-Laplacian
commutator for the transposed ordering, `Σᵢ ∇²H(♯bⁱ, bᵢ) = Δ_∇ H − Σᵢ −(curvature on H's slots)`.
The rough Laplacian taken in the reversed derivative order equals the standard one minus the traced
curvature commutator — the reordering used when matching the Bochner blocks of `δRic` against the
rough Laplacian (roadmap item 3). -/
theorem connectionLaplacian_transpose
    {G H : E → E →L[ℝ] E →L[ℝ] ℝ} {x : E} (p q : E)
    (hGd : DifferentiableAt ℝ G x)
    (hGsymm : ∀ (y : E) (a b : E), G y a b = G y b a)
    (hHC2 : ContDiffAt ℝ 2 H x)
      ...
      = connectionLaplacian G H x p q
        - ∑ i, -(H x ((coordCurvatureOp G x ((Module.finBasis ℝ E) i)
              ...
              + H x p ((coordCurvatureOp G x ((Module.finBasis ℝ E) i)
                  ((G x).inverse (LinearMap.toContinuousLinearMap
                    ((Module.finBasis ℝ E).coord i)))) q)) := by
  have h := connectionLaplacian_sub_transpose_eq_curvature p q hGd hGsymm hHC2 hH
    hpure1 hpure2 hc1 hc1' hc2 hc2' hΓ hΓ'
  linarith [h]
...
/-- **The rough Laplacian extracted from `div δΓ`**: the Bochner C-block's leading second
covariant derivative sums to the connection Laplacian, so
`deltaGammaDivergence G H x u w = −½ Δ_∇ H(u,w) + (divergence + correction terms)`. The first
explicit appearance of the rough Laplacian `Δ_∇ H` inside the untraced Ricci variation
(roadmap item 3). -/
theorem deltaGammaDivergence_extract_connectionLaplacian
    {G H : E → E →L[ℝ] E →L[ℝ] ℝ} {x : E} {u : E}
    (hGd : ∀ y : E, DifferentiableAt ℝ G y)
    (hGsymm : ∀ (y : E) (a b : E), G y a b = G y b a)
    (hinv : ∀ y : E, (G y).IsInvertible)
    ...
    (w : E) :
    deltaGammaDivergence G H x u w
      = -(1 / 2 : ℝ) * connectionLaplacian G H x u w
        + (∑ i, ((1 / 2 : ℝ) *
            ((covTensor2SndDeriv G H x ((Module.finBasis ℝ E) i) u w ((G x).inverse (LinearMap.toContinuousLinearMap ((Module.finBasis ℝ E).coord i)))
              + covTensor2Deriv G H x (christoffelClosedOp G x ((Module.finBasis ℝ E) i) u) w ((G x).inverse (LinearMap.toContinuousLinearMap ((Module.finBasis ℝ E).coord i)))
              + covTensor2Deriv G H x u (christoffelClosedOp G x ((Module.finBasis ℝ E) i) w) ((G x).inverse (LinearMap.toContinuousLinearMap ((Module.finBasis ℝ E).coord i)))
          ...
          - G x (christoffelDerivOp G H x u (christoffelClosedOp G x ((Module.finBasis ℝ E) i) w)) ((G x).inverse (LinearMap.toContinuousLinearMap ((Module.finBasis ℝ E).coord i)))
          - G x (christoffelDerivOp G H x u w) (christoffelClosedOp G x ((Module.finBasis ℝ E) i) ((G x).inverse (LinearMap.toContinuousLinearMap ((Module.finBasis ℝ E).coord i)))))) := by
  rw [deltaGammaDivergence_sndDeriv_bochner_form hGd hGsymm hinv hHsymm hHd hH2 hΓd hVd w]
  unfold connectionLaplacian
...
/-- **The rough Laplacian extracted from `δRic`**: lifting the `div δΓ` extraction through the
untraced Ricci variation, `δRic(u,w) = −½ Δ_∇ H(u,w) + (divergence + correction terms) − ½ Hess_g(tr_g H)(u,w)`.
The rough Laplacian `Δ_∇ H` now appears explicitly in the Ricci variation itself — the structural
target `δRic = −½Δ_∇ H + …` made concrete (roadmap item 3). -/
theorem ricciDeriv_extract_connectionLaplacian
    {G H : E → E →L[ℝ] E →L[ℝ] ℝ} {x : E} {u : E}
    (hGd : ∀ y : E, DifferentiableAt ℝ G y)
    (hGsymm : ∀ (y : E) (p q : E), G y p q = G y q p)
    (hinv : ∀ y : E, (G y).IsInvertible)
    ...
    ricciDeriv G H x u w
      = -(1 / 2 : ℝ) * connectionLaplacian G H x u w
        ...
        - (1 / 2 : ℝ) * covariantHessian G (fun y ↦ metricBilin (G y))
          (fun y ↦ metricBilin_nondeg (hGsymm y) (hinv y))
          (tensorMetricTrace G H) x u w := by
  rw [ricciDeriv_sndDeriv_bochner_form hGd hGsymm hinv hHd hHsymm hH2 hΓd hVd hTr2 hΓsymm w]
  unfold connectionLaplacian
  rw [Finset.mul_sum, ← Finset.sum_add_distrib]
  congr 1
  refine Finset.sum_congr rfl fun i _ ↦ ?_
...
/-- **The transposed rough Laplacian is homogeneous in the tensor**:
`(Δ_∇ + lichnerowiczCurvature)(c • H) = c · (Δ_∇ + lichnerowiczCurvature)(H)`. Both the rough
Laplacian and its Weitzenböck curvature correction scale by `c`, so the transposed rough Laplacian
`Σᵢ ∇²H(♯bⁱ, bᵢ)` is `ℝ`-homogeneous in `H` — as the linear operator acting in `δRic` must be
(roadmap item 3). -/
theorem connectionLaplacian_add_lichnerowiczCurvature_smul
    {G H : E → E →L[ℝ] E →L[ℝ] ℝ} {x : E}
    (hHd : ∀ y : E, DifferentiableAt ℝ H y)
    (hH2 : DifferentiableAt ℝ (fun y ↦ fderiv ℝ H y) x)
    (hΓd : ∀ a : E, DifferentiableAt ℝ (fun y ↦ christoffelClosedOp G y a) x)
    ...
    connectionLaplacian G (fun y ↦ c • H y) x p q
        + lichnerowiczCurvature G (fun y ↦ c • H y) x p q
      = c * (connectionLaplacian G H x p q + lichnerowiczCurvature G H x p q) := by
  rw [connectionLaplacian_smul_field hHd hH2 hΓd c p q,
    lichnerowiczCurvature_smul c p q]
  ring
...
/-- **The Ricci action is homogeneous in its first slot**:
`(Ric · H)(c • p, q) = c · (Ric · H)(p,q)`, from homogeneity of `Ric^♯` and `H`. With the additivity
lemmas this makes the `Ric·H` term a fully `ℝ`-bilinear form in `(p,q)` (roadmap item 3). -/
theorem ricciActionOnTensor_smul_left
    {G H : E → E →L[ℝ] E →L[ℝ] ℝ} {x : E}
    (hdiff : ∀ u : E, DifferentiableAt ℝ (fun y ↦ christoffelClosedOp G y u) x)
    (c : ℝ) (p q : E) :
    ricciActionOnTensor G H x (c • p) q = c * ricciActionOnTensor G H x p q := by
  unfold ricciActionOnTensor
  rw [ricciSharp_smul hdiff c p]
  simp only [map_smul, ContinuousLinearMap.smul_apply, smul_eq_mul]
  ring
...
/-- **Constant scalar curvature forces divergence-free Ricci**: if `dR = 0` at `x` (the scalar
curvature is critical/locally constant), then `div Ric = 0` there, by the twice-contracted Bianchi
identity `div Ric = ½ dR`. The Ricci tensor of a constant-scalar-curvature metric is divergence-free —
the conservation law underlying Einstein/steady states of the Ricci flow (roadmap item 3). -/
theorem ricciDivergence_eq_zero_of_scalar_const
    {G : E → E →L[ℝ] E →L[ℝ] ℝ} {x : E}
    (hGC2 : ContDiff ℝ 2 G)
    (hGsymm : ∀ (y : E) (p q : E), G y p q = G y q p)
    (hinv : ∀ y : E, (G y).IsInvertible)
    ...
    (hsymΓ : ∀ p : E, IsSymmSndFDerivAt ℝ
      (fun z ↦ christoffelClosedOp G z p) x)
    (hconst : fderiv ℝ (fun y ↦ coordScalar G y) x = 0) (w : E) :
    ricciDivergence G x w = 0 := by
  rw [ricciDivergence_eq_half_fderiv_scalar hGC2 hGsymm hinv hdiffΓ hdd hsymΓ w, hconst]
  simp
...
/-- **The Ricci-divergence 1-form vanishes at a critical point of scalar curvature**: if `dR = 0` at
`x`, then the covector `u ↦ div Ric(u)` is the zero functional, by the twice-contracted Bianchi
identity. The packaged covariant statement of `ricciDivergence_eq_zero_of_scalar_const` (roadmap item 3). -/
theorem ricciDivergenceForm_eq_zero_of_scalar_const
    {G : E → E →L[ℝ] E →L[ℝ] ℝ} {x : E}
    (hGC2 : ContDiff ℝ 2 G)
    (hGsymm : ∀ (y : E) (p q : E), G y p q = G y q p)
    (hinv : ∀ y : E, (G y).IsInvertible)
    ...
    (hsymΓ : ∀ p : E, IsSymmSndFDerivAt ℝ
      (fun z ↦ christoffelClosedOp G z p) x)
    (hconst : fderiv ℝ (fun y ↦ coordScalar G y) x = 0) :
