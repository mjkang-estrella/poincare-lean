...
/-!
# Doubly augmented residual discharge

This module replays `SecondDischarge.lean` one level up.  The generic residual
comparison below is the Frechet-parameter version of the scalar comparison in
`SecondFlowDerivative.lean`: a flow endpoint has derivative `D` when the
linearized endpoint family is represented by `D` and the residual is controlled
by the same compact Taylor/Gronwall argument.

The chart-Christoffel theorem then applies that comparison to the
doubly-augmented field
...
/-- A fixed multiple of `‖h‖` is eventually below any positive threshold. -/
theorem eventually_const_mul_norm_le_nhds_zero_normed {C δ : ℝ}
    (hC : 0 ≤ C) (hδ : 0 < δ) :
    ∀ᶠ h in 𝓝 (0 : P), C * ‖h‖ ≤ δ := by
  have hden : 0 < C + 1 := by linarith
  have hball :
      Metric.ball (0 : P) (δ / (C + 1)) ∈ 𝓝 (0 : P) :=
    Metric.ball_mem_nhds _ (div_pos hδ hden)
  filter_upwards [hball] with h hh
  have hh_norm : ‖h‖ < δ / (C + 1) := by
    simpa [Metric.mem_ball, dist_eq_norm] using hh
  exact le_of_lt <| by
    calc
      C * ‖h‖ ≤ (C + 1) * ‖h‖ :=
        mul_le_mul_of_nonneg_right (by linarith) (norm_nonneg h)
      _ < (C + 1) * (δ / (C + 1)) :=
        mul_lt_mul_of_pos_left hh_norm hden
      _ = δ := by
        field_simp [ne_of_gt hden]

omit [NormedSpace ℝ P] in
/--
Uniform residual little-o on a closed time interval from a Gronwall bound, with
an arbitrary normed parameter space.
-/
theorem residual_uniform_isLittleO_on_Icc_of_gronwall_bound_param
    {R R' : P → ℝ → X} {K T : ℝ}
    (hT : 0 ≤ T) (hK : 0 ≤ K)
    (hRcont : ∀ᶠ h in 𝓝 (0 : P),
      ContinuousOn (R h) (Icc (0 : ℝ) T))
    (hRderiv : ∀ᶠ h in 𝓝 (0 : P),
      ∀ τ ∈ Ico (0 : ℝ) T,
        HasDerivWithinAt (R h) (R' h τ) (Ici τ) τ)
    (hR0 : ∀ᶠ h in 𝓝 (0 : P), R h 0 = 0)
    (hbound : ∀ η > (0 : ℝ), ∀ᶠ h in 𝓝 (0 : P),
      ∀ τ ∈ Ico (0 : ℝ) T,
        ‖R' h τ‖ ≤ K * ‖R h τ‖ + η * ‖h‖) :
    ∀ ε > (0 : ℝ), ∀ᶠ h in 𝓝 (0 : P),
      ∀ t ∈ Icc (0 : ℝ) T, ‖R h t‖ ≤ ε * ‖h‖ := by
  intro ε hε
  ...
  filter_upwards [hRcont, hRderiv, hR0, hbound η hη_pos] with
    h hscont hsderiv hs0 hsbound
  intro t ht
  have hgr :
      ‖R h t‖ ≤ gronwallBound 0 K (η * ‖h‖) t :=
    gronwall_residual_norm_le
      (R := R h) (R' := R' h) (K := K) (η := η * ‖h‖)
      hscont hsderiv hs0 hsbound ht
  calc
    ‖R h t‖ ≤ gronwallBound 0 K (η * ‖h‖) t := hgr
    _ = (η * ‖h‖) * gronwallBound 0 K 1 t := by
    ...
    _ ≤ (η * ‖h‖) * C := by
      ...
      exact mul_le_mul_of_nonneg_left (hmono ht.2)
        (mul_nonneg hη_pos.le (norm_nonneg h))
    _ ≤ ε * ‖h‖ := by
      calc
        (η * ‖h‖) * C = (η * C) * ‖h‖ := by ring
        _ ≤ ε * ‖h‖ := mul_le_mul_of_nonneg_right hηC_le (norm_nonneg h)

omit [NormedSpace ℝ P] [NormedSpace ℝ X] in
/-- Fixed-time little-o extracted from the uniform-on-interval estimate. -/
theorem residual_isLittleO_at_fixedTime_of_uniform_param
    {R : P → ℝ → X} {T t : ℝ}
    (hunif : ∀ ε > (0 : ℝ), ∀ᶠ h in 𝓝 (0 : P),
      ∀ τ ∈ Icc (0 : ℝ) T, ‖R h τ‖ ≤ ε * ‖h‖)
    (ht : t ∈ Icc (0 : ℝ) T) :
    (fun h : P => R h t) =o[𝓝 (0 : P)] (fun h : P => h) := by
  rw [isLittleO_iff]
  intro c hc
  filter_upwards [hunif c hc] with h hh
  exact hh t ht

/--
Frechet endpoint derivative for a local flow from the residual/Gronwall
comparison.

The theorem is intentionally field-agnostic.  The concrete chart-Christoffel
doubly-augmented theorem below supplies the Taylor remainder from `FieldC1`
and the compact Lipschitz constant from the `FlowSmoothness` regularity layer.
-/
theorem flowEndpoint_hasFDerivAt_of_linearized_gronwall
    {F : X → X} {β : X → ℝ → X}
...
omit [T2Space M] in
/--
Fixed-time Frechet derivative for the doubly-augmented
chart-Christoffel flow.

The supplied `β` is the augmented geodesic/first-variation flow, and `Ξ` is
its first variation, i.e. the second-variation solution family.  Pairing them
as `y ↦ (β y.1, Ξ y.1 y.2)` gives a solution of the doubly-augmented system
by direct substitution.  The third-variation family `Ω`, represented at time
`t` by the CLM `D`, is then the derivative of the paired endpoint map.
-/
theorem chartChristoffel_doublyAugmented_endpoint_hasFDerivAt_of_thirdVariation_data
  ...
  have hΩder' : ∀ h : A × A, ∀ τ ∈ Icc (0 : ℝ) T,
      HasDerivWithinAt (Ω h)
        (fderiv ℝ doubleF (Υ y τ) (Ω h τ))
        (Icc (0 : ℝ) T) τ := by
    intro h τ hτ
    simpa [Υ, doubleF, F, Γ] using hΩder h τ hτ
  have hresult :=
    flowEndpoint_hasFDerivAt_of_linearized_gronwall
      (F := doubleF) (β := Υ) (z := y) (Ψ := Ω) (D := D)
      (T := T) (a := a) (K := K) (p := p) (t := t)
      hT hLip hTaylor hbase0 hbase_der hbase_mem hpert_double
      hΩ0 hΩder' hD ht
