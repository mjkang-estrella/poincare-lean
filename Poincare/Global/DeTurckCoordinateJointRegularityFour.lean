...
/-!
# Joint C4 regularity of the coordinate DeTurck field from C5 metric entries
...
`DeTurckCoordinateJointRegularityThree`: it proves the complete regularity budget
...
section FifthOrderSpatialDerivative
...
Frechet derivative. -/
theorem contDiffAt_spatial_fderiv_of_joint_contDiffAt_five_vector
    (F : ℝ → V → W) (t₀ : ℝ) (x : V)
    (hF : ContDiffAt ℝ 5 (Function.uncurry F) (t₀, x)) :
    ContDiffAt ℝ 4
      (fun p : ℝ × V ↦ fderiv ℝ (F p.1) p.2) (t₀, x) := by
  let U : ℝ × V → W := Function.uncurry F
  let p : ℝ × V := (t₀, x)
  have hU : ContDiffAt ℝ 5 U p := by
    simpa [U, p] using hF
  have hDU : ContDiffAt ℝ 4 (fderiv ℝ U) p :=
    hU.fderiv_right (m := 4) (by norm_num)
...
end FifthOrderSpatialDerivative

namespace DeTurckCoordinateJointRegularityFour
...
theorem metricEntriesJointContDiffAt_const_five
    (bg : ClosedSmoothRiemannianMetric n M) (t₀ : ℝ) (x : M) :
    MetricEntriesJointContDiffAt (fun _ : ℝ ↦ bg) t₀ x 5 := by
  intro b c
  ...
  have hpair : ContMDiffAt Iₘ 𝓘(ℝ) 5
      (fun y : M ↦ bg.inner y (extend E b y) (extend E c y)) x := by
    have hinner : ContMDiffAt Iₘ 𝓘(ℝ) 5
        (fun y : M ↦ inner ℝ (extend E b y) (extend E c y)) x :=
      ContMDiffAt.inner_bundle
        (FiberBundle.contMDiffAt_extend' (k := 5) Iₘ E b)
        (FiberBundle.contMDiffAt_extend' (k := 5) Iₘ E c)
  ...
  have heSymm : ContMDiffAt 𝓘(ℝ, E) Iₘ 5 e.symm q := by
    exact (heSymmTop.of_le hfive_le_top).contMDiffAt
  ...
  have hchartMD : ContMDiffAt 𝓘(ℝ, E) 𝓘(ℝ) 5
  ...
  have hjoint := hchart.comp (t₀, q)
    (contDiffAt_snd : ContDiffAt ℝ 5 (Prod.snd : ℝ × E → E) (t₀, q))
...
theorem anchorBlendedMetricFlow_jointContDiffAt_five_of_metricEntries
    {gt : ℝ → ClosedSmoothRiemannianMetric n M} {t₀ : ℝ} {x : M}
    (hJoint : MetricEntriesJointContDiffAt gt t₀ x 5) :
    ContDiffAt ℝ 5 (Function.uncurry (anchorBlendedMetricFlow gt x))
      (t₀, extChartAt Iₘ x x) :=
...
covector. -/
theorem jointChristoffelCovectorAt_contDiffAt_four
    (G : ℝ → E → E →L[ℝ] E →L[ℝ] ℝ)
    (t₀ : ℝ) (z u v : E)
    (hG : ContDiffAt ℝ 5 (Function.uncurry G) (t₀, z)) :
    ContDiffAt ℝ 4
      (fun p : ℝ × E ↦ jointChristoffelCovectorAt G p.1 p.2 u v)
      (t₀, z) := by
  have hD := contDiffAt_spatial_fderiv_of_joint_contDiffAt_five_vector
    G t₀ z hG
  apply contDiffAt_clm_path_of_apply
  intro w
  have hu : ContDiffAt ℝ 4 (fun _ : ℝ × E ↦ u) (t₀, z) :=
    contDiffAt_const
  have hv : ContDiffAt ℝ 4 (fun _ : ℝ × E ↦ v) (t₀, z) :=
    contDiffAt_const
  have hw : ContDiffAt ℝ 4 (fun _ : ℝ × E ↦ w) (t₀, z) :=
    contDiffAt_const
...
flow jointly `C⁴`. -/
theorem anchorChartChristoffelFlow_jointContDiffAt_four_of_metricEntries
    {gt : ℝ → ClosedSmoothRiemannianMetric n M} {t₀ : ℝ} {x : M}
    (hJoint : MetricEntriesJointContDiffAt gt t₀ x 5) :
    ContDiffAt ℝ 4 (Function.uncurry (anchorChartChristoffelFlow gt x))
      (t₀, extChartAt Iₘ x x) := by
  let q : E := extChartAt Iₘ x x
  have hG : ContDiffAt ℝ 5
      (Function.uncurry (anchorBlendedMetricFlow gt x)) (t₀, q) :=
    anchorBlendedMetricFlow_jointContDiffAt_five_of_metricEntries hJoint
  have hG₄ : ContDiffAt ℝ 4
      (Function.uncurry (anchorBlendedMetricFlow gt x)) (t₀, q) :=
    hG.of_le (show (4 : ℕ∞ω) ≤ 5 by norm_num)
  have hInv : ContDiffAt ℝ 4
...
theorem anchorChartChristoffelFieldFlow_jointContDiffAt_four_of_metricEntries
    {gt : ℝ → ClosedSmoothRiemannianMetric n M} {t₀ : ℝ} {x : M}
    (hJoint : MetricEntriesJointContDiffAt gt t₀ x 5) :
    ContDiffAt ℝ 4
      ...
      (t₀, extChartAt Iₘ x x) := by
  have hGamma :=
    anchorChartChristoffelFlow_jointContDiffAt_four_of_metricEntries hJoint
  apply contDiffAt_clm_path_of_apply
  intro u
  apply contDiffAt_clm_path_of_apply
  intro v
  have hu : ContDiffAt ℝ 4
...
theorem anchorBlendedMetricFlow_inverse_jointContDiffAt_four_of_metricEntries
    {gt : ℝ → ClosedSmoothRiemannianMetric n M} {t₀ : ℝ} {x : M}
    (hJoint : MetricEntriesJointContDiffAt gt t₀ x 5) :
    ContDiffAt ℝ 4
      (fun p : ℝ × E ↦ (anchorBlendedMetricFlow gt x p.1 p.2).inverse)
...
theorem anchorChartDeTurckContractionFlow_jointContDiffAt_four_of_metricEntries
    {gt : ℝ → ClosedSmoothRiemannianMetric n M}
    {bg : ClosedSmoothRiemannianMetric n M} {t₀ : ℝ} {x : M}
    (hgt : MetricEntriesJointContDiffAt gt t₀ x 5) :
    ContDiffAt ℝ 4
      (Function.uncurry
  ...
  let q : E := extChartAt Iₘ x x
  have hbg : MetricEntriesJointContDiffAt (fun _ : ℝ ↦ bg) t₀ x 5 :=
    metricEntriesJointContDiffAt_const_five bg t₀ x
  have hGammaG : ContDiffAt ℝ 4
      (Function.uncurry (anchorChartChristoffelFieldFlow gt x)) (t₀, q) := by
    simpa [q] using
      anchorChartChristoffelFieldFlow_jointContDiffAt_four_of_metricEntries hgt
  have hGammaB : ContDiffAt ℝ 4
      (Function.uncurry
        (anchorChartChristoffelFieldFlow (fun _ : ℝ ↦ bg) x)) (t₀, q) := by
    simpa [q] using
      anchorChartChristoffelFieldFlow_jointContDiffAt_four_of_metricEntries hbg
  have hInv : ContDiffAt ℝ 4
      (fun p : ℝ × E ↦ (anchorBlendedMetricFlow gt x p.1 p.2).inverse)
      (t₀, q) := by
    simpa [q] using
      anchorBlendedMetricFlow_inverse_jointContDiffAt_four_of_metricEntries hgt
  unfold DeTurckCoordinateJointRegularity.anchorChartDeTurckContractionFlow
  apply ContDiffAt.sum
  intro i _hi
  let coord : E →L[ℝ] ℝ :=
    LinearMap.toContinuousLinearMap (b.coord i)
  have hcoord : ContDiffAt ℝ 4 (fun _ : ℝ × E ↦ coord) (t₀, q) :=
    contDiffAt_const
  have hraised : ContDiffAt ℝ 4
      (fun p : ℝ × E ↦
        (anchorBlendedMetricFlow gt x p.1 p.2).inverse coord) (t₀, q) :=
    hInv.clm_apply hcoord
  have hbi : ContDiffAt ℝ 4 (fun _ : ℝ × E ↦ b i) (t₀, q) :=
    contDiffAt_const
  have hGammaGi : ContDiffAt ℝ 4
...
theorem deTurckChartCoordinateField_jointContDiffAt_four_of_metricEntries
    {gt : ℝ → ClosedSmoothRiemannianMetric n M}
    {bg : ClosedSmoothRiemannianMetric n M} {t₀ : ℝ} {x : M}
    (hgt : MetricEntriesJointContDiffAt gt t₀ x 5) :
    ContDiffAt ℝ 4
  ...
  have hcontraction :=
    anchorChartDeTurckContractionFlow_jointContDiffAt_four_of_metricEntries
      (bg := bg) hgt
  simpa [q] using hcontraction.congr_of_eventuallyEq heq

end DeTurckCoordinateJointRegularityFour
