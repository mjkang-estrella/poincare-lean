...
/-- Construct the automatic-finite-nerve boundary directly from Bochner norm fields. -/
noncomputable def
...
/-- Construct the automatic-finite-nerve boundary from Bochner norm fields using
canonical Ricci-variation regularity. -/
noncomputable def
...
/-- Construct the automatic-finite-nerve boundary from the joint Bochner fields
using canonical Ricci regularity. -/
noncomputable def
    AutomaticFiniteNerveTetrahedralStarSmoothingReactionDecayHausdorffJointScalarDerivativeCovRicciSubordinatePartitionPositiveEinsteinGenericInverseEndpointODEPrimitiveCompactHistoryBoundaryData3.ofBochnerFieldsCanonical
    {M : Type u} [TopologicalSpace M] [T2Space M]
    [ambientChartedSpace : ChartedSpace (EuclideanSpace ℝ (Fin 3)) M]
    [CompactSpace M] [SimplyConnectedSpace M]
    (tetrahedralStarProvider : FiniteTetrahedralStarPresentationProvider3 M)
