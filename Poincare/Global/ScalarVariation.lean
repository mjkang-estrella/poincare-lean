...
set_option maxHeartbeats 5000000 in
/--
Differentiating the moving Gram coordinate block and contracting the resulting
coordinate derivative against a left tensor slot recovers the moving-field
covariant derivative, with the canonical-extension derivative of `K x` left as
the correction cancelled by the fixed-frame product-rule block.
-/
theorem gram_inv_moving_inner_deriv_contraction_eq_leviCivita_sub_correction
    (g : ClosedSmoothRiemannianMetric n M)
    (h : ∀ y : M, TM y → TM y → ℝ) (x : M)
    {K : ∀ y : M, TM y}
    (hK : MDifferentiableAt I ((I).prod 𝓘(ℝ, E)) (T% K) x)
  ...
  let H : ∀ y : M, TM y → TM y → ℝ :=
    fun y p r ↦ h y p (extend E q y) * g.inner y (extend E (K x) y) r
  have hHAddL : Tensor2AddLeft H := by
    intro y p₁ p₂ r
    dsimp [H]
    rw [hAddL y p₁ p₂ (extend E q y)]
    ring
  have hHSMulL : Tensor2SMulLeft H := by
    intro y c p r
    dsimp [H]
    rw [hSMulL y c p (extend E q y)]
    ring
  have hHAddR : Tensor2AddRight H := by
    intro y p r₁ r₂
    dsimp [H]
    rw [map_add]
    ring
  have hHSMulR : Tensor2SMulRight H := by
    intro y c p r
    dsimp [H]
    rw [map_smul]
  ...
  have hΓK : Γ (K x) = ∑ i, b.coord i (K x) • Γ (b i) := by
    change Γlin (K x) = ∑ i, b.coord i (K x) • Γlin (b i)
    have hrepr : K x = ∑ i, b.coord i (K x) • b i := (b.sum_repr (K x)).symm
    rw [hrepr]
    simpa using map_sum Γlin (fun i ↦ b.coord i (K x) • b i) Finset.univ
  have hcoordK : ∀ i,
      b.coord i (K x) = g.inner x (K x) (sharp i) := by
    intro i
    rw [coord_eq_inner_metricDualVectorAt]
  have hinnerSharp : ∀ (v : TM x) i,
      g.inner x v (sharp i) = ∑ j, G⁻¹ i j * g.inner x v (b j) := by
    intro v i
    calc
      g.inner x v (sharp i) =
          g.inner x v (∑ j, G⁻¹ i j • b j) := by rw [hsharp i]
      _ = ∑ j, G⁻¹ i j * g.inner x v (b j) := by
            have hmap := map_sum (g.inner x v)
              (fun j ↦ G⁻¹ i j • b j) Finset.univ
            simpa [smul_eq_mul] using hmap
  have hNabla :
      (∑ i, ∑ j,
          G⁻¹ i j * g.inner x (g.leviCivita K x u) (b j) *
            h x (b i) q)
        = h x (g.leviCivita K x u) q := by
    calc
      (∑ i, ∑ j,
          G⁻¹ i j * g.inner x (g.leviCivita K x u) (b j) *
            h x (b i) q)
          =
          ∑ i, g.inner x (g.leviCivita K x u) (sharp i) *
            h x (b i) q := by
            refine Finset.sum_congr rfl fun i _hi ↦ ?_
            rw [hinnerSharp (g.leviCivita K x u) i]
            rw [Finset.sum_mul]
      _ = h x (g.leviCivita K x u) q := by
            exact (tensor2_basis_expansion_left
              (g := g) (h := h) hAddL hSMulL x
              (g.leviCivita K x u) q).symm
  have hFrame :
      (∑ i, ∑ j,
          G⁻¹ i j * g.inner x (K x) (Γ (b j)) *
            h x (b i) q)
        =
        ∑ i, h x (b i) q * g.inner x (K x) (Γ (sharp i)) := by
    calc
      (∑ i, ∑ j,
          G⁻¹ i j * g.inner x (K x) (Γ (b j)) *
            h x (b i) q)
          =
          ∑ i,
            (∑ j, G⁻¹ i j * g.inner x (K x) (Γ (b j))) *
              h x (b i) q := by
            refine Finset.sum_congr rfl fun i _hi ↦ ?_
            rw [Finset.sum_mul]
      _ =
          ∑ i, g.inner x (K x) (Γ (sharp i)) *
            h x (b i) q := by
            refine Finset.sum_congr rfl fun i _hi ↦ ?_
            rw [hΓsharp i]
            have hmap := map_sum (g.inner x (K x))
              (fun j ↦ G⁻¹ i j • Γ (b j)) Finset.univ
            simpa [smul_eq_mul] using hmap
      _ =
          ∑ i, h x (b i) q * g.inner x (K x) (Γ (sharp i)) := by
            refine Finset.sum_congr rfl fun i _hi ↦ ?_
  ...
  have hFirstInv :
    ...
    calc
      (∑ i, H x (Γ (b i)) (sharp i))
          =
          ∑ i, b.coord i (K x) * h x (Γ (b i)) q := by
            refine Finset.sum_congr rfl fun i _hi ↦ ?_
            simp [H, sharp, hcoordK i, mul_comm, mul_left_comm, mul_assoc]
      _ = h x (∑ i, b.coord i (K x) • Γ (b i)) q := by
            exact (tensor2_sum_left (h := h) hAddL hSMulL x
              (fun i ↦ b.coord i (K x)) (fun i ↦ Γ (b i)) q).symm
      _ = h x (Γ (K x)) q := by rw [hΓK]
  have hSecondInv :
      (∑ i, H x (b i) (Γ (sharp i))) =
        ∑ i, h x (b i) q * g.inner x (K x) (Γ (sharp i)) := by
    refine Finset.sum_congr rfl fun i _hi ↦ ?_
  ...
  have hInvRaw :=
    gram_inv_deriv_contraction_eq_leviCivita_corrections
      (g := g) (h := H) (x := x)
      hHAddL hHSMulL hHAddR hHSMulR u
  have hInv :
      (∑ i, ∑ j,
        extDerivFun (fun y : M ↦ (gramMatrix g x y)⁻¹ i j) x u *
          g.inner x (K x) (b j) * h x (b i) q)
        =
        -h x (Γ (K x)) q
          - ∑ i, h x (b i) q * g.inner x (K x) (Γ (sharp i)) := by
    have hInv' :
        (∑ i, ∑ j,
          ...
          -∑ i, H x (Γ (b i)) (sharp i)
            - ∑ i, H x (b i) (Γ (sharp i)) := by
      simpa [gramFrame, b, sharp, Γ] using hInvRaw
    calc
      (∑ i, ∑ j,
        extDerivFun (fun y : M ↦ (gramMatrix g x y)⁻¹ i j) x u *
          g.inner x (K x) (b j) * h x (b i) q)
      ...
      _ =
          -∑ i, H x (Γ (b i)) (sharp i)
            - ∑ i, H x (b i) (Γ (sharp i)) := hInv'
      _ =
          -h x (Γ (K x)) q
            - ∑ i, h x (b i) q * g.inner x (K x) (Γ (sharp i)) := by
            rw [hFirstInv, hSecondInv]
  calc
    (∑ i, ∑ j,
      extDerivFun
        (fun y : M ↦ (gramMatrix g x y)⁻¹ i j *
      ...
      (∑ i, ∑ j,
        ...
        + (∑ i, ∑ j,
          extDerivFun (fun y : M ↦ (gramMatrix g x y)⁻¹ i j) x u *
            g.inner x (K x) (b j) * h x (b i) q) := by
        simp_rw [gram_inv_moving_inner_extDerivFun_eq_product
          (g := g) (K := K) hK u]
        simp [gramFrame, b, G, Γ, Finset.sum_add_distrib, mul_add,
          add_mul, mul_assoc]
    _ =
      h x (g.leviCivita K x u) q
        + (∑ i, h x (b i) q * g.inner x (K x) (Γ (sharp i)))
        + (-h x (Γ (K x)) q
          - ∑ i, h x (b i) q * g.inner x (K x) (Γ (sharp i))) := by
        rw [hNabla, hFrame, hInv]
    _ =
      h x (g.leviCivita K x u) q
        - h x (g.leviCivita (extend E (K x)) x u) q := by
        simp [Γ]
...
theorem tensor2_moving_left_mdiffAt
    (g : ClosedSmoothRiemannianMetric n M)
    (h : ∀ y : M, TM y → TM y → ℝ) (x : M)
    {K : ∀ y : M, TM y}
    (hK : MDifferentiableAt I ((I).prod 𝓘(ℝ, E)) (T% K) x)
    ...
    (q : TM x) :
    MDifferentiableAt I 𝓘(ℝ)
      (fun y : M ↦ h y (K y) (extend E q y)) x := by
  classical
  letI : FiniteDimensional ℝ (TM x) :=
    inferInstanceAs (FiniteDimensional ℝ E)
  let b := Module.finBasis ℝ (TM x)
  ...
  have htermDiff : ∀ i,
      MDifferentiableAt I 𝓘(ℝ)
        (fun y : M ↦
          (∑ j, (gramMatrix g x y)⁻¹ i j *
              g.inner y (K y) (gramFrame x y j)) *
            h y (gramFrame x y i) (extend E q y)) x := by
    intro i
    ...
    have hH : MDifferentiableAt I 𝓘(ℝ)
        (fun y : M ↦ h y (gramFrame x y i) (extend E q y)) x := by
      simpa [gramFrame, b, CovTensor2ExtDifferentiableAt] using hDiff (b i) q
    exact hC.mul hH
  have hrhs : MDifferentiableAt I 𝓘(ℝ) rhs x := by
    have hsum :
        MDifferentiableAt I 𝓘(ℝ)
          (∑ i, fun y : M ↦
            (∑ j, (gramMatrix g x y)⁻¹ i j *
                g.inner y (K y) (gramFrame x y j)) *
              h y (gramFrame x y i) (extend E q y)) x :=
      MDifferentiableAt.sum
        (t := (Finset.univ : Finset (Fin (Module.finrank ℝ (TM x)))))
        (fun i _hi ↦ htermDiff i)
    exact hsum.congr_of_eventuallyEq
      (Filter.Eventually.of_forall fun y ↦ by simp [rhs])
  exact hrhs.congr_of_eventuallyEq hevent

theorem tensor2_moving_right_mdiffAt
    (g : ClosedSmoothRiemannianMetric n M)
    (h : ∀ y : M, TM y → TM y → ℝ) (x : M)
    {K : ∀ y : M, TM y}
    (hK : MDifferentiableAt I ((I).prod 𝓘(ℝ, E)) (T% K) x)
    ...
    (p : TM x) :
    MDifferentiableAt I 𝓘(ℝ)
      (fun y : M ↦ h y (extend E p y) (K y)) x := by
  classical
  letI : FiniteDimensional ℝ (TM x) :=
    inferInstanceAs (FiniteDimensional ℝ E)
  let b := Module.finBasis ℝ (TM x)
  let rhs : M → ℝ := fun y : M ↦
    ∑ i, (∑ j, (gramMatrix g x y)⁻¹ i j *
        g.inner y (K y) (gramFrame x y j)) *
      h y (extend E p y) (gramFrame x y i)
  have hevent :
      (fun y : M ↦ h y (extend E p y) (K y)) =ᶠ[nhds x] rhs := by
    simpa [rhs] using
      tensor2_moving_right_eventually_eq_sum_gram_inv
        (g := g) (h := h) hAddR hSMulR x p K
  have hcoeffDiff : ∀ i j,
      MDifferentiableAt I 𝓘(ℝ)
        (fun y : M ↦ (gramMatrix g x y)⁻¹ i j *
          g.inner y (K y) (gramFrame x y j)) x := by
  ...
  have htermDiff : ∀ i,
      MDifferentiableAt I 𝓘(ℝ)
        (fun y : M ↦
          (∑ j, (gramMatrix g x y)⁻¹ i j *
              g.inner y (K y) (gramFrame x y j)) *
            h y (extend E p y) (gramFrame x y i)) x := by
    intro i
    have hC : MDifferentiableAt I 𝓘(ℝ)
        (fun y : M ↦ ∑ j, (gramMatrix g x y)⁻¹ i j *
          g.inner y (K y) (gramFrame x y j)) x := by
      ...
      exact hsum.congr_of_eventuallyEq
        (Filter.Eventually.of_forall fun y ↦ by simp)
    have hH : MDifferentiableAt I 𝓘(ℝ)
        (fun y : M ↦ h y (extend E p y) (gramFrame x y i)) x := by
      simpa [gramFrame, b, CovTensor2ExtDifferentiableAt] using hDiff p (b i)
    exact hC.mul hH
  have hrhs : MDifferentiableAt I 𝓘(ℝ) rhs x := by
    have hsum :
        MDifferentiableAt I 𝓘(ℝ)
          (∑ i, fun y : M ↦
            (∑ j, (gramMatrix g x y)⁻¹ i j *
                g.inner y (K y) (gramFrame x y j)) *
              h y (extend E p y) (gramFrame x y i)) x :=
      MDifferentiableAt.sum
        (t := (Finset.univ : Finset (Fin (Module.finrank ℝ (TM x)))))
        (fun i _hi ↦ htermDiff i)
    exact hsum.congr_of_eventuallyEq
...
theorem tensor2_moving_both_eventually_eq_sum_gram_inv
    {h : ∀ y : M, TM y → TM y → ℝ}
    (g : ClosedSmoothRiemannianMetric n M)
    (hAddL : Tensor2AddLeft h) (hSMulL : Tensor2SMulLeft h)
    (x : M) (K L : ∀ y : M, TM y) :
    (fun y : M ↦ h y (K y) (L y)) =ᶠ[nhds x]
      fun y : M ↦
        ∑ i, (∑ j, (gramMatrix g x y)⁻¹ i j *
            g.inner y (K y) (gramFrame x y j)) *
          h y (gramFrame x y i) (L y) := by
  classical
  filter_upwards [gramMatrix_eventually_isUnit (g := g) x] with y hG
  simpa using
    tensor2_gramFrame_expansion_left
...
set_option maxHeartbeats 5000000 in
theorem moving_both_gram_rhs_extDerivFun_eq_sum_product
    (g : ClosedSmoothRiemannianMetric n M)
    (h : ∀ y : M, TM y → TM y → ℝ) (x : M)
    {K L : ∀ y : M, TM y}
    (hK : MDifferentiableAt I ((I).prod 𝓘(ℝ, E)) (T% K) x)
    (hL : MDifferentiableAt I ((I).prod 𝓘(ℝ, E)) (T% L) x)
    (hDiff : CovTensor2ExtDifferentiableAt h x)
    (hAddR : Tensor2AddRight h) (hSMulR : Tensor2SMulRight h)
    (u : TM x) :
    extDerivFun
      ...
      ∑ i,
        (∑ j, (gramMatrix g x x)⁻¹ i j *
            g.inner x (K x) (gramFrame x x j)) *
          extDerivFun
            (fun y : M ↦ h y (gramFrame x y i) (L y)) x u
        + ∑ i, ∑ j,
            extDerivFun
              (fun y : M ↦ (gramMatrix g x y)⁻¹ i j *
                g.inner y (K y) (gramFrame x y j)) x u *
              h x (gramFrame x x i) (L x)) := by
  classical
  letI : FiniteDimensional ℝ (TM x) :=
    inferInstanceAs (FiniteDimensional ℝ E)
  let b := Module.finBasis ℝ (TM x)
  ...
  let C : Fin (Module.finrank ℝ (TM x)) → M → ℝ :=
    fun i ↦ ∑ j, coeff i j
  let H : Fin (Module.finrank ℝ (TM x)) → M → ℝ :=
    fun i y ↦ h y (gramFrame x y i) (L y)
  let term : Fin (Module.finrank ℝ (TM x)) → M → ℝ :=
    fun i y ↦ C i y * H i y
  have hcoeffDiff : ∀ i j,
      MDifferentiableAt I 𝓘(ℝ) (coeff i j) x := by
  ...
  have hHDiff : ∀ i,
      MDifferentiableAt I 𝓘(ℝ) (H i) x := by
    intro i
    simpa [H, gramFrame, b] using
      tensor2_moving_right_mdiffAt
        (g := g) (h := h) (x := x) (K := L)
        hL hDiff hAddR hSMulR (b i)
  have htermDiff : ∀ i,
      MDifferentiableAt I 𝓘(ℝ) (term i) x := by
    intro i
    exact (hCDiff i).mul (hHDiff i)
...
set_option maxHeartbeats 5000000 in
theorem gram_h_extDerivFun_moving_both_contraction_eq_covTensor2DerivAt_add_corrections
    (g : ClosedSmoothRiemannianMetric n M)
    (h : ∀ y : M, TM y → TM y → ℝ) (x : M)
    {L : ∀ y : M, TM y}
    (hL : MDifferentiableAt I ((I).prod 𝓘(ℝ, E)) (T% L) x)
      ...
      ∑ i,
        (∑ j, (gramMatrix g x x)⁻¹ i j *
          ...
          extDerivFun
            (fun y : M ↦ h y (gramFrame x y i) (L y)) x u)
      =
      covTensor2DerivAt g h x u p (L x)
        + h x (g.leviCivita (extend E p) x u) (L x)
        + h x p (g.leviCivita L x u) := by
  classical
  letI : FiniteDimensional ℝ (TM x) := inferInstanceAs (FiniteDimensional ℝ E)
  let b := Module.finBasis ℝ (TM x)
  let G : Matrix (Fin (Module.finrank ℝ (TM x)))
      (Fin (Module.finrank ℝ (TM x))) ℝ := gramMatrix g x x
  let Γ : TM x → TM x := fun r ↦ g.leviCivita (extend E r) x u
  let ΓL : TM x := g.leviCivita L x u
  let c : Fin (Module.finrank ℝ (TM x)) → ℝ :=
    fun i ↦ ∑ j, G⁻¹ i j * g.inner x p (b j)
  let sharp : Fin (Module.finrank ℝ (TM x)) → TM x :=
    fun i ↦ metricDualVectorAt g x (b.coord i)
  ...
  have hcov :
      covTensor2DerivAt g h x u p (L x) =
        ∑ i, b.coord i p * covTensor2DerivAt g h x u (b i) (L x) := by
    calc
      covTensor2DerivAt g h x u p (L x) =
          covTensor2DerivAt g h x u (∑ i, b.coord i p • b i) (L x) := by
            exact congrArg (fun r ↦ covTensor2DerivAt g h x u r (L x)) hp_repr
      _ = ∑ i, b.coord i p * covTensor2DerivAt g h x u (b i) (L x) :=
            covTensor2DerivAt_sum_left
              (g := g) (h := h) (x := x) hDiff hAddL hSMulL
              u (L x) (fun i ↦ b.coord i p) b
  have hΓcorr :
      h x (Γ p) (L x) =
        ∑ i, b.coord i p * h x (Γ (b i)) (L x) := by
    calc
      h x (Γ p) (L x) = h x (∑ i, b.coord i p • Γ (b i)) (L x) := by
        rw [hΓp]
      _ = ∑ i, b.coord i p * h x (Γ (b i)) (L x) :=
            tensor2_sum_left (h := h) hAddL hSMulL x
              (fun i ↦ b.coord i p) (fun i ↦ Γ (b i)) (L x)
  have hslotcorr :
      h x p ΓL =
        ∑ i, b.coord i p * h x (b i) ΓL := by
    calc
      h x p ΓL = h x (∑ i, b.coord i p • b i) ΓL := by
        exact congrArg (fun r ↦ h x r ΓL) hp_repr
      _ = ∑ i, b.coord i p * h x (b i) ΓL :=
            tensor2_sum_left (h := h) hAddL hSMulL x
              (fun i ↦ b.coord i p) b ΓL
  have hentry : ∀ i,
      extDerivFun
          (fun y : M ↦ h y (gramFrame x y i) (L y)) x u =
        covTensor2DerivAt g h x u (b i) (L x)
          + h x (Γ (b i)) (L x)
          + h x (b i) ΓL := by
    intro i
    have hbase :=
      tensor2_moving_right_extDerivFun_eq_covTensor2DerivAt_add_corrections
        (g := g) (h := h) (x := x) (K := L)
        hL hDiff hAddR hSMulR u (b i)
    simpa [gramFrame, b, Γ, ΓL] using hbase
  calc
    ∑ i,
        (∑ j, (gramMatrix g x x)⁻¹ i j * g.inner x p (gramFrame x x j)) *
          extDerivFun
        ...
        ∑ i,
          ...
          rw [hcoef]
    _ =
        (∑ i, b.coord i p * covTensor2DerivAt g h x u (b i) (L x))
          + (∑ i, b.coord i p * h x (Γ (b i)) (L x))
          + (∑ i, b.coord i p * h x (b i) ΓL) := by
          simp_rw [mul_add]
          simp [Finset.sum_add_distrib]
    _ =
        covTensor2DerivAt g h x u p (L x) + h x (Γ p) (L x) + h x p ΓL := by
          rw [← hcov, ← hΓcorr, ← hslotcorr]
    _ =
        covTensor2DerivAt g h x u p (L x)
          + h x (g.leviCivita (extend E p) x u) (L x)
...
set_option maxHeartbeats 5000000 in
theorem tensor2_moving_both_extDerivFun_eq_covTensor2DerivAt_add_corrections
    (g : ClosedSmoothRiemannianMetric n M)
    (h : ∀ y : M, TM y → TM y → ℝ) (x : M)
    {K L : ∀ y : M, TM y}
    (hK : MDifferentiableAt I ((I).prod 𝓘(ℝ, E)) (T% K) x)
    ...
    (hAddR : Tensor2AddRight h) (hSMulR : Tensor2SMulRight h)
    (u : TM x) :
    extDerivFun (fun y : M ↦ h y (K y) (L y)) x u =
      covTensor2DerivAt g h x u (K x) (L x)
        + h x (g.leviCivita K x u) (L x)
        + h x (K x) (g.leviCivita L x u) := by
  classical
  letI : FiniteDimensional ℝ (TM x) :=
    inferInstanceAs (FiniteDimensional ℝ E)
  let rhs : M → ℝ := fun y : M ↦
    ∑ i, (∑ j, (gramMatrix g x y)⁻¹ i j *
      ...
      h y (gramFrame x y i) (L y)
  have hevent :
      (fun y : M ↦ h y (K y) (L y)) =ᶠ[nhds x] rhs := by
    simpa [rhs] using
      tensor2_moving_both_eventually_eq_sum_gram_inv
        (g := g) (h := h) hAddL hSMulL x K L
  have hcongr := congrArg (fun L' : TM x →L[ℝ] ℝ ↦ L' u)
    (CovariantDerivative.extDerivFun_congr hevent)
  have hprod :=
    moving_both_gram_rhs_extDerivFun_eq_sum_product
      (g := g) (h := h) (x := x) (K := K) (L := L)
      hK hL hDiff hAddR hSMulR u
  have hfixed :=
    gram_h_extDerivFun_moving_both_contraction_eq_covTensor2DerivAt_add_corrections
      (g := g) (h := h) (x := x) (L := L)
      hL hDiff hAddL hSMulL hAddR hSMulR u (K x)
  have hcoeff :=
    gram_inv_moving_inner_deriv_contraction_eq_leviCivita_sub_correction
      (g := g) (h := h) (x := x) (K := K)
      hK hAddL hSMulL u (L x)
  calc
    extDerivFun (fun y : M ↦ h y (K y) (L y)) x u =
        extDerivFun rhs x u := by
          exact hcongr
    ...
    _ =
        (covTensor2DerivAt g h x u (K x) (L x)
          + h x (g.leviCivita (extend E (K x)) x u) (L x)
          + h x (K x) (g.leviCivita L x u))
          + (h x (g.leviCivita K x u) (L x)
            - h x (g.leviCivita (extend E (K x)) x u) (L x)) := by
          rw [hfixed, hcoeff]
    _ =
        covTensor2DerivAt g h x u (K x) (L x)
          + h x (g.leviCivita K x u) (L x)
...
noncomputable def ricciPairingInvFirstContractionTensor
  ...
  fun y p q ↦
    ...
    ∑ b', ∑ d,
      (gramMatrix g x x)⁻¹ b' d *
        g.ricciAt y p (extend E (b b') y) *
        g.ricciAt y q (extend E (b d) y)

/-- Tensor contracted against the second inverse-Gram derivative group. -/
noncomputable def ricciPairingInvSecondContractionTensor
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M) : ∀ y : M, TM y → TM y → ℝ :=
  fun y p q ↦
    ...
    ∑ a, ∑ c,
      (gramMatrix g x x)⁻¹ a c *
        g.ricciAt y (extend E (b a) y) p *
        g.ricciAt y (extend E (b c) y) q

/-- The two Levi-Civita correction terms produced by the first inverse-Gram derivative. -/
noncomputable def ricciPairingInvFirstLeviCorrectionGroup
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M) (v : TM x) : ℝ :=
  letI : FiniteDimensional ℝ (TM x) :=
  ...
  let sharp : Fin (Module.finrank ℝ (TM x)) → TM x :=
    fun i ↦ metricDualVectorAt g x (b.coord i)
  let Γ : TM x → TM x := fun p ↦ g.leviCivita (extend E p) x v
  let H := ricciPairingInvFirstContractionTensor g x
  ∑ i, H x (Γ (b i)) (sharp i)
    + ∑ i, H x (b i) (Γ (sharp i))

/-- The two Levi-Civita correction terms produced by the second inverse-Gram derivative. -/
noncomputable def ricciPairingInvSecondLeviCorrectionGroup
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M) (v : TM x) : ℝ :=
  letI : FiniteDimensional ℝ (TM x) :=
  ...
  let sharp : Fin (Module.finrank ℝ (TM x)) → TM x :=
    fun i ↦ metricDualVectorAt g x (b.coord i)
  let Γ : TM x → TM x := fun p ↦ g.leviCivita (extend E p) x v
  let H := ricciPairingInvSecondContractionTensor g x
  ∑ i, H x (Γ (b i)) (sharp i)
    + ∑ i, H x (b i) (Γ (sharp i))

set_option maxHeartbeats 5000000 in
private theorem tensor2AddLeft_ricciPairingInvFirstContractionTensor
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M) :
    Tensor2AddLeft (ricciPairingInvFirstContractionTensor g x) := by
  intro y p₁ p₂ q
  classical
  unfold ricciPairingInvFirstContractionTensor
  simp [g.ricciAt_add_left y, Finset.sum_add_distrib, mul_add, add_mul,
    mul_assoc]

set_option maxHeartbeats 5000000 in
private theorem tensor2SMulLeft_ricciPairingInvFirstContractionTensor
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M) :
    Tensor2SMulLeft (ricciPairingInvFirstContractionTensor g x) := by
  intro y c p q
  classical
  unfold ricciPairingInvFirstContractionTensor
...
set_option maxHeartbeats 5000000 in
private theorem tensor2AddRight_ricciPairingInvFirstContractionTensor
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M) :
    Tensor2AddRight (ricciPairingInvFirstContractionTensor g x) := by
  intro y p q₁ q₂
  classical
  unfold ricciPairingInvFirstContractionTensor
  simp [g.ricciAt_add_left y, Finset.sum_add_distrib, mul_add, add_mul,
...
set_option maxHeartbeats 5000000 in
private theorem tensor2SMulRight_ricciPairingInvFirstContractionTensor
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M) :
    Tensor2SMulRight (ricciPairingInvFirstContractionTensor g x) := by
  intro y c p q
  classical
  unfold ricciPairingInvFirstContractionTensor
  simp [g.ricciAt_smul_left y, Finset.mul_sum, mul_comm, mul_left_comm,
...
set_option maxHeartbeats 5000000 in
private theorem tensor2AddLeft_ricciPairingInvSecondContractionTensor
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M) :
    Tensor2AddLeft (ricciPairingInvSecondContractionTensor g x) := by
  intro y p₁ p₂ q
  classical
  unfold ricciPairingInvSecondContractionTensor
  simp [g.ricciAt_add_right y, Finset.sum_add_distrib, mul_add, add_mul,
    mul_assoc]

set_option maxHeartbeats 5000000 in
private theorem tensor2SMulLeft_ricciPairingInvSecondContractionTensor
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M) :
    Tensor2SMulLeft (ricciPairingInvSecondContractionTensor g x) := by
  intro y c p q
  classical
  unfold ricciPairingInvSecondContractionTensor
...
set_option maxHeartbeats 5000000 in
private theorem tensor2AddRight_ricciPairingInvSecondContractionTensor
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M) :
    Tensor2AddRight (ricciPairingInvSecondContractionTensor g x) := by
  intro y p q₁ q₂
  classical
  unfold ricciPairingInvSecondContractionTensor
  simp [g.ricciAt_add_right y, Finset.sum_add_distrib, mul_add, add_mul,
...
set_option maxHeartbeats 5000000 in
private theorem tensor2SMulRight_ricciPairingInvSecondContractionTensor
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M) :
    Tensor2SMulRight (ricciPairingInvSecondContractionTensor g x) := by
  intro y c p q
  classical
  unfold ricciPairingInvSecondContractionTensor
  simp [g.ricciAt_smul_right y, Finset.mul_sum, mul_comm, mul_left_comm,
...
set_option maxHeartbeats 5000000 in
/--
The two inverse-Gram product-rule groups are exactly the corresponding
Levi-Civita slot-correction contractions, with the sign from differentiating
the inverse Gram matrix.
-/
theorem pairing_gram_inv_deriv_groups_eq_leviCivita_corrections
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M) (v : TM x) :
    ricciPairingGramInvFirstDerivGroup g x v =
        -ricciPairingInvFirstLeviCorrectionGroup g x v
      ∧
      ricciPairingGramInvSecondDerivGroup g x v =
        -ricciPairingInvSecondLeviCorrectionGroup g x v := by
  classical
  letI : FiniteDimensional ℝ (TM x) :=
    inferInstanceAs (FiniteDimensional ℝ E)
  let b := Module.finBasis ℝ (TM x)
  ...
  · let H := ricciPairingInvFirstContractionTensor g x
    have hraw :=
      gram_inv_deriv_contraction_eq_leviCivita_corrections
        (g := g) (h := H) (x := x)
        (tensor2AddLeft_ricciPairingInvFirstContractionTensor (g := g) x)
        (tensor2SMulLeft_ricciPairingInvFirstContractionTensor (g := g) x)
        (tensor2AddRight_ricciPairingInvFirstContractionTensor (g := g) x)
        (tensor2SMulRight_ricciPairingInvFirstContractionTensor (g := g) x)
        v
    have hlhs :
        ricciPairingGramInvFirstDerivGroup g x v =
          ∑ i, ∑ j,
      ...
      let D : Fin (Module.finrank ℝ (TM x)) →
          Fin (Module.finrank ℝ (TM x)) → ℝ :=
        fun i j ↦ extDerivFun (fun y : M ↦ (gramMatrix g x y)⁻¹ i j) x v
      let C : Fin (Module.finrank ℝ (TM x)) →
          Fin (Module.finrank ℝ (TM x)) → ℝ :=
        fun i j ↦ (gramMatrix g x x)⁻¹ i j
      let R : TM x → TM x → ℝ := fun p q ↦ g.ricciAt x p q
      unfold ricciPairingGramInvFirstDerivGroup H
      unfold ricciPairingInvFirstContractionTensor
      simp only [gramFrame, extend_apply_self]
      change
        (∑ a, ∑ b', ∑ c, ∑ d,
          D a c * C b' d * R (b a) (b b') * R (b c) (b d))
          =
        ∑ i, ∑ j, D i j *
          (∑ b', ∑ d, C b' d * R (b i) (b b') * R (b j) (b d))
      calc
        (∑ a, ∑ b', ∑ c, ∑ d,
          D a c * C b' d * R (b a) (b b') * R (b c) (b d))
          =
          ∑ a, ∑ c, ∑ b', ∑ d,
            D a c * C b' d * R (b a) (b b') * R (b c) (b d) := by
            refine Finset.sum_congr rfl fun a _ ↦ ?_
            rw [Finset.sum_comm]
        _ =
          ∑ i, ∑ j, D i j *
            (∑ b', ∑ d, C b' d * R (b i) (b b') * R (b j) (b d)) := by
            refine Finset.sum_congr rfl fun i _ ↦ ?_
            refine Finset.sum_congr rfl fun j _ ↦ ?_
            rw [Finset.mul_sum]
            refine Finset.sum_congr rfl fun b' _ ↦ ?_
    ...
    calc
      ...
      _ =
          -∑ i, H x (g.leviCivita (extend E (b i)) x v)
              (metricDualVectorAt g x (b.coord i))
            - ∑ i, H x (b i)
              (g.leviCivita
                (extend E (metricDualVectorAt g x (b.coord i))) x v) := by
          simpa [H, b] using hraw
      _ = -ricciPairingInvFirstLeviCorrectionGroup g x v := by
          unfold ricciPairingInvFirstLeviCorrectionGroup
          simp [H, b]
          ring
  · let H := ricciPairingInvSecondContractionTensor g x
    have hraw :=
      gram_inv_deriv_contraction_eq_leviCivita_corrections
        (g := g) (h := H) (x := x)
        (tensor2AddLeft_ricciPairingInvSecondContractionTensor (g := g) x)
        (tensor2SMulLeft_ricciPairingInvSecondContractionTensor (g := g) x)
        (tensor2AddRight_ricciPairingInvSecondContractionTensor (g := g) x)
        (tensor2SMulRight_ricciPairingInvSecondContractionTensor (g := g) x)
        v
    have hlhs :
        ricciPairingGramInvSecondDerivGroup g x v =
          ∑ i, ∑ j,
      ...
      let C : Fin (Module.finrank ℝ (TM x)) →
        ...
        fun i j ↦ (gramMatrix g x x)⁻¹ i j
      let R : TM x → TM x → ℝ := fun p q ↦ g.ricciAt x p q
      unfold ricciPairingGramInvSecondDerivGroup H
      unfold ricciPairingInvSecondContractionTensor
      simp only [gramFrame, extend_apply_self]
      change
        (∑ a, ∑ b', ∑ c, ∑ d,
          C a c * D b' d * R (b a) (b b') * R (b c) (b d))
          =
        ∑ i, ∑ j, D i j *
          (∑ a, ∑ c, C a c * R (b a) (b i) * R (b c) (b j))
      calc
        (∑ a, ∑ b', ∑ c, ∑ d,
          C a c * D b' d * R (b a) (b b') * R (b c) (b d))
          =
          ∑ b', ∑ a, ∑ c, ∑ d,
            C a c * D b' d * R (b a) (b b') * R (b c) (b d) := by
            rw [Finset.sum_comm]
        _ =
          ∑ b', ∑ d, ∑ a, ∑ c,
            C a c * D b' d * R (b a) (b b') * R (b c) (b d) := by
        ...
        _ =
          ∑ i, ∑ j, D i j *
            (∑ a, ∑ c, C a c * R (b a) (b i) * R (b c) (b j)) := by
            refine Finset.sum_congr rfl fun i _ ↦ ?_
            refine Finset.sum_congr rfl fun j _ ↦ ?_
            rw [Finset.mul_sum]
            refine Finset.sum_congr rfl fun a _ ↦ ?_
          ...
          -∑ i, H x (g.leviCivita (extend E (b i)) x v)
            ...
            - ∑ i, H x (b i)
              (g.leviCivita
                (extend E (metricDualVectorAt g x (b.coord i))) x v) := by
          simpa [H, b] using hraw
      _ = -ricciPairingInvSecondLeviCorrectionGroup g x v := by
          unfold ricciPairingInvSecondLeviCorrectionGroup
          simp [H, b]
          ring

/-- The covariant-Ricci part produced by differentiating the first Ricci entry. -/
noncomputable def ricciPairingRicciFirstCovariantGroup
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M) (v : TM x) : ℝ :=
  letI : FiniteDimensional ℝ (TM x) :=
...
/-- The covariant-Ricci part produced by differentiating the second Ricci entry. -/
noncomputable def ricciPairingRicciSecondCovariantGroup
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M) (v : TM x) : ℝ :=
  letI : FiniteDimensional ℝ (TM x) :=
...
/-- Levi-Civita correction from the first slot of the first Ricci entry. -/
noncomputable def ricciPairingRicciFirstLeftLeviCorrectionGroup
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M) (v : TM x) : ℝ :=
  letI : FiniteDimensional ℝ (TM x) :=
  ...
  ∑ a, ∑ b', ∑ c, ∑ d,
    (gramMatrix g x x)⁻¹ a c *
      (gramMatrix g x x)⁻¹ b' d *
      g.ricciAt x (Γ (b a)) (b b') *
      g.ricciAt x (b c) (b d)

/-- Levi-Civita correction from the second slot of the first Ricci entry. -/
noncomputable def ricciPairingRicciFirstRightLeviCorrectionGroup
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M) (v : TM x) : ℝ :=
  letI : FiniteDimensional ℝ (TM x) :=
  ...
  ∑ a, ∑ b', ∑ c, ∑ d,
    (gramMatrix g x x)⁻¹ a c *
      (gramMatrix g x x)⁻¹ b' d *
      g.ricciAt x (b a) (Γ (b b')) *
      g.ricciAt x (b c) (b d)

/-- Levi-Civita correction from the first slot of the second Ricci entry. -/
noncomputable def ricciPairingRicciSecondLeftLeviCorrectionGroup
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M) (v : TM x) : ℝ :=
  letI : FiniteDimensional ℝ (TM x) :=
  ...
  ∑ a, ∑ b', ∑ c, ∑ d,
    (gramMatrix g x x)⁻¹ a c *
      (gramMatrix g x x)⁻¹ b' d *
      g.ricciAt x (b a) (b b') *
      g.ricciAt x (Γ (b c)) (b d)

/-- Levi-Civita correction from the second slot of the second Ricci entry. -/
noncomputable def ricciPairingRicciSecondRightLeviCorrectionGroup
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M) (v : TM x) : ℝ :=
  letI : FiniteDimensional ℝ (TM x) :=
  ...
  ∑ a, ∑ b', ∑ c, ∑ d,
    (gramMatrix g x x)⁻¹ a c *
      (gramMatrix g x x)⁻¹ b' d *
      g.ricciAt x (b a) (b b') *
      g.ricciAt x (b c) (Γ (b d))

/-- The two Levi-Civita corrections produced by the first Ricci-entry derivative. -/
noncomputable def ricciPairingRicciFirstLeviCorrectionGroup
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M) (v : TM x) : ℝ :=
  ricciPairingRicciFirstLeftLeviCorrectionGroup g x v
    + ricciPairingRicciFirstRightLeviCorrectionGroup g x v

/-- The two Levi-Civita corrections produced by the second Ricci-entry derivative. -/
noncomputable def ricciPairingRicciSecondLeviCorrectionGroup
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M) (v : TM x) : ℝ :=
  ricciPairingRicciSecondLeftLeviCorrectionGroup g x v
    + ricciPairingRicciSecondRightLeviCorrectionGroup g x v

set_option maxHeartbeats 5000000 in
/--
The Ricci-entry derivative product-rule groups split into their covariant
Ricci derivative terms and the two canonical Levi-Civita slot corrections.
-/
theorem pairing_ricci_deriv_groups_eq_covariant_plus_leviCivita_corrections
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M) (v : TM x) :
    ricciPairingRicciFirstDerivGroup g x v =
        ricciPairingRicciFirstCovariantGroup g x v
          + ricciPairingRicciFirstLeviCorrectionGroup g x v
      ∧
      ricciPairingRicciSecondDerivGroup g x v =
        ricciPairingRicciSecondCovariantGroup g x v
          + ricciPairingRicciSecondLeviCorrectionGroup g x v := by
  classical
  letI : FiniteDimensional ℝ (TM x) :=
    inferInstanceAs (FiniteDimensional ℝ E)
  let b := Module.finBasis ℝ (TM x)
  ...
  · have hsplit :
        ricciPairingRicciFirstDerivGroup g x v =
          ∑ a, ∑ b', ∑ c, ∑ d,
            (gramMatrix g x x)⁻¹ a c *
              (gramMatrix g x x)⁻¹ b' d *
              (closedCovRicciDerivAt g x v (b a) (b b')
                + g.ricciAt x (g.leviCivita (extend E (b a)) x v) (b b')
                + g.ricciAt x (b a)
                    (g.leviCivita (extend E (b b')) x v)) *
      ...
      refine Finset.sum_congr rfl fun c _ ↦ ?_
      refine Finset.sum_congr rfl fun d _ ↦ ?_
      rw [closedRicciDerivativeExpansionAt_canonical (g := g) x v (b a) (b b')]
    calc
      ricciPairingRicciFirstDerivGroup g x v =
          ∑ a, ∑ b', ∑ c, ∑ d,
            (gramMatrix g x x)⁻¹ a c *
              ...
              (closedCovRicciDerivAt g x v (b a) (b b')
                ...
                + g.ricciAt x (b a)
                    (g.leviCivita (extend E (b b')) x v)) *
              g.ricciAt x (b c) (b d) := hsplit
      _ = ricciPairingRicciFirstCovariantGroup g x v
            + ricciPairingRicciFirstLeviCorrectionGroup g x v := by
          unfold ricciPairingRicciFirstCovariantGroup
          unfold ricciPairingRicciFirstLeviCorrectionGroup
          unfold ricciPairingRicciFirstLeftLeviCorrectionGroup
          unfold ricciPairingRicciFirstRightLeviCorrectionGroup
          simp [b, Finset.sum_add_distrib, mul_add, add_mul, add_assoc]
  · have hsplit :
        ricciPairingRicciSecondDerivGroup g x v =
          ∑ a, ∑ b', ∑ c, ∑ d,
            (gramMatrix g x x)⁻¹ a c *
              ...
              g.ricciAt x (b a) (b b') *
              (closedCovRicciDerivAt g x v (b c) (b d)
                + g.ricciAt x (g.leviCivita (extend E (b c)) x v) (b d)
                + g.ricciAt x (b c)
                    (g.leviCivita (extend E (b d)) x v)) := by
      unfold ricciPairingRicciSecondDerivGroup
      simp only [gramFrame, extend_apply_self]
      refine Finset.sum_congr rfl fun a _ ↦ ?_
      refine Finset.sum_congr rfl fun b' _ ↦ ?_
      refine Finset.sum_congr rfl fun c _ ↦ ?_
      refine Finset.sum_congr rfl fun d _ ↦ ?_
      rw [closedRicciDerivativeExpansionAt_canonical (g := g) x v (b c) (b d)]
    calc
      ricciPairingRicciSecondDerivGroup g x v =
          ∑ a, ∑ b', ∑ c, ∑ d,
            (gramMatrix g x x)⁻¹ a c *
              ...
              (closedCovRicciDerivAt g x v (b c) (b d)
                + g.ricciAt x (g.leviCivita (extend E (b c)) x v) (b d)
                + g.ricciAt x (b c)
                    (g.leviCivita (extend E (b d)) x v)) := hsplit
      _ = ricciPairingRicciSecondCovariantGroup g x v
            + ricciPairingRicciSecondLeviCorrectionGroup g x v := by
          unfold ricciPairingRicciSecondCovariantGroup
          unfold ricciPairingRicciSecondLeviCorrectionGroup
          unfold ricciPairingRicciSecondLeftLeviCorrectionGroup
          unfold ricciPairingRicciSecondRightLeviCorrectionGroup
          simp [b, Finset.sum_add_distrib, mul_add, add_mul, add_assoc]

set_option maxHeartbeats 5000000 in
/--
The inverse-Gram derivative groups cancel the four Levi-Civita slot-correction
groups produced by differentiating the two Ricci entries.
-/
theorem pairing_gram_inv_deriv_groups_cancel_ricci_leviCivita_corrections
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M) (v : TM x) :
    ricciPairingGramInvFirstDerivGroup g x v
      + ricciPairingGramInvSecondDerivGroup g x v
      + ricciPairingRicciFirstLeviCorrectionGroup g x v
      + ricciPairingRicciSecondLeviCorrectionGroup g x v = 0 := by
  classical
  letI : FiniteDimensional ℝ (TM x) :=
    inferInstanceAs (FiniteDimensional ℝ E)
  let b := Module.finBasis ℝ (TM x)
  ...
  set Γlin : TM x →ₗ[ℝ] TM x :=
    IsLinearMap.mk' Γ
      ⟨(by
          intro p q
          change g.leviCivita (extend E (p + q)) x v =
            g.leviCivita (extend E p) x v + g.leviCivita (extend E q) x v
          rw [extend_tangent_add (x := x) p q]
          have hadd := g.leviCivita.isCovariantDerivativeOnUniv.add
            (by simpa [MDiffAtTangentField] using
              (mdifferentiableAt_extend I E p))
        ...
        (by
          intro c p
          change g.leviCivita (extend E (c • p)) x v =
            c • g.leviCivita (extend E p) x v
          rw [extend_tangent_smul (x := x) c p]
          have hsmul := g.leviCivita.isCovariantDerivativeOnUniv.smul_const c
            (by simpa [MDiffAtTangentField] using
              (mdifferentiableAt_extend I E p))
  ...
  have hFirstLeft :
      (∑ i, H₁ x (Γ (b i)) (sharp i)) =
        ricciPairingRicciFirstLeftLeviCorrectionGroup g x v := by
    calc
      (∑ i, H₁ x (Γ (b i)) (sharp i)) =
          ∑ i, ∑ c, G⁻¹ i c * H₁ x (Γ (b i)) (b c) := by
            ...
            exact tensor2_sum_right
              (h := H₁)
              (tensor2AddRight_ricciPairingInvFirstContractionTensor
                (g := g) x)
              (tensor2SMulRight_ricciPairingInvFirstContractionTensor
                (g := g) x)
              x (Γ (b i)) (fun c ↦ G⁻¹ i c) b
      _ = ricciPairingRicciFirstLeftLeviCorrectionGroup g x v := by
            unfold H₁
            unfold ricciPairingInvFirstContractionTensor
            unfold ricciPairingRicciFirstLeftLeviCorrectionGroup
  ...
  have hFirstRight :
      (∑ i, H₁ x (b i) (Γ (sharp i))) =
        ricciPairingRicciSecondLeftLeviCorrectionGroup g x v := by
    calc
      (∑ i, H₁ x (b i) (Γ (sharp i))) =
      ...
      _ = ricciPairingRicciSecondLeftLeviCorrectionGroup g x v := by
            unfold H₁
            unfold ricciPairingInvFirstContractionTensor
            unfold ricciPairingRicciSecondLeftLeviCorrectionGroup
            simp only [extend_apply_self]
  ...
  have hSecondLeft :
      (∑ i, H₂ x (Γ (b i)) (sharp i)) =
        ricciPairingRicciFirstRightLeviCorrectionGroup g x v := by
    calc
      (∑ i, H₂ x (Γ (b i)) (sharp i)) =
          ∑ i, ∑ d, G⁻¹ i d * H₂ x (Γ (b i)) (b d) := by
            ...
            exact tensor2_sum_right
              (h := H₂)
              (tensor2AddRight_ricciPairingInvSecondContractionTensor
                (g := g) x)
              (tensor2SMulRight_ricciPairingInvSecondContractionTensor
                (g := g) x)
              x (Γ (b i)) (fun d ↦ G⁻¹ i d) b
      _ = ricciPairingRicciFirstRightLeviCorrectionGroup g x v := by
            unfold H₂
            unfold ricciPairingInvSecondContractionTensor
            unfold ricciPairingRicciFirstRightLeviCorrectionGroup
            simp only [extend_apply_self]
  ...
  have hSecondRight :
      (∑ i, H₂ x (b i) (Γ (sharp i))) =
        ricciPairingRicciSecondRightLeviCorrectionGroup g x v := by
    calc
      (∑ i, H₂ x (b i) (Γ (sharp i))) =
      ...
      _ = ricciPairingRicciSecondRightLeviCorrectionGroup g x v := by
            unfold H₂
            unfold ricciPairingInvSecondContractionTensor
            unfold ricciPairingRicciSecondRightLeviCorrectionGroup
            simp only [extend_apply_self]
  ...
  have hInvSecond :
      ricciPairingInvSecondLeviCorrectionGroup g x v =
        ricciPairingRicciFirstRightLeviCorrectionGroup g x v
          + ricciPairingRicciSecondRightLeviCorrectionGroup g x v := by
    unfold ricciPairingInvSecondLeviCorrectionGroup
    simp [H₂, b, sharp, Γ, hSecondLeft, hSecondRight]
  have hInv :=
    pairing_gram_inv_deriv_groups_eq_leviCivita_corrections
      (g := g) x v
  have hFirst :
      ricciPairingGramInvFirstDerivGroup g x v =
        -(ricciPairingRicciFirstLeftLeviCorrectionGroup g x v
...
set_option maxHeartbeats 5000000 in
/--
Ricci-field specialization of the divergence-slot trace identity: the fixed
basis trace of the closed second covariant derivative is the covariant
derivative of the Ricci divergence one-form.
-/
theorem covTensor2SecondDerivAt_ricciVariationField_divergence_trace_eq
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    {x : M}
    (hRicSecond :
    ...
    (letI : FiniteDimensional ℝ (TM x) :=
      ...
      =
      (extDerivFun
          (fun y : M ↦ tensorDivergenceOneFormAt g (ricciVariationField g) y
            (extend E w y))
          x u
  ...
  let K : ∀ y : M, TM y → TM y → ℝ :=
    fun y p q ↦ covTensor2DerivAt g H y p q (extend E w y)
  have hHAddL : Tensor2AddLeft H := tensor2AddLeft_ricciVariationField g
  have hHSMulL : Tensor2SMulLeft H := tensor2SMulLeft_ricciVariationField g
  have hHAddR : Tensor2AddRight H := tensor2AddRight_ricciVariationField g
  have hHSMulR : Tensor2SMulRight H := tensor2SMulRight_ricciVariationField g
  have hHDiff : ∀ y : M, CovTensor2ExtDifferentiableAt H y :=
    fun y ↦ covTensor2ExtDifferentiableAt_ricciVariationField_canonical
      (g := g) (x := y)
  have hKDiff : CovTensor2ExtDifferentiableAt K x := by
    intro p q
    simpa [K, H] using hRicSecond p q w
  have hKAddL : Tensor2AddLeft K := by
    intro y p₁ p₂ q
    dsimp [K]
    exact covTensor2DerivAt_add_deriv
  ...
  have hKAddR : Tensor2AddRight K := by
    ...
    dsimp [K]
    exact covTensor2DerivAt_add_left
      (g := g) (h := H) (x := y) (hHDiff y) hHAddL
      p q₁ q₂ (extend E w y)
  have hKSMulR : Tensor2SMulRight K := by
    intro y c p q
    dsimp [K]
    exact covTensor2DerivAt_smul_left
      (g := g) (h := H) (x := y) (hHDiff y) hHSMulL
      c p q (extend E w y)
  let BK : ∀ y : M, LinearMap.BilinForm ℝ (TM y) :=
    fun y ↦ LinearMap.mk₂ ℝ (K y)
      (fun p p' q ↦ hKAddL y p p' q)
  ...
  have hTraceField :
    ...
    funext y
    have hswap : CovTensor2DerivTraceSwapAt g H y :=
      covTensor2DerivTraceSwapAt_of_regular g H y
        (hHDiff y) hHAddL hHSMulL hHAddR hHSMulR
    simpa [TraceMetricVariationDerivAt, traceMetricVariationAt,
      tensorDivergenceOneFormAt, K, H] using hswap (extend E w y)
  have hTraceK' :
      (∑ i, covTensor2DerivAt g K x u (b i) (sharp i)) =
        extDerivFun
          (fun y : M ↦ tensorDivergenceOneFormAt g H y (extend E w y))
  ...
  have hGammaTrace :
      (∑ i, covTensor2DerivAt g H x (b i) (sharp i) Γw) =
        tensorDivergenceOneFormAt g H x Γw := by
    have hswap : CovTensor2DerivTraceSwapAt g H x :=
      covTensor2DerivTraceSwapAt_of_regular g H x
        (hHDiff x) hHAddL hHSMulL hHAddR hHSMulR
    simpa [CovTensor2DerivTraceSwapAt, tensorDivergenceOneFormAt,
      H, b, sharp] using hswap Γw
  have hmain :
      (∑ i, covTensor2SecondDerivAt g H x u (b i) (sharp i) w)
          + tensorDivergenceOneFormAt g H x Γw =
        extDerivFun
...
theorem eventually_closedContractedBianchiOneFormAt_canonical
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M) :
    ∀ᶠ y in nhds x, ClosedContractedBianchiOneFormAt g y := by
  filter_upwards
...
The input is a neighborhood identity `div Ric = 1/2 dR`.  Differentiating in
canonical-extension slots and subtracting the covariant one-form correction
leaves exactly one half of the closed Hessian of scalar curvature.
-/
theorem ClosedContractedBianchiOneFormAt.differentiated_of_near
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M)
    (hDiv :
    ...
    extDerivFun
      ...
      =
        (1 / 2 : ℝ) *
          g.hessianAt (fun y : M ↦ g.scalarAt y) x u w := by
  let f : M → ℝ := fun y ↦ g.scalarAt y
  let F : M → ℝ := fun y ↦ extDerivFun f y (extend E w y)
  let Γw : TM x := g.leviCivita (extend E w) x u
  have hgrad :
      MDifferentiableAt I ((I).prod 𝓘(ℝ, E)) (T% (g.gradient f)) x := by
    simpa [f] using g.mdifferentiableAt_gradient hScalar₂
  ...
  have hDerivCongr :
      extDerivFun
          (fun y : M ↦
            tensorDivergenceOneFormAt g (ricciVariationField g) y
              (extend E w y)) x u =
        extDerivFun (fun y : M ↦ (1 / 2 : ℝ) * F y) x u := by
    exact congrArg (fun L : TM x →L[ℝ] ℝ ↦ L u)
      (CovariantDerivative.extDerivFun_congr hDivField)
  have hDerivScale :
      extDerivFun (fun y : M ↦ (1 / 2 : ℝ) * F y) x u =
        (1 / 2 : ℝ) * extDerivFun F x u := by
    have h :=
      congrArg (fun L : TM x →L[ℝ] ℝ ↦ L u)
        (extDerivFun_const_smul_at
          (n := n) (M := M) (f := F) (x := x)
          (by simpa [F, f] using hScalarExt₂ w) (1 / 2 : ℝ))
    simpa [Pi.smul_apply, smul_eq_mul, F] using h
  have hCompat :
      extDerivFun F x u =
        g.hessianAt f x u w + extDerivFun f x Γw := by
    simpa [F, f, Γw] using
      extDerivFun_extDerivFun_extend_eq_hessianAt_add
        (g := g) (f := f) (x := x) hgrad u w
  have hCorr :
      tensorDivergenceOneFormAt g (ricciVariationField g) x Γw =
        (1 / 2 : ℝ) * extDerivFun f x Γw := by
    simpa [f, Γw] using hDiv.self_of_nhds Γw
  rw [hDerivCongr, hDerivScale, hCompat, hCorr]
  ring

theorem closedContractedBianchiOneFormAt_differentiated_canonical
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M)
    (hScalar₂ : ContMDiffAt I 𝓘(ℝ) 2 (fun y : M ↦ g.scalarAt y) x)
    ...
    extDerivFun
      ...
      =
        (1 / 2 : ℝ) *
          g.hessianAt (fun y : M ↦ g.scalarAt y) x u w :=
  ClosedContractedBianchiOneFormAt.differentiated_of_near
    (g := g) (x := x)
    ((eventually_closedContractedBianchiOneFormAt_canonical
      (g := g) (x := x)).mono fun _ hy w ↦ hy w)
    hScalar₂ hScalarExt₂ u w

theorem covTensor2SecondDerivAt_ricciVariationField_divergence_trace_eq_half_hessian
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    {x : M}
    (hRicSecond :
    ...
    (letI : FiniteDimensional ℝ (TM x) :=
      ...
      =
        (1 / 2 : ℝ) *
          g.hessianAt (fun y : M ↦ g.scalarAt y) x u w := by
  rw [covTensor2SecondDerivAt_ricciVariationField_divergence_trace_eq
    (g := g) (x := x) hRicSecond u w]
  exact closedContractedBianchiOneFormAt_differentiated_canonical
    (g := g) (x := x) hScalar₂ hScalarExt₂ u w

theorem covTensor2SecondDerivAt_ricciVariationField_Hslot_trace_eq_hessianAt_scalar
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    {x : M}
    (hRicSecond :
  ...
  let H : ∀ y : M, TM y → TM y → ℝ := ricciVariationField g
  let f : M → ℝ := fun y ↦ traceMetricVariationAt g H y
  have hf₂ : ContMDiffAt I 𝓘(ℝ) 2 f x := by
    have hfun : f = fun y : M ↦ g.scalarAt y := by
      funext y
  ...
  have hgradf :
      MDifferentiableAt I ((I).prod 𝓘(ℝ, E)) (T% (g.gradient f)) x := by
    exact g.mdifferentiableAt_gradient hf₂
  have hgradScalar :
      MDifferentiableAt I ((I).prod 𝓘(ℝ, E))
        (T% (g.gradient (fun y : M ↦ g.scalarAt y))) x := by
    exact g.mdifferentiableAt_gradient hScalar₂
  have htrace :
      (letI : FiniteDimensional ℝ (TM x) :=
          inferInstanceAs (FiniteDimensional ℝ E)
        let b := Module.finBasis ℝ (TM x)
    ...
    simpa [H, f] using
      covTensor2SecondDerivAt_Hslot_trace_eq_hessianAt
        ...
        (by intro y p q; rfl)
        hgradf u w
  have hEq : f =ᶠ[nhds x] fun y : M ↦ g.scalarAt y :=
    Filter.Eventually.of_forall fun y ↦ by
      simpa [f, H] using traceMetricVariationAt_ricci (g := g) y
  have hhess :
      g.hessianAt f x u w =
        g.hessianAt (fun y : M ↦ g.scalarAt y) x u w :=
    g.hessianAt_congr_of_eventuallyEq hEq hgradf hgradScalar u w
  rw [htrace, hhess]
...
theorem covTensor2DerivAt_moving_extDerivFun_expansion
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    {h : ∀ y : M, TM y → TM y → ℝ} {x : M}
    ...
    extDerivFun
      ...
      extDerivFun
        ...
        - extDerivFun
            (fun y : M ↦
              h y (extend E p y)
                (g.leviCivita (extend E q) y (extend E v y))) x u := by
  let A : M → ℝ := fun y : M ↦
    extDerivFun
      (fun z : M ↦ h z (extend E p z) (extend E q z)) y
      (extend E v y)
  ...
  have hevent :
      (fun y : M ↦ covTensor2DerivAt g h y
        ...
        fun y : M ↦ A y - B y - C y := by
    have hp_event := eventually_contMDiffAt_two_extend (n := n) (M := M) p
    have hq_event := eventually_contMDiffAt_two_extend (n := n) (M := M) q
  ...
  have hnegB :
      extDerivFun (fun y : M ↦ -B y) x u =
        -extDerivFun B x u := by
    have hnegFun : (fun y : M ↦ -B y) = (-1 : ℝ) • B := by
      funext y
      simp
    rw [hnegFun]
    have h := congrArg (fun L : TM x →L[ℝ] ℝ ↦ L u)
      (extDerivFun_const_smul_at (n := n) (M := M) hB (-1 : ℝ))
    simpa [Pi.smul_apply, smul_eq_mul] using h
  have hsplit :
      extDerivFun (fun y : M ↦ A y - B y - C y) x u =
        extDerivFun A x u - extDerivFun B x u - extDerivFun C x u := by
    have hsplit' :=
      extDerivFun_add_sub_at
        (n := n) (M := M)
        (f := A) (g := fun y : M ↦ -B y) (h := C)
        hA hB.neg hC u
    rw [hnegB] at hsplit'
    ...
    extDerivFun
        ...
        extDerivFun (fun y : M ↦ A y - B y - C y) x u := by
          exact congrArg (fun L : TM x →L[ℝ] ℝ ↦ L u)
            (CovariantDerivative.extDerivFun_congr hevent)
    _ =
        extDerivFun A x u - extDerivFun B x u - extDerivFun C x u := hsplit
    _ =
      extDerivFun
          (fun y : M ↦
            extDerivFun
...
/--
Closed tensor Ricci identity for a raw bilinear `(0,2)` tensor field.

The `hDiff` hypothesis supplies the pointwise extension-independence bridge for
the moving-anchor first covariant derivative; `hC2` supplies the fixed scalar
Schwarz cancellation at `x`.
-/
theorem covTensor2SecondDerivAt_antisymm_eq_curvatureAction
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    {h : ∀ y : M, TM y → TM y → ℝ} {x : M}
    (hC2 : CovTensor2ExtContMDiffAt h x 2)
  ...
  have hraw :=
    covTensor2SecondDerivAt_antisymm_expansion
      (g := g) (h := h) (x := x) u v p q
  have hbridgeV :=
    covTensor2DerivAt_moving_extDerivFun_expansion
      (g := g) (h := h) (x := x)
      (covTensor2ExtSecondDifferentiableAt_of_contMDiffAt_two hC2)
      hDiff hAddL hSMulL hAddR hSMulR u v p q
  have hbridgeU :=
    covTensor2DerivAt_moving_extDerivFun_expansion
      (g := g) (h := h) (x := x)
      (covTensor2ExtSecondDifferentiableAt_of_contMDiffAt_two hC2)
      hDiff hAddL hSMulL hAddR hSMulR v u p q
  have hschwarz :
      Av - Au = 0 := by
    simpa [Av, Au] using
      covTensor2SecondDerivAt_pure_schwarz_cancel
        (g := g) (h := h) (x := x) hC2 u v p q
  have hconn :
      - (Bv - Bu) - (Cv - Cu) - D - F =
        covTensor2SecondDerivCurvatureActionAt g h x u v p q := by
    simpa [Bv, Bu, Cv, Cu, D, F] using
      tensor2_connection_slot_antisymm_corrections_eq_curvatureAction
        (g := g) (h := h) (x := x)
        (hDiff x) hAddL hSMulL hAddR hSMulR u v p q
  calc
    covTensor2SecondDerivAt g h x u v p q
        - covTensor2SecondDerivAt g h x v u p q =
        (extDerivFun
          ...
          - extDerivFun
            (fun y : M ↦ covTensor2DerivAt g h y
              (extend E u y) (extend E p y) (extend E q y)) x v)
          - D - F := by
          simpa [D, F] using hraw
    _ = ((Av - Bv - Cv) - (Au - Bu - Cu)) - D - F := by
          rw [hbridgeV, hbridgeU]
    _ = (Av - Au) + (-(Bv - Bu) - (Cv - Cu) - D - F) := by
          ring
    _ = covTensor2SecondDerivCurvatureActionAt g h x u v p q := by
          rw [hschwarz, hconn]
          ring
...
theorem closedCurvatureOp_first_bianchiAt
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M) (u v w : TM x) :
    CovariantDerivative.curvatureOp g.leviCivita
      ...
      + CovariantDerivative.curvatureOp g.leviCivita
        (extend E v) (extend E w) (extend E u) x
      + CovariantDerivative.curvatureOp g.leviCivita
        (extend E w) (extend E u) (extend E v) x = 0 := by
  simpa using
    CovariantDerivative.bianchi_first_at
      (cov := g.leviCivita)
      (fun y ↦ g.leviCivita_torsionFreeAt y)
      (X := extend E u) (Y := extend E v) (Z := extend E w) (x := x)
      (FiberBundle.contMDiffAt_extend' (k := 2) I E u)
      (FiberBundle.contMDiffAt_extend' (k := 2) I E v)
      (FiberBundle.contMDiffAt_extend' (k := 2) I E w)
...
closed-manifold version of `tr(Ric# ∘ R(u,w)) = 0`, using trace cyclicity,
Ricci self-adjointness, and curvature skew-adjointness.
-/
theorem ricciEndo_curvatureTraceAt_eq_zero
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M) (u w : TM x) :
    (letI : FiniteDimensional ℝ (TM x) :=
      ...
      ∑ i, g.ricciAt x
        (CovariantDerivative.curvatureOp g.leviCivita
          (extend E u) (extend E w) (extend E (b i)) x)
        (sharp i)) = 0 := by
  classical
  letI : FiniteDimensional ℝ (TM x) :=
    inferInstanceAs (FiniteDimensional ℝ E)
  ...
  let sharp : Fin (Module.finrank ℝ (TM x)) → TM x :=
    fun i ↦ metricDualVectorAt g x (b.coord i)
  let Rlin : TM x →ₗ[ℝ] TM x :=
    CovariantDerivative.pairCurvatureEndAt g.leviCivita x u w
  let A : TM x →ₗ[ℝ] TM x := g.ricciEndoAt x
  have hTraceAR :
      (∑ i, g.inner x (A (Rlin (b i))) (sharp i)) =
        LinearMap.trace ℝ (TM x) (A.comp Rlin) := by
    rw [LinearMap.trace_eq_matrix_trace ℝ b, Matrix.trace]
    refine Finset.sum_congr rfl fun i _ ↦ ?_
    rw [Matrix.diag_apply, LinearMap.toMatrix_apply]
    exact (coord_eq_inner_metricDualVectorAt
      (g := g) (x := x) i ((A.comp Rlin) (b i))).symm
  have hTraceRA :
      (∑ i, g.inner x (Rlin (A (b i))) (sharp i)) =
        LinearMap.trace ℝ (TM x) (Rlin.comp A) := by
    rw [LinearMap.trace_eq_matrix_trace ℝ b, Matrix.trace]
    refine Finset.sum_congr rfl fun i _ ↦ ?_
    rw [Matrix.diag_apply, LinearMap.toMatrix_apply]
    exact (coord_eq_inner_metricDualVectorAt
      (g := g) (x := x) i ((Rlin.comp A) (b i))).symm
  have hRicTrace :
      (∑ i, g.ricciAt x (Rlin (b i)) (sharp i)) =
        LinearMap.trace ℝ (TM x) (A.comp Rlin) := by
    calc
      (∑ i, g.ricciAt x (Rlin (b i)) (sharp i)) =
          ∑ i, g.inner x (A (Rlin (b i))) (sharp i) := by
            refine Finset.sum_congr rfl fun i _ ↦ ?_
            rw [ClosedSmoothRiemannianMetric.inner_ricciEndoAt]
      _ = LinearMap.trace ℝ (TM x) (A.comp Rlin) := hTraceAR
  let F : TM x → TM x → ℝ :=
    fun p q ↦ g.inner x (A (Rlin p)) q
  have hswap := sum_metricDualVectorAt_contraction_swap
    (g := g) (x := x) (F := F)
    (fun p₁ p₂ q ↦ by
      dsimp [F]
    ...
    (fun c p q ↦ by
      ...
      simp [smul_eq_mul])
  have hRAneg :
      LinearMap.trace ℝ (TM x) (Rlin.comp A) =
        -LinearMap.trace ℝ (TM x) (A.comp Rlin) := by
    rw [← hTraceRA, ← hTraceAR]
    calc
      (∑ i, g.inner x (Rlin (A (b i))) (sharp i))
          =
        ∑ i, -g.inner x (A (b i)) (Rlin (sharp i)) := by
          refine Finset.sum_congr rfl fun i _ ↦ ?_
          have hskew := closedCurvaturePairLastPairAntisymmAt
            (g := g) (x := x) u w (A (b i)) (sharp i)
          calc
            g.inner x (Rlin (A (b i))) (sharp i) =
                -g.inner x (Rlin (sharp i)) (A (b i)) := by
                  simpa [Rlin] using hskew
            _ = -g.inner x (A (b i)) (Rlin (sharp i)) := by
                  rw [g.inner_symm x (Rlin (sharp i)) (A (b i))]
      _ =
        -∑ i, g.inner x (A (b i)) (Rlin (sharp i)) := by
          rw [Finset.sum_neg_distrib]
      _ =
        -∑ i, g.inner x (b i) (A (Rlin (sharp i))) := by
          congr 1
          refine Finset.sum_congr rfl fun i _ ↦ ?_
          exact g.ricciEndoAt_selfAdjoint x (b i) (Rlin (sharp i))
      _ =
        -∑ i, g.inner x (A (Rlin (sharp i))) (b i) := by
          congr 1
          refine Finset.sum_congr rfl fun i _ ↦ ?_
          rw [g.inner_symm x (b i) (A (Rlin (sharp i)))]
      _ =
        -∑ i, g.inner x (A (Rlin (b i))) (sharp i) := by
          simpa [F, b, sharp] using congrArg Neg.neg hswap
  have hcyc :
      LinearMap.trace ℝ (TM x) (A.comp Rlin) =
        LinearMap.trace ℝ (TM x) (Rlin.comp A) :=
    (LinearMap.trace_comp_comm' A Rlin).symm
  have hzero :
      LinearMap.trace ℝ (TM x) (A.comp Rlin) = 0 := by
    linarith
  change
    (∑ i, g.ricciAt x
      (CovariantDerivative.curvatureOp g.leviCivita
        (extend E u) (extend E w) (extend E (b i)) x)
      (sharp i)) = 0
  simpa [Rlin] using hRicTrace.trans hzero

/-- The corrected mixed Lichnerowicz curvature contraction is symmetric on Ricci. -/
theorem lichnerowiczCurvatureAt_ricciVariationField_symm
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M) (u w : TM x) :
    lichnerowiczCurvatureAt g (ricciVariationField g) x u w =
      lichnerowiczCurvatureAt g (ricciVariationField g) x w u := by
  classical
  letI : FiniteDimensional ℝ (TM x) :=
    inferInstanceAs (FiniteDimensional ℝ E)
  let b := Module.finBasis ℝ (TM x)
  ...
  have hzero :
      (∑ i, g.ricciAt x
        ...
        (sharp i)) = 0 := by
    simpa [b, sharp] using
      ricciEndo_curvatureTraceAt_eq_zero (g := g) (x := x) u w
  have hentry :
      ∀ i : Fin (Module.finrank ℝ (TM x)),
        g.ricciAt x
          (CovariantDerivative.curvatureOp g.leviCivita
        ...
        =
          ...
          + g.ricciAt x
            (CovariantDerivative.curvatureOp g.leviCivita
              (extend E (b i)) (extend E w) (extend E u) x)
            (sharp i) := by
    intro i
    have hb := closedCurvatureOp_first_bianchiAt
      (g := g) (x := x) (b i) u w
    have hwu :
        CovariantDerivative.curvatureOp g.leviCivita
            (extend E w) (extend E (b i)) (extend E u) x =
          -CovariantDerivative.curvatureOp g.leviCivita
            (extend E (b i)) (extend E w) (extend E u) x := by
      rw [CovariantDerivative.curvatureOp_antisymm_apply]
    have hvec :
        CovariantDerivative.curvatureOp g.leviCivita
            (extend E (b i)) (extend E u) (extend E w) x =
    ...
    let Ruw : TM x := CovariantDerivative.curvatureOp g.leviCivita
      (extend E u) (extend E w) (extend E (b i)) x
    let Rbw : TM x := CovariantDerivative.curvatureOp g.leviCivita
      (extend E (b i)) (extend E w) (extend E u) x
    rw [hvec]
    change g.ricciAt x (-Ruw + Rbw) (sharp i) =
      -g.ricciAt x Ruw (sharp i) + g.ricciAt x Rbw (sharp i)
    calc
      g.ricciAt x (-Ruw + Rbw) (sharp i) =
          g.ricciAt x (-Ruw) (sharp i) + g.ricciAt x Rbw (sharp i) := by
            rw [g.ricciAt_add_left]
      _ = -g.ricciAt x Ruw (sharp i) + g.ricciAt x Rbw (sharp i) := by
            have hneg := g.ricciAt_smul_left x (-1 : ℝ) Ruw (sharp i)
            simpa using congrArg
              (fun r : ℝ ↦ r + g.ricciAt x Rbw (sharp i)) hneg
  change
    (∑ i, g.ricciAt x
      (CovariantDerivative.curvatureOp g.leviCivita
        (extend E (b i)) (extend E u) (extend E w) x)
...
theorem curvatureOp_trace_sharp_inner_eq_neg_ricciAt
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M) (u z : TM x) :
    (letI : FiniteDimensional ℝ (TM x) :=
      ...
      ∑ i, g.inner x
        (CovariantDerivative.curvatureOp g.leviCivita
          (extend E (b i)) (extend E u) (extend E (sharp i)) x)
        z)
      =
        -g.ricciAt x u z := by
  classical
  ...
  have hentry : ∀ i : Fin (Module.finrank ℝ (TM x)),
      ...
      =
        -g.inner x
          (CovariantDerivative.curvatureOp g.leviCivita
            (extend E (b i)) (extend E u) (extend E z) x)
          (sharp i) := by
    intro i
    exact closedCurvaturePairLastPairAntisymmAt
      (g := g) (x := x) (b i) u (sharp i) z
  have hric :
      g.ricciAt x u z =
        ∑ i, g.inner x
          (CovariantDerivative.curvatureOp g.leviCivita
          ...
          (sharp i) := by
    simpa [b, sharp] using
      ricciAt_eq_curvature_inner_contraction (g := g) (x := x) u z
  change
    (∑ i, g.inner x
      (CovariantDerivative.curvatureOp g.leviCivita
        (extend E (b i)) (extend E u) (extend E (sharp i)) x)
  ...
  calc
    ...
    _ =
      -∑ i, g.inner x
        ...
        (sharp i) := by
        rw [Finset.sum_neg_distrib]
    _ = -g.ricciAt x u z := by
        rw [← hric]

theorem covTensor2SecondDerivCurvatureActionAt_ricciVariationField_trace_eq_neg_lichnerowicz_add_ricciEndo
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M) (u w : TM x) :
    (letI : FiniteDimensional ℝ (TM x) :=
      ...
      =
        -lichnerowiczCurvatureAt g (ricciVariationField g) x u w
          + g.ricciAt x (g.ricciEndoAt x u) w := by
  classical
  letI : FiniteDimensional ℝ (TM x) :=
    inferInstanceAs (FiniteDimensional ℝ E)
  let b := Module.finBasis ℝ (TM x)
  ...
  have hfirst :
      (∑ i, -g.ricciAt x
        ...
        (sharp i))
      =
        -lichnerowiczCurvatureAt g (ricciVariationField g) x u w := by
    unfold lichnerowiczCurvatureAt ricciVariationField
    rw [Finset.sum_neg_distrib]
  have hRicEndo :
      g.ricciAt x u (A w) = g.ricciAt x (A u) w := by
        ...
        ∑ i, g.inner x
          ...
          calc
            g.ricciAt x w
              ...
              g.inner x (A w)
                (CovariantDerivative.curvatureOp g.leviCivita
                  (extend E (b i)) (extend E u) (extend E (sharp i)) x) := by
                rw [ClosedSmoothRiemannianMetric.inner_ricciEndoAt]
            _ =
              g.inner x
                (CovariantDerivative.curvatureOp g.leviCivita
                  (extend E (b i)) (extend E u) (extend E (sharp i)) x)
                (A w) := by
                rw [g.inner_symm x (A w)]
      _ = -g.ricciAt x u (A w) := by
          simpa [b, sharp, A] using
            curvatureOp_trace_sharp_inner_eq_neg_ricciAt
              (g := g) (x := x) (u := u) (z := A w)
      _ = -g.ricciAt x (A u) w := by
          rw [hRicEndo]
  have hsecond :
      ...
      =
        g.ricciAt x (A u) w := by
    rw [Finset.sum_neg_distrib, hsecondRaw]
    ring
  change
    (∑ i,
      (-g.ricciAt x
      ...
      =
        -lichnerowiczCurvatureAt g (ricciVariationField g) x u w
          + g.ricciAt x (A u) w
  rw [Finset.sum_sub_distrib, hfirst, hsecondRaw]
  ring

theorem covTensor2SecondDerivCurvatureActionAt_ricciVariationField_trace_eq_action_sub_quadratic
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M) (u w : TM x) :
    (letI : FiniteDimensional ℝ (TM x) :=
  ...
  have hU :
      ...
      =
        -lichnerowiczCurvatureAt g (ricciVariationField g) x u w
          + g.ricciAt x (g.ricciEndoAt x u) w := by
    simpa [b, sharp] using
      covTensor2SecondDerivCurvatureActionAt_ricciVariationField_trace_eq_neg_lichnerowicz_add_ricciEndo
        (g := g) (x := x) u w
  have hW :
      (∑ i, covTensor2SecondDerivCurvatureActionAt
        g (ricciVariationField g) x (b i) w u (sharp i))
      =
        -lichnerowiczCurvatureAt g (ricciVariationField g) x w u
          + g.ricciAt x (g.ricciEndoAt x w) u := by
    simpa [b, sharp] using
      covTensor2SecondDerivCurvatureActionAt_ricciVariationField_trace_eq_neg_lichnerowicz_add_ricciEndo
        (g := g) (x := x) w u
  have hLsym :
      lichnerowiczCurvatureAt g (ricciVariationField g) x w u =
        lichnerowiczCurvatureAt g (ricciVariationField g) x u w :=
    (lichnerowiczCurvatureAt_ricciVariationField_symm
      (g := g) (x := x) u w).symm
  have hRicSymm :
      g.ricciAt x (g.ricciEndoAt x w) u =
        g.ricciAt x u (g.ricciEndoAt x w) := by
    rw [g.ricciAt_symm]
  have hQ :
      ricciQuadraticAt g x u w =
        2 * lichnerowiczCurvatureAt g (ricciVariationField g) x u w :=
  ...
  change
    ...
    + (∑ i, covTensor2SecondDerivCurvatureActionAt
      ...
      =
        ricciActionOnTensorAt g (ricciVariationField g) x u w
          - ricciQuadraticAt g x u w
  rw [hU, hW, hLsym, hRicSymm, hQ]
  unfold ricciActionOnTensorAt ricciVariationField
  ring

theorem covTensor2SecondDerivAt_ricciVariationField_antisymm_eq_curvatureAction
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M)
    (hRicC2 : CovTensor2ExtContMDiffAt (ricciVariationField g) x 2)
    (u v p q : TM x) :
    covTensor2SecondDerivAt g (ricciVariationField g) x u v p q
      ...
      covTensor2SecondDerivCurvatureActionAt
        g (ricciVariationField g) x u v p q :=
  covTensor2SecondDerivAt_antisymm_eq_curvatureAction
    (g := g) (h := ricciVariationField g) (x := x)
    hRicC2
...
theorem covTensor2SecondDerivAt_ricciVariationField_trace_swap_inner_left
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M)
    (hRicSecond :
  ...
  let sharp : Fin (Module.finrank ℝ (TM x)) → TM x :=
    fun i ↦ metricDualVectorAt g x (b.coord i)
  let F : TM x → TM x → ℝ :=
    fun p q ↦ covTensor2SecondDerivAt g (ricciVariationField g) x u p q w
  have hswap := sum_metricDualVectorAt_contraction_swap
    (g := g) (x := x) (F := F)
    (hadd1 := by
      intro p₁ p₂ q
    ...
    (hsmul2 := by
      ...
      exact covTensor2SecondDerivAt_smul_left
        ...
        (tensor2SMulLeft_ricciVariationField g)
        c u p q w)
  change (∑ i, F (sharp i) (b i)) =
    ∑ i, covTensor2SecondDerivAt g (ricciVariationField g) x
      u (b i) w (sharp i)
  rw [hswap]
  refine Finset.sum_congr rfl fun i _ ↦ ?_
  dsimp [F]
  exact covTensor2SecondDerivAt_ricciVariationField_symm
    (g := g) (x := x) u (b i) (sharp i) w

theorem deltaRicciSecondDerivContractionAt_ricciVariationField_trace_cancel
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M)
    (hRicSecond :
      CovTensor2DerivExtDifferentiableAt g (ricciVariationField g) x)
    (u w : TM x) :
    deltaRicciSecondDerivContractionAt g (ricciVariationField g) x u w =
      (letI : FiniteDimensional ℝ (TM x) :=
          inferInstanceAs (FiniteDimensional ℝ E)
        let b := Module.finBasis ℝ (TM x)
        let sharp : Fin (Module.finrank ℝ (TM x)) → TM x :=
...
set_option maxHeartbeats 5000000 in
/--
After the differentiated contracted-Bianchi bridge, the `-2 Ric` second
derivative contraction reduces to the rough trace minus the two curvature
actions coming from commuting the free derivative slots.
-/
theorem deltaRicciSecondDerivContractionAt_negTwoRicci_eq_rough_sub_curvatureActions
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M)
    (hRicSecond :
    ...
    deltaRicciSecondDerivContractionAt g (negTwoRicciVariationField g) x u w =
      roughTensorLaplacianAt g (ricciVariationField g) x u w
        ...
        let sharp : Fin (Module.finrank ℝ (TM x)) → TM x :=
          fun i ↦ metricDualVectorAt g x (b.coord i)
        ∑ i, covTensor2SecondDerivCurvatureActionAt
          g (ricciVariationField g) x (b i) w u (sharp i)) := by
  classical
  letI : FiniteDimensional ℝ (TM x) :=
    inferInstanceAs (FiniteDimensional ℝ E)
  let H : ∀ y : M, TM y → TM y → ℝ := ricciVariationField g
  ...
  let sharp : Fin (Module.finrank ℝ (TM x)) → TM x :=
    fun i ↦ metricDualVectorAt g x (b.coord i)
  let A : ℝ :=
    ∑ i, covTensor2SecondDerivAt g H x (b i) u w (sharp i)
  let B : ℝ :=
    ∑ i, covTensor2SecondDerivAt g H x (b i) w u (sharp i)
  let C : ℝ :=
    ∑ i, covTensor2SecondDerivAt g H x (b i) (sharp i) u w
  let T : ℝ :=
    ∑ i, covTensor2SecondDerivAt g H x u w (b i) (sharp i)
  let Du : ℝ :=
    ∑ i, covTensor2SecondDerivAt g H x u (b i) (sharp i) w
  let Dw : ℝ :=
    ∑ i, covTensor2SecondDerivAt g H x w (b i) (sharp i) u
  let CAu : ℝ :=
    ∑ i, covTensor2SecondDerivCurvatureActionAt g H x (b i) u w (sharp i)
  let CAw : ℝ :=
    ∑ i, covTensor2SecondDerivCurvatureActionAt g H x (b i) w u (sharp i)
  let Hu : ℝ := g.hessianAt (fun y : M ↦ g.scalarAt y) x u w
  let Hw : ℝ := g.hessianAt (fun y : M ↦ g.scalarAt y) x w u
  have hA : A = Du + CAu := by
    calc
      A =
          ∑ i,
            (covTensor2SecondDerivAt g H x u (b i) (sharp i) w
              + covTensor2SecondDerivCurvatureActionAt g H x
                (b i) u w (sharp i)) := by
            refine Finset.sum_congr rfl fun i _hi ↦ ?_
            have hcomm :=
              covTensor2SecondDerivAt_ricciVariationField_antisymm_eq_curvatureAction
                (g := g) (x := x) hRicC2 (b i) u w (sharp i)
            have hsymm :
                covTensor2SecondDerivAt g H x u (b i) w (sharp i) =
                  covTensor2SecondDerivAt g H x u (b i) (sharp i) w := by
              simpa [H] using
                covTensor2SecondDerivAt_ricciVariationField_symm
                  (g := g) (x := x) u (b i) w (sharp i)
            dsimp [H] at hcomm hsymm ⊢
            linarith
      _ = Du + CAu := by
            simp [Du, CAu, Finset.sum_add_distrib]
  have hB : B = Dw + CAw := by
    calc
      B =
          ∑ i,
            (covTensor2SecondDerivAt g H x w (b i) (sharp i) u
              + covTensor2SecondDerivCurvatureActionAt g H x
                (b i) w u (sharp i)) := by
            refine Finset.sum_congr rfl fun i _hi ↦ ?_
            have hcomm :=
              covTensor2SecondDerivAt_ricciVariationField_antisymm_eq_curvatureAction
                (g := g) (x := x) hRicC2 (b i) w u (sharp i)
            have hsymm :
                covTensor2SecondDerivAt g H x w (b i) u (sharp i) =
                  covTensor2SecondDerivAt g H x w (b i) (sharp i) u := by
              simpa [H] using
                covTensor2SecondDerivAt_ricciVariationField_symm
                  (g := g) (x := x) w (b i) u (sharp i)
            dsimp [H] at hcomm hsymm ⊢
            linarith
      _ = Dw + CAw := by
            simp [Dw, CAw, Finset.sum_add_distrib]
  have hDu : Du = (1 / 2 : ℝ) * Hu := by
    simpa [Du, H, Hu, b, sharp] using
      covTensor2SecondDerivAt_ricciVariationField_divergence_trace_eq_half_hessian
        (g := g) (x := x) hRicSecond hScalar₂ hScalarExt₂ u w
  have hDw : Dw = (1 / 2 : ℝ) * Hw := by
    simpa [Dw, H, Hw, b, sharp] using
      covTensor2SecondDerivAt_ricciVariationField_divergence_trace_eq_half_hessian
        (g := g) (x := x) hRicSecond hScalar₂ hScalarExt₂ w u
  have hT : T = Hu := by
    simpa [T, H, Hu, b, sharp] using
      covTensor2SecondDerivAt_ricciVariationField_Hslot_trace_eq_hessianAt_scalar
        (g := g) (x := x) hRicSecond hScalar₂ u w
  have hHsymm : Hw = Hu := by
    simpa [Hu, Hw] using
      g.hessianAt_symm' hScalar₂ w u
  have hNeg :=
    deltaRicciSecondDerivContractionAt_negTwoRicci_eq_neg_two_ricci
      (g := g) (x := x) hRicSecond u w
  have hTrace :=
    deltaRicciSecondDerivContractionAt_ricciVariationField_trace_cancel
      (g := g) (x := x) hRicSecond u w
  rw [hNeg, hTrace]
  unfold deltaRicciSecondDerivContractionAt at hTrace
  change
    -2 *
      ((∑ i, (1 / 2 : ℝ) *
        (covTensor2SecondDerivAt g H x (b i) u w (sharp i)
    ...
    calc
      (∑ i, (1 / 2 : ℝ) *
        (covTensor2SecondDerivAt g H x (b i) u w (sharp i)
          ...
          =
        ∑ i,
          ((1 / 2 : ℝ) * covTensor2SecondDerivAt g H x (b i) u w (sharp i)
            + (1 / 2 : ℝ) * covTensor2SecondDerivAt g H x (b i) w u (sharp i)
            - (1 / 2 : ℝ) * covTensor2SecondDerivAt g H x (b i) (sharp i) u w) := by
            refine Finset.sum_congr rfl fun i _hi ↦ ?_
            ring
      _ = (1 / 2 : ℝ) * A + (1 / 2 : ℝ) * B - (1 / 2 : ℝ) * C := by
            simp [A, B, C, Finset.sum_add_distrib, Finset.sum_sub_distrib,
              Finset.mul_sum]
  have hSecond :
      (∑ i, (1 / 2 : ℝ) *
        covTensor2SecondDerivAt g H x u w (b i) (sharp i))
  ...
  rw [hFirst, hSecond, hA, hB, hDu, hDw, hT, hHsymm]
  ring

set_option maxHeartbeats 5000000 in
/--
Corrected curvature-commutation assembly: the M4-prep-15 Hessian-cancelled
identity gives `rough - CAu - CAw`, and the corrected CA fold gives
`CAu + CAw = A - Q`.
-/
theorem RicciSecondDerivCurvatureCommutationAt.of_traceSecondRegularity
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M)
    (hRicSecond :
  ...
  have hAsm :
    ...
    simpa [b, sharp] using
      deltaRicciSecondDerivContractionAt_negTwoRicci_eq_rough_sub_curvatureActions
        (g := g) (x := x) hRicSecond hRicC2 hScalar₂ hScalarExt₂ u w
  have hCA :
      (∑ i, covTensor2SecondDerivCurvatureActionAt
        g (ricciVariationField g) x (b i) u w (sharp i))
        + (∑ i, covTensor2SecondDerivCurvatureActionAt
  ...
  calc
    deltaRicciSecondDerivContractionAt g (negTwoRicciVariationField g) x u w =
        roughTensorLaplacianAt g (ricciVariationField g) x u w
          ...
          -
          (∑ i, covTensor2SecondDerivCurvatureActionAt
            g (ricciVariationField g) x (b i) w u (sharp i)) := hAsm
    _ = roughTensorLaplacianAt g (ricciVariationField g) x u w
          -
          ((∑ i, covTensor2SecondDerivCurvatureActionAt
              g (ricciVariationField g) x (b i) u w (sharp i))
            + (∑ i, covTensor2SecondDerivCurvatureActionAt
    ...
    _ = roughTensorLaplacianAt g (ricciVariationField g) x u w
          ...
          + ricciQuadraticAt g x u w := by
          ring
    _ = ricciEvolutionTensorRHSAt g x u w := hRHS.symm
...
/--
The metric trace of the corrected Lichnerowicz curvature contraction on the
Ricci field is `|Ric|²`.
-/
theorem lichnerowiczCurvatureAt_ricciVariationField_trace_eq_ricciNormSqAt
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M) :
    (letI : FiniteDimensional ℝ (TM x) :=
      ...
      let sharp : Fin (Module.finrank ℝ (TM x)) → TM x :=
        fun j ↦ metricDualVectorAt g x (b.coord j);
      ∑ j, lichnerowiczCurvatureAt g (ricciVariationField g) x
        (b j) (sharp j))
      = g.ricciNormSqAt x := by
  classical
  letI : FiniteDimensional ℝ (TM x) :=
  ...
  have hentry :
      ∀ i j : Fin (Module.finrank ℝ (TM x)),
        g.ricciAt x
            (CovariantDerivative.curvatureOp g.leviCivita
              (extend E (b i)) (extend E (b j)) (extend E (sharp j)) x)
            (sharp i)
          =
        g.inner x
            (CovariantDerivative.curvatureOp g.leviCivita
              (extend E (sharp j)) (extend E (A (sharp i)))
              (extend E (b i)) x)
            (b j) := by
    intro i j
    calc
      ...
      _ =
          g.inner x
            (CovariantDerivative.curvatureOp g.leviCivita
              (extend E (b i)) (extend E (b j)) (extend E (sharp j)) x)
            (A (sharp i)) := by
            exact g.ricciEndoAt_selfAdjoint x
              (CovariantDerivative.curvatureOp g.leviCivita
                (extend E (b i)) (extend E (b j)) (extend E (sharp j)) x)
              (sharp i)
      _ =
          g.inner x
            (CovariantDerivative.curvatureOp g.leviCivita
              ...
              (extend E (b i)) x)
            (b j) := by
            exact closedCurvaturePairSymmAt
              (g := g) (x := x) (b i) (b j) (sharp j) (A (sharp i))
  have htoRic :
      ∀ i : Fin (Module.finrank ℝ (TM x)),
        (∑ j, g.inner x
            (CovariantDerivative.curvatureOp g.leviCivita
              ...
              (extend E (b i)) x)
            (b j))
          = g.ricciAt x (A (sharp i)) (b i) := by
    intro i
    let F : TM x → TM x → ℝ := fun p q ↦
      g.inner x
        (CovariantDerivative.curvatureOp g.leviCivita
          (extend E p) (extend E (A (sharp i))) (extend E (b i)) x)
        q
    have hswap := sum_metricDualVectorAt_contraction_swap
      (g := g) (x := x) (F := F)
      (fun p₁ p₂ q ↦ by
      ...
      (fun c p q ↦ by
        ...
        simp [smul_eq_mul])
    have hric :
        g.ricciAt x (A (sharp i)) (b i) =
          ∑ j, g.inner x
            (CovariantDerivative.curvatureOp g.leviCivita
              (extend E (b j)) (extend E (A (sharp i))) (extend E (b i)) x)
            (sharp j) := by
      simpa [b, sharp, A] using
        ricciAt_eq_curvature_inner_contraction
          (g := g) (x := x) (u := A (sharp i)) (w := b i)
    change (∑ j, F (sharp j) (b j)) =
      g.ricciAt x (A (sharp i)) (b i)
    rw [hswap, hric]
  have hnorm :
      (∑ i, g.ricciAt x (A (sharp i)) (b i)) =
        g.ricciNormSqAt x := by
    let F : TM x → TM x → ℝ := fun p q ↦
      g.inner x ((A ∘ₗ A) p) q
    have hswap := sum_metricDualVectorAt_contraction_swap
    ...
    have htrace :
        (∑ i, F (b i) (sharp i)) =
          LinearMap.trace ℝ (TM x) (A ∘ₗ A) := by
      dsimp [F]
      rw [LinearMap.trace_eq_matrix_trace ℝ b, Matrix.trace]
      refine Finset.sum_congr rfl fun i _ ↦ ?_
      rw [Matrix.diag_apply, LinearMap.toMatrix_apply]
      exact (coord_eq_inner_metricDualVectorAt
        (g := g) (x := x) i ((A ∘ₗ A) (b i))).symm
    calc
      (∑ i, g.ricciAt x (A (sharp i)) (b i)) =
          ∑ i, F (sharp i) (b i) := hleft
      _ = ∑ i, F (b i) (sharp i) := hswap
      _ = LinearMap.trace ℝ (TM x) (A ∘ₗ A) := htrace
      _ = g.ricciNormSqAt x := by
            exact (g.ricciNormSqAt_eq_trace x).symm
  change
    (∑ j, lichnerowiczCurvatureAt g (ricciVariationField g) x
      (b j) (sharp j)) = g.ricciNormSqAt x
  calc
    (∑ j, lichnerowiczCurvatureAt g (ricciVariationField g) x
      (b j) (sharp j))
        =
    ...
    _ =
      ∑ j, ∑ i,
        g.inner x
          ...
          (b j) := by
        refine Finset.sum_congr rfl fun j _ ↦ ?_
        exact Finset.sum_congr rfl fun i _ ↦ hentry i j
    _ =
      ∑ i, ∑ j,
        g.inner x
          (CovariantDerivative.curvatureOp g.leviCivita
          ...
          (b j) := by
        rw [Finset.sum_comm]
    _ = ∑ i, g.ricciAt x (A (sharp i)) (b i) := by
        exact Finset.sum_congr rfl fun i _ ↦ htoRic i
    _ = g.ricciNormSqAt x := hnorm
...
/--
Fixed-metric trace of the corrected tensor RHS:
`tr_g(Δ_∇ Ric + 2 Rm(Ric, ·) - Ric·Ric - Ric·Ric) = ΔR`.
-/
theorem ricciEvolutionTensorRHSAt_trace_eq_scalar_laplacian
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M)
    (hTrace : RicciEvolutionTraceIdentitiesAt g x) :
    (letI : FiniteDimensional ℝ (TM x) :=
      ...
      let sharp : Fin (Module.finrank ℝ (TM x)) → TM x :=
        fun j ↦ metricDualVectorAt g x (b.coord j);
      ∑ j, ricciEvolutionTensorRHSAt g x (b j) (sharp j))
      =
        g.laplacianAt (fun y ↦ g.scalarAt y) x := by
  letI : FiniteDimensional ℝ (TM x) :=
    inferInstanceAs (FiniteDimensional ℝ E)
  ...
  have hDecomp :
      (∑ j, ricciEvolutionTensorRHSAt g x (b j) (sharp j)) =
        (∑ j, roughTensorLaplacianAt g (ricciVariationField g) x
          ...
          - (∑ j, ricciActionOnTensorAt g (ricciVariationField g) x
            (b j) (sharp j)) := by
    simp only [ricciEvolutionTensorRHSAt]
    rw [Finset.sum_sub_distrib, Finset.sum_add_distrib, ← Finset.mul_sum]
  change
    (∑ j, ricciEvolutionTensorRHSAt g x (b j) (sharp j)) =
      g.laplacianAt (fun y ↦ g.scalarAt y) x
  rw [hDecomp, hRough', hAction', hL]
  ring

theorem ricciEvolutionTensorRHSAt_trace_eq_scalar_laplacian_of_traceSecondRegularity
    (g : ClosedSmoothRiemannianMetric n M)
    [CovariantDerivative.ContMDiffCovariantDerivative g.leviCivita 1]
    (x : M)
    (hReg : RicciEvolutionTraceSecondRegularityAt g x) :
...
/--
Moving-metric reconciliation for the corrected tensor RHS under Ricci flow.

The corrected fixed-metric tensor trace is only `ΔR`; subtracting the
metric-variation Ricci pairing, as in the scalar-variation chain, contributes
the missing `+2 |Ric|²` because `h = ∂ₜg = -2 Ric`.
-/
theorem ricciEvolutionTensorRHS_trace_sub_metricVariationRicciPairing_eq_hamilton_rhs
    {gt : ℝ → ClosedSmoothRiemannianMetric n M} {t₀ : ℝ} {x : M}
    [∀ t : ℝ,
      CovariantDerivative.ContMDiffCovariantDerivative (gt t).leviCivita 1]
    (hTrace : RicciEvolutionTraceIdentitiesAt (gt t₀) x)
    (hEq : ∀ v w : TM x,
      timeDerivAt gt t₀ x v w = -2 * (gt t₀).ricciAt x v w) :
    (letI : FiniteDimensional ℝ (TM x) :=
        inferInstanceAs (FiniteDimensional ℝ E)
      ...
      ∑ j, ricciEvolutionTensorRHSAt (gt t₀) x (b j) (sharp j))
      - metricVariationRicciPairingAt (gt t₀) (timeDerivAt gt t₀) x
      =
  ...
  have hRHS :
    ...
    simpa [g, b, sharp] using
      ricciEvolutionTensorRHSAt_trace_eq_scalar_laplacian
        (g := g) (x := x) (by simpa [g] using hTrace)
  have hPair :
      metricVariationRicciPairingAt (gt t₀) (timeDerivAt gt t₀) x =
...
theorem ricciEvolutionTensorRHS_trace_sub_metricVariationRicciPairing_eq_hamilton_rhs_of_traceSecondRegularity
    {gt : ℝ → ClosedSmoothRiemannianMetric n M} {t₀ : ℝ} {x : M}
    [∀ t : ℝ,
      CovariantDerivative.ContMDiffCovariantDerivative (gt t).leviCivita 1]
    (hReg : RicciEvolutionTraceSecondRegularityAt (gt t₀) x)
    (hEq : ∀ v w : TM x,
      timeDerivAt gt t₀ x v w = -2 * (gt t₀).ricciAt x v w) :
    (letI : FiniteDimensional ℝ (TM x) :=
        inferInstanceAs (FiniteDimensional ℝ E)
...
theorem satisfiesRicciEvolutionAt_of_ricciFlow_traceSecondRegularity
    ...
    (hRicC2 :
      CovTensor2ExtContMDiffAt (ricciVariationField (gt t₀)) x 2)
    (hScalar₂ :
  ...
  have hCurvComm : RicciSecondDerivCurvatureCommutationAt (gt t₀) x :=
    RicciSecondDerivCurvatureCommutationAt.of_traceSecondRegularity
      (g := gt t₀) (x := x)
      hRicSecond hRicC2 hScalar₂ hScalarExt₂
  exact satisfiesRicciEvolutionAt_of_ricciFlow_curvatureCommutation
    (gt := gt) (t₀ := t₀) (x := x)
    hreg hgt hExt hNear hBridge hSecond hFlowNear hCurvComm
