...
/-!
# Finite extinction from scalar continuity on surgery slabs

The interior Hamilton comparison only uses scalar continuity on each compact
relative-time slab.  These adapters carry that local hypothesis through the
geometric width contradiction and the first-extinction conclusion, without
requiring any assertion about the metric path outside its surgery segment.
-/
...
/-- Type-changing Hamilton/width contradiction from scalar continuity on each
compact surgery slab. -/
theorem no_global_nonnegative_width_of_hamilton_surgery_schedule_family_compact_Ioc_continuousOn
    {X : ℕ → Type u}
...
/-- If the slab-local analytic and surgery hypotheses hold on every alive
segment, then some segment is extinct. -/
theorem exists_extinct_segment_of_hamilton_surgery_schedule_family_compact_Ioc_continuousOn
    {X : ℕ → Type u}
    [∀ k, TopologicalSpace (X k)] [∀ k, T2Space (X k)]
    [∀ k, ChartedSpace (ClosedSmoothModel 3) (X k)]
    [∀ k, IsManifold (closedSmoothModelWithCorners 3) ∞ (X k)]
  ...
  exact
    no_global_nonnegative_width_of_hamilton_surgery_schedule_family_compact_Ioc_continuousOn
...
/-- The slab-local strict-positive-time Hamilton schedule has a first extinct
segment, with every earlier segment alive. -/
theorem exists_first_extinct_segment_of_hamilton_surgery_schedule_family_compact_Ioc_continuousOn
    {X : ℕ → Type u}
    [∀ k, TopologicalSpace (X k)] [∀ k, T2Space (X k)]
    [∀ k, ChartedSpace (ClosedSmoothModel 3) (X k)]
    [∀ k, IsManifold (closedSmoothModelWithCorners 3) ∞ (X k)]
  ...
  exact
    exists_extinct_segment_of_hamilton_surgery_schedule_family_compact_Ioc_continuousOn
