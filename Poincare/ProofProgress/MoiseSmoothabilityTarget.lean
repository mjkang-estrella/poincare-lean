...
/--
The complete Moise consumer can be selected and kept open while also exposing
the fixed-target package prefix, transported concrete smooth/surgery
structures, and bridge-tail compatibility witnesses.  This gives
final-certificate consumers a single synchronized object carrying the residual
smoothability package requirement, both Moise theorem-shaped targets, the
global recognition/target families, and the concrete fixed-target witnesses.
-/
theorem moiseSmoothability_selectedCompleteConsumer_fields_fixedTarget_packageMoisePrefix_concrete_structures_and_bridge_tail_of_completeConsumerPayload
    (payload : Nonempty (MoiseSmoothabilityCompleteConsumerPayload.{u}))
    (M : Type u) [TopologicalSpace M] [T2Space M]
    [ChartedSpace ThreeManifoldModel M]
    [SimplyConnectedSpace M] [CompactSpace M] :
...
/--
The package-layer smoothability requirement exposes the same one-point
recognition complete-consumer endpoint as the concrete residual
`SmoothabilityPackage`.  This lets final-assembly callers remain at
`DependencyPackageLayer.smoothabilityPackage` while still recovering the
selected complete Moise consumer, theorem-shaped smooth and surgery Moise
targets, fixed-target one-point and sphere recognition, transported concrete
smooth/surgery prerequisites, Moise package prefix, and bridge-tail
compatibility data from the same package-layer input.
-/
theorem smoothabilityPackage_layerRequirement_completeMoiseConsumer_fixedTarget_packageMoisePrefix_concrete_structures_and_bridge_tail_of_packageLayer_and_onePointRecognition
    (smoothability :
      dependencyPackageLayerRequirement.{u}
        DependencyPackageLayer.smoothabilityPackage)
    (recognizeOnePoint :
  ...
  exact
    ⟨ package
    , rfl
    , smoothabilityPackage_requirement_completeMoiseConsumer_fixedTarget_packageMoisePrefix_concrete_structures_and_bridge_tail_of_smoothabilityPackage_and_onePointRecognition
        package recognizeOnePoint M
    ⟩
...
selected complete Moise consumer, theorem-shaped smooth and surgery Moise
targets, fixed-target sphere and one-point recognitions, transported concrete
smooth/surgery prerequisites, Moise package prefix, and bridge-tail
compatibility data synchronized with the same package-layer input.
-/
theorem smoothabilityPackage_layerRequirement_completeMoiseConsumer_fixedTarget_packageMoisePrefix_concrete_structures_and_bridge_tail_of_packageLayer_and_threeSphereRecognition
    (smoothability :
      dependencyPackageLayerRequirement.{u}
        DependencyPackageLayer.smoothabilityPackage)
    (recognizeSphere :
  ...
  exact
    ⟨ package
    , rfl
    , smoothabilityPackage_requirement_completeMoiseConsumer_fixedTarget_packageMoisePrefix_concrete_structures_and_bridge_tail_of_smoothabilityPackage_and_threeSphereRecognition
        package recognizeSphere M
    ⟩

/--
The package-layer smoothability requirement and a uniform `ThreeSphere`
recognition route expose the complete Moise consumer endpoint uniformly over
all compact simply connected targets.  This packages the fixed-target
transported smooth/surgery prerequisites, Moise prefix, and bridge-tail
compatibility data into the family-shaped contract consumed by final assembly.
-/
theorem smoothabilityPackage_layerRequirement_completeMoiseConsumer_family_packageMoisePrefix_concrete_structures_and_bridge_tail_of_packageLayer_and_threeSphereRecognition
    (smoothability :
      dependencyPackageLayerRequirement.{u}
        DependencyPackageLayer.smoothabilityPackage)
    (recognizeSphere :
