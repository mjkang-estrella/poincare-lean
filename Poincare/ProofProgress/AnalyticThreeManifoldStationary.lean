...
/--
Concrete stationary-zero production data exposes the analytic sub-obligation
payload, the constructed analytic-foundation package, and the Ricci
contraction, scalar-curvature, equation-boundary, and curvature-evolution
theory attached to the same derived stationary-zero Ricci flow.
-/
theorem stationary_zero_subobligations_package_boundary_contraction_scalar_and_curvature_of_production_data_current_api
    {n : ℕ∞ω}
    {M : Type u} [TopologicalSpace M] [ChartedSpace ThreeManifoldModel M]
    [IsManifold ThreeManifoldModelWithCorners 1 M]
    [IsManifold ThreeManifoldModelWithCorners 2 M]
  ...
  let flow :=
    stationary_zero_ricci_flow_data_current_api
      metric identifiesDerivative identifiesRicci
  rcases
    stationary_zero_subobligations_package_boundary_and_curvature_of_production_data_current_api
      metric identifiesDerivative identifiesRicci data with
    ⟨hSubobligations, package, hflow, hBoundary, hCurvatureEvolution⟩
  have hRicciContraction :
      HasRicciContractionTheory (curvature_data_of_ricci_flow_data flow) := by
    dsimp [flow]
    rw [← hflow]
    exact ricci_contraction_theory_of_analytic_foundation_package package
  have hScalarCurvature :
      HasScalarCurvatureTheory (curvature_data_of_ricci_flow_data flow) := by
    dsimp [flow]
    rw [← hflow]
    exact scalar_curvature_theory_of_analytic_foundation_package package
  exact
...
/--
Concrete stationary-zero production data exposes the combined analytic
foundation payload needed by downstream Ricci-flow interfaces: sub-obligations,
the stored analytic-foundation package, equation-boundary evidence,
Ricci/scalar contraction theory, the four explicit evolution-equation fields,
and aggregate curvature evolution for the same derived stationary-zero flow.
-/
theorem stationary_zero_full_analytic_foundation_payload_of_production_data_current_api
    {n : ℕ∞ω}
    {M : Type u} [TopologicalSpace M] [ChartedSpace ThreeManifoldModel M]
    [IsManifold ThreeManifoldModelWithCorners 1 M]
    [IsManifold ThreeManifoldModelWithCorners 2 M]
  ...
  let flow :=
    stationary_zero_ricci_flow_data_current_api
      metric identifiesDerivative identifiesRicci
  rcases
    stationary_zero_subobligations_package_boundary_contraction_scalar_and_curvature_of_production_data_current_api
      metric identifiesDerivative identifiesRicci data with
    ⟨ hSubobligations, package, hflow, hBoundary, hRicciContraction,
      hScalarCurvature, _hCurvatureEvolution ⟩
  rcases
    stationary_zero_analytic_foundation_evolution_payload_of_production_data_current_api
      metric identifiesDerivative identifiesRicci data with
    ⟨ _evolutionPackage, _hEvolutionFlow, hMetricEvolution,
      hRicciTensorEvolution, hScalarCurvatureEvolution,
      hCurvatureNormEvolution, hCurvatureEvolution ⟩
  exact
    ⟨ hSubobligations, package, hflow, hBoundary, hRicciContraction,
      hScalarCurvature, hMetricEvolution, hRicciTensorEvolution,
      hScalarCurvatureEvolution, hCurvatureNormEvolution,
      hCurvatureEvolution ⟩

/--
Concrete stationary-zero production data also exposes the theorem-shaped
analytic-foundation statement projected from the same stored package, together
with the full downstream boundary payload for the derived stationary-zero flow.
-/
theorem stationary_zero_full_analytic_foundation_statement_payload_of_production_data_current_api
    {n : ℕ∞ω}
    {M : Type u} [TopologicalSpace M] [ChartedSpace ThreeManifoldModel M]
    [IsManifold ThreeManifoldModelWithCorners 1 M]
    [IsManifold ThreeManifoldModelWithCorners 2 M]
    ...
    AnalyticFoundationSubobligationsPayload flow ∧
      ∃ package :
        ...
        HasCurvatureEvolutionEquations flow := by
  rcases
    stationary_zero_full_analytic_foundation_payload_of_production_data_current_api
      metric identifiesDerivative identifiesRicci data with
    ⟨ hSubobligations, package, hflow, hBoundary, hRicciContraction,
      hScalarCurvature, hMetricEvolution, hRicciTensorEvolution,
      hScalarCurvatureEvolution, hCurvatureNormEvolution,
...
/--
Universal stationary-zero production data exposes the concrete stationary-zero
surface equations for every package-layer target, while retaining the stored
analytic package and the terminal curvature-evolution field.
-/
theorem stationaryZeroAnalyticFoundation_surfaceCurvatureFamily_of_stationaryZeroProductionData_current_api
    (data :
  ...
  exact
    ⟨n, smooth2, metric, identifiesDerivative, identifiesRicci,
      productionData,
      three_manifold_stationary_zero_analytic_foundation_surface_and_curvature_payload_of_production_data_current_api
        metric identifiesDerivative identifiesRicci productionData⟩
...
/--
Universal stationary-zero production data exposes the full analytic statement
payload pointwise, retaining the smoothness-2 upgrade, stationary metric, zero
derivative/Ricci identifications, concrete production data, stored analytic
package, equation boundary, Ricci/scalar theories, and evolution equations.
-/
theorem stationary_zero_full_analytic_statement_payload_family_of_stationaryZeroProductionData_current_api
    (data :
      StationaryZeroAnalyticFoundationPackageLayerProductionData.{u}) :
    ∀ (M : Type u) [TopologicalSpace M] [T2Space M]
      [ChartedSpace ThreeManifoldModel M]
  ...
  exact
    ⟨ n, smooth2, metric, identifiesDerivative, identifiesRicci,
      productionData,
      stationary_zero_full_analytic_foundation_statement_payload_of_production_data_current_api
        metric identifiesDerivative identifiesRicci productionData ⟩
...
/--
Detailed stationary-zero analytic assembly payload.  This strengthens the
compact package/statement/curvature family by also retaining the concrete
stationary-zero production witnesses and every analytic boundary/evolution
field produced from them.
-/
structure StationaryZeroAnalyticFoundationDetailedAssemblyPayload where
  analyticFoundationPackageRequirement :
    dependencyPackageLayerRequirement.{u}
      DependencyPackageLayer.analyticFoundationPackage
  ricciFlowAnalyticFoundationMilestone :
...
/--
Universal stationary-zero production data constructs the detailed analytic
assembly payload, retaining both the compact package projection and the full
stationary-zero analytic statement payload family.
-/
def stationaryZeroAnalyticFoundationDetailedAssemblyPayload
    (data :
      StationaryZeroAnalyticFoundationPackageLayerProductionData.{u}) :
    StationaryZeroAnalyticFoundationDetailedAssemblyPayload.{u} where
  analyticFoundationPackageRequirement :=
    analyticFoundationPackage_requirement_of_stationaryZeroProductionData_current_api
      data
  ricciFlowAnalyticFoundationMilestone :=
  ...
  packageStatementCurvatureFamily :=
    (analyticFoundationPackage_statement_and_curvature_payload_of_stationaryZeroProductionData_current_api
      data).2.2
  fullStatementPayloadFamily :=
    stationary_zero_full_analytic_statement_payload_family_of_stationaryZeroProductionData_current_api
      data

/--
The detailed analytic assembly payload unpacks to the package-layer endpoint,
the compact curvature family, and the full stationary-zero statement payload
family.
-/
theorem stationaryZeroAnalyticFoundationDetailedAssemblyPayload_fields
    (data :
      StationaryZeroAnalyticFoundationPackageLayerProductionData.{u}) :
    dependencyPackageLayerRequirement.{u}
        DependencyPackageLayer.analyticFoundationPackage ∧
  ...
  exact
    ...
    , payload.ricciFlowAnalyticFoundationMilestone
    , payload.packageStatementCurvatureFamily
    , payload.fullStatementPayloadFamily
    ⟩

/--
The detailed analytic assembly payload also exposes a compact package-plus-
subobligations family: for every target, it gives the actual stationary-zero
flow, the stored analytic package whose flow data is that flow, the
sub-obligation payload used to build the package, the theorem-shaped analytic
statement, the equation-boundary statement, and the terminal curvature
evolution field.
-/
theorem stationaryZeroAnalyticFoundationDetailedAssemblyPayload_packageSubobligationsFamily
  ...
  rcases fullPayload with
    ⟨subobligations, package, packageFlow, statement, boundary,
      _ricciContraction, _scalarCurvature, _metricEvolution,
      _ricciTensorEvolution, _scalarCurvatureEvolution,
      _curvatureNormEvolution, curvatureEvolution⟩
  exact
    ⟨n, flow, package, subobligations, packageFlow, statement, boundary,
      curvatureEvolution⟩

/--
Construct the compact package-plus-subobligations family directly from
universal stationary-zero production data.
-/
theorem stationaryZeroAnalyticFoundation_packageSubobligationsFamily_of_stationaryZeroProductionData
    (data :
      StationaryZeroAnalyticFoundationPackageLayerProductionData.{u}) :
    ∀ (M : Type u) [TopologicalSpace M] [T2Space M]
      [ChartedSpace ThreeManifoldModel M]
...
/--
The detailed analytic assembly payload also exposes a normalized full analytic
package family.  Compared with the raw stationary-zero production family, this
projection keeps the actual flow and package at the front while retaining the
sub-obligation payload, package-flow identity, analytic statement, equation
boundary, Ricci/scalar contraction theories, all individual evolution
equations, and the aggregate curvature-evolution field.
-/
theorem stationaryZeroAnalyticFoundationDetailedAssemblyPayload_fullPackageEvolutionFamily
    (payload : StationaryZeroAnalyticFoundationDetailedAssemblyPayload.{u}) :
    ∀ (M : Type u) [TopologicalSpace M] [T2Space M]
      [ChartedSpace ThreeManifoldModel M]
      [SimplyConnectedSpace M] [CompactSpace M]
  ...
  exact
    ⟨ n, flow, package, subobligations, packageFlow, statement, boundary,
      ricciContraction, scalarCurvature, metricEvolution,
      ricciTensorEvolution, scalarCurvatureEvolution, curvatureNormEvolution,
      curvatureEvolution ⟩

/--
Universal stationary-zero production data directly constructs the normalized
full analytic package/evolution family.
-/
theorem stationaryZeroAnalyticFoundation_fullPackageEvolutionFamily_of_stationaryZeroProductionData
    (data :
      StationaryZeroAnalyticFoundationPackageLayerProductionData.{u}) :
    ∀ (M : Type u) [TopologicalSpace M] [T2Space M]
      [ChartedSpace ThreeManifoldModel M]
...
/--
Any inhabited detailed stationary-zero analytic assembly payload can be
consumed directly as the analytic package boundary: it exposes the package
requirement, analytic milestone, compact package/curvature family, and the
normalized full package/evolution family without retaining the original
stationary-zero production data.
-/
theorem stationaryZeroAnalyticFoundation_nonemptyDetailedAssemblyPayload_fullPackageEvolutionFamily
    (payload :
...
/--
Universal stationary-zero production data constructs the inhabited detailed
analytic assembly payload and exposes, from that same payload, the package-layer
requirement, analytic milestone, compact package/statement/curvature family,
and normalized full package/evolution family.
-/
theorem stationaryZeroAnalyticFoundation_nonemptyDetailedAssemblyPayload_and_fullPackageEvolutionFamily_of_stationaryZeroProductionData
    (data :
      StationaryZeroAnalyticFoundationPackageLayerProductionData.{u}) :
    Nonempty StationaryZeroAnalyticFoundationDetailedAssemblyPayload.{u} ∧
      dependencyPackageLayerRequirement.{u}
...
/--
Any inhabited detailed stationary-zero analytic assembly payload exposes both
the raw stationary-zero production family retained in the payload and the
normalized full package/evolution family derived from it.  This lets downstream
finite-extinction consumers choose whether they need the original metric and
identification witnesses or the package-first normalized view.
-/
theorem stationaryZeroAnalyticFoundation_nonemptyDetailedAssemblyPayload_raw_and_normalized_fullFamilies
    (payload :
      Nonempty StationaryZeroAnalyticFoundationDetailedAssemblyPayload.{u}) :
    (∀ (M : Type u) [TopologicalSpace M] [T2Space M]
      [ChartedSpace ThreeManifoldModel M]
...
/--
Consumer payload for the analytic-foundation pillar.  It retains the concrete
detailed stationary-zero assembly payload, the package-layer and milestone
requirements, the compact package/curvature family, the compact
package/subobligation and evolution-field projections, the normalized full
package/evolution family, and the raw stationary-zero production family.
-/
structure StationaryZeroAnalyticFoundationCompleteConsumerPayload where
  detailedPayload :
...
/--
An inhabited detailed stationary-zero analytic assembly payload constructs the
complete analytic consumer payload, without requiring downstream users to carry
the original production-data source.
-/
theorem stationaryZeroAnalyticFoundation_completeConsumerPayload_of_nonemptyDetailedAssemblyPayload
    (payload :
      Nonempty StationaryZeroAnalyticFoundationDetailedAssemblyPayload.{u}) :
    Nonempty StationaryZeroAnalyticFoundationCompleteConsumerPayload.{u} := by
  rcases payload with ⟨payload⟩
  exact
...
/--
Universal stationary-zero production data constructs the complete analytic
consumer payload, retaining both raw production witnesses and normalized
package-first analytic foundation data.
-/
theorem stationaryZeroAnalyticFoundation_completeConsumerPayload_of_stationaryZeroProductionData
    (data :
      StationaryZeroAnalyticFoundationPackageLayerProductionData.{u}) :
...
/--
An inhabited complete analytic consumer payload exposes the package-layer
requirement, the Ricci-flow analytic milestone, the normalized package-first
evolution family, and the raw stationary-zero production family carried by the
same detailed assembly object.
-/
theorem stationaryZeroAnalyticFoundation_requirements_fullPackageEvolution_and_rawProductionFamily_of_completeConsumerPayload
    (payload :
      Nonempty StationaryZeroAnalyticFoundationCompleteConsumerPayload.{u}) :
    dependencyPackageLayerRequirement.{u}
        DependencyPackageLayer.analyticFoundationPackage ∧
  ...
  exact
    ⟨ payload.analyticFoundationPackageRequirement
    , payload.ricciFlowAnalyticFoundationMilestone
    , payload.fullPackageEvolutionFamily
    , payload.rawFullStatementPayloadFamily
    ⟩

/--
Universal stationary-zero production data constructs the complete analytic
consumer payload and immediately exposes the target-free analytic families
carried by that payload: the analytic package-layer requirement, the
Ricci-flow analytic milestone, the normalized package/evolution family, and
the raw stationary-zero production family.  Downstream finite-extinction
consumers can therefore use the production-data boundary without first
selecting a fixed target manifold.
-/
theorem stationaryZeroAnalyticFoundation_completeConsumerPayload_requirements_fullPackageEvolution_and_rawProductionFamily_of_stationaryZeroProductionData
    (data :
      StationaryZeroAnalyticFoundationPackageLayerProductionData.{u}) :
    Nonempty StationaryZeroAnalyticFoundationCompleteConsumerPayload.{u} ∧
      dependencyPackageLayerRequirement.{u}
        ...
        [IsManifold ThreeManifoldModelWithCorners 1 M],
          ...
          AnalyticFoundationSubobligationsPayload flow ∧
            ∃ package :
              ...
              HasCurvatureEvolutionEquations flow) := by
  let payload :=
    stationaryZeroAnalyticFoundation_completeConsumerPayload_of_stationaryZeroProductionData
      data
  exact
    ⟨ payload
    , stationaryZeroAnalyticFoundation_requirements_fullPackageEvolution_and_rawProductionFamily_of_completeConsumerPayload
        payload
    ⟩
...
For a fixed target, the complete analytic consumer payload exposes the
analytic package-layer requirement, the Ricci-flow analytic milestone, and the
raw stationary-zero production witnesses from the same detailed assembly.
-/
theorem stationaryZeroAnalyticFoundation_requirements_and_fixedTarget_rawProductionPayload_of_completeConsumerPayload
    (payload :
      Nonempty StationaryZeroAnalyticFoundationCompleteConsumerPayload.{u})
    (M : Type u) [TopologicalSpace M] [T2Space M]
    [ChartedSpace ThreeManifoldModel M]
  ...
  exact
    ⟨ payload.analyticFoundationPackageRequirement
    , payload.ricciFlowAnalyticFoundationMilestone
    , payload.rawFullStatementPayloadFamily M
    ⟩

/--
For a fixed target, the complete analytic consumer payload exposes the full
analytic endpoint at once: package-layer requirement, Ricci-flow analytic
milestone, compact package/subobligation projection, standalone evolution
fields, normalized package/evolution payload, and the raw stationary-zero
production witnesses.  This keeps all downstream analytic consumers tied to
the same complete payload instead of selecting the fixed-target projections
independently.
-/
theorem stationaryZeroAnalyticFoundation_requirements_and_fixedTarget_completeAnalyticEndpoint_of_completeConsumerPayload
    (payload :
      Nonempty StationaryZeroAnalyticFoundationCompleteConsumerPayload.{u})
    (M : Type u) [TopologicalSpace M] [T2Space M]
    [ChartedSpace ThreeManifoldModel M]
    ...
    dependencyPackageLayerRequirement.{u}
      ...
      AnalyticFoundationSubobligationsPayload flow ∧
        ∃ package :
          ...
          HasCurvatureEvolutionEquations flow := by
  rcases
    stationaryZeroAnalyticFoundation_requirements_and_fixedTarget_compactProjection_of_completeConsumerPayload
      payload M with
    ⟨ analyticFoundationPackageRequirement
    , ricciFlowAnalyticFoundationMilestone
    , compactProjection
  ...
  rcases
    stationaryZeroAnalyticFoundation_requirements_and_fixedTarget_fullPackageEvolution_of_completeConsumerPayload
      payload M with
    ⟨ _analyticFoundationPackageRequirement
    , _ricciFlowAnalyticFoundationMilestone
    , fullPackageEvolution
    ⟩
  rcases
    stationaryZeroAnalyticFoundation_requirements_and_fixedTarget_rawProductionPayload_of_completeConsumerPayload
      payload M with
    ⟨ _analyticFoundationPackageRequirement'
    , _ricciFlowAnalyticFoundationMilestone'
...
/--
The complete analytic consumer payload is exactly the analytic package-layer
requirement, Ricci-flow analytic milestone, compact package/curvature family,
and raw stationary-zero full statement family.  The reverse direction rebuilds
the detailed stationary-zero assembly payload, after which the existing
complete-consumer constructor derives the normalized package/evolution
families.
-/
theorem stationaryZeroAnalyticFoundation_nonemptyCompleteConsumerPayload_iff_requirements_and_rawStatementFamilies :
    Nonempty StationaryZeroAnalyticFoundationCompleteConsumerPayload.{u} ↔
      dependencyPackageLayerRequirement.{u}
          DependencyPackageLayer.analyticFoundationPackage ∧
        dependencyMilestoneRequirement.{u}
    ...
    exact
      stationaryZeroAnalyticFoundation_completeConsumerPayload_of_nonemptyDetailedAssemblyPayload
        ⟨ { analyticFoundationPackageRequirement :=
            ...
            ricciFlowAnalyticFoundationMilestone :=
              ricciFlowAnalyticFoundationMilestone
            packageStatementCurvatureFamily := packageStatementCurvatureFamily
            fullStatementPayloadFamily := rawFullStatementPayloadFamily } ⟩

/--
The complete analytic consumer payload is also equivalent to the package-layer
requirement, Ricci-flow analytic milestone, normalized full package/evolution
family, and raw stationary-zero full statement family.  The reverse direction
derives the compact package/curvature family from the normalized full
package/evolution family before rebuilding the detailed assembly payload.
-/
theorem stationaryZeroAnalyticFoundation_nonemptyCompleteConsumerPayload_iff_requirements_fullPackageEvolution_and_rawProductionFamily :
    Nonempty StationaryZeroAnalyticFoundationCompleteConsumerPayload.{u} ↔
      dependencyPackageLayerRequirement.{u}
          DependencyPackageLayer.analyticFoundationPackage ∧
        dependencyMilestoneRequirement.{u}
  ...
  · rintro
      ...
      , rawFullStatementPayloadFamily
      ⟩
    let packageStatementCurvatureFamily :
        ∀ (M : Type u) [TopologicalSpace M] [T2Space M]
          [ChartedSpace ThreeManifoldModel M]
          [SimplyConnectedSpace M] [CompactSpace M]
          [IsManifold ThreeManifoldModelWithCorners 1 M],
              ...
              RicciFlowAnalyticFoundationStatement
                ...
                HasCurvatureEvolutionEquations
                  (ricci_flow_data_of_analytic_foundation_package
                    package) := by
      intro M _top _t2 _charted _simple _compact _smooth1
      rcases fullPackageEvolutionFamily M with
        ⟨n, flow, package, _subobligations, hFlow, hStatement,
          _equationBoundary, _ricci, _scalar, _metricEvolution,
          _ricciEvolution, _scalarEvolution, _normEvolution, hCurvature⟩
      exact
        ⟨n, package, hStatement, by
          simpa [← hFlow] using hCurvature⟩
    exact
      stationaryZeroAnalyticFoundation_completeConsumerPayload_of_nonemptyDetailedAssemblyPayload
        ⟨ { analyticFoundationPackageRequirement :=
              analyticFoundationPackageRequirement
...
/--
The complete analytic consumer payload can also be reconstructed from the
compact package/subobligation family, the standalone evolution-field family,
and the raw stationary-zero production family.  The reverse direction derives
the package/curvature family from the package/subobligation family, then
rebuilds the detailed assembly payload.
-/
theorem stationaryZeroAnalyticFoundation_nonemptyCompleteConsumerPayload_iff_requirements_compactProjection_and_rawProductionFamily :
    Nonempty StationaryZeroAnalyticFoundationCompleteConsumerPayload.{u} ↔
      dependencyPackageLayerRequirement.{u}
          DependencyPackageLayer.analyticFoundationPackage ∧
        dependencyMilestoneRequirement.{u}
  ...
  constructor
  · intro payload
    rcases stationaryZeroAnalyticFoundation_compactFamilies_of_completeConsumerPayload
        payload with
      ⟨_packageStatementCurvatureFamily, packageSubobligationsFamily,
        evolutionFieldsFamily⟩
    rcases payload with ⟨payload⟩
    exact
      ⟨ payload.analyticFoundationPackageRequirement
              ...
              RicciFlowAnalyticFoundationStatement
                ...
                HasCurvatureEvolutionEquations
                  (ricci_flow_data_of_analytic_foundation_package
                    package) := by
      intro M _top _t2 _charted _simple _compact _smooth1
      rcases packageSubobligationsFamily M with
        ⟨n, flow, package, _subobligations, hFlow, hStatement,
          _equationBoundary, hCurvature⟩
      exact
        ⟨n, package, hStatement, by
          simpa [← hFlow] using hCurvature⟩
    exact
...
/--
Compact projection data plus the raw stationary-zero production family
constructs the complete analytic consumer payload and immediately exposes the
fixed-target normalized package/evolution payload from that constructed object.
This is the downstream-facing route when callers have already separated the
package/subobligation projection, evolution-field projection, and raw
stationary-zero production witnesses.
-/
theorem stationaryZeroAnalyticFoundation_requirements_compactProjection_rawProductionFamily_completeConsumerPayload_and_fixedTarget_fullPackageEvolution
    (analyticFoundationPackageRequirement :
      dependencyPackageLayerRequirement.{u}
        DependencyPackageLayer.analyticFoundationPackage)
    (ricciFlowAnalyticFoundationMilestone :
  ...
  let payload :
      Nonempty StationaryZeroAnalyticFoundationCompleteConsumerPayload.{u} :=
    stationaryZeroAnalyticFoundation_nonemptyCompleteConsumerPayload_iff_requirements_compactProjection_and_rawProductionFamily.2
      ⟨ analyticFoundationPackageRequirement
      , ricciFlowAnalyticFoundationMilestone
      , packageSubobligationsFamily
      , evolutionFieldsFamily
...
/--
Normalized full package/evolution data plus the raw stationary-zero production
family constructs the complete analytic consumer payload and immediately
exposes the fixed-target normalized package/evolution endpoint from that
constructed object.  This is the downstream-facing route when callers already
have the full analytic package/evolution family rather than the compact
package/subobligation and standalone evolution-field projections separately.
-/
theorem stationaryZeroAnalyticFoundation_requirements_fullPackageEvolution_rawProductionFamily_completeConsumerPayload_and_fixedTarget_fullPackageEvolution
    (analyticFoundationPackageRequirement :
      dependencyPackageLayerRequirement.{u}
        DependencyPackageLayer.analyticFoundationPackage)
    (ricciFlowAnalyticFoundationMilestone :
  ...
  let payload :
      Nonempty StationaryZeroAnalyticFoundationCompleteConsumerPayload.{u} :=
    stationaryZeroAnalyticFoundation_nonemptyCompleteConsumerPayload_iff_requirements_fullPackageEvolution_and_rawProductionFamily.2
      ⟨ analyticFoundationPackageRequirement
      , ricciFlowAnalyticFoundationMilestone
      , fullPackageEvolutionFamily
      , rawFullStatementPayloadFamily
...
/--
Normalized full package/evolution data plus the raw stationary-zero production
family also constructs a named complete analytic consumer payload whose
complete fixed-target analytic endpoint is available immediately.  Unlike the
single normalized projection above, this retains the compact projection,
standalone evolution-field projection, normalized full package/evolution
projection, and raw stationary-zero production payload through the same
selected complete consumer.
-/
theorem stationaryZeroAnalyticFoundation_requirements_fullPackageEvolution_rawProductionFamily_completeConsumerPayload_and_fixedTarget_completeAnalyticEndpoint
    (analyticFoundationPackageRequirement :
      dependencyPackageLayerRequirement.{u}
        DependencyPackageLayer.analyticFoundationPackage)
    (ricciFlowAnalyticFoundationMilestone :
...
/--
An inhabited detailed stationary-zero assembly payload selects a complete
analytic consumer and exposes the fixed-target raw stationary-zero production
payload through that exact selected consumer.  The selected complete payload
keeps the normalized full package/evolution family and raw production family
definitionally tied to the detailed assembly witness.
-/
theorem stationaryZeroAnalyticFoundation_nonemptyDetailedAssemblyPayload_selected_completeConsumerPayload_and_fixedTarget_rawProductionPayload
    (payload :
      Nonempty StationaryZeroAnalyticFoundationDetailedAssemblyPayload.{u})
    (M : Type u) [TopologicalSpace M] [T2Space M]
    [ChartedSpace ThreeManifoldModel M]
  ...
  rcases
    stationaryZeroAnalyticFoundation_requirements_and_fixedTarget_rawProductionPayload_of_completeConsumerPayload
      ⟨selected⟩ M with
    ⟨_analyticFoundationPackageRequirement,
      _ricciFlowAnalyticFoundationMilestone, rawProductionPayload⟩
  exact
    ⟨ selected
    , detailed
    , rfl
...
analytic consumer and exposes both fixed-target endpoints through that same
selection: the normalized full package/evolution payload and the raw
stationary-zero production payload.  This keeps downstream finite-extinction
consumers from having to independently reselect the complete analytic
consumer for the two analytic routes.
-/
theorem stationaryZeroAnalyticFoundation_nonemptyDetailedAssemblyPayload_selected_completeConsumerPayload_and_fixedTarget_fullPackageEvolution_and_rawProductionPayload
    (payload :
      Nonempty StationaryZeroAnalyticFoundationDetailedAssemblyPayload.{u})
    (M : Type u) [TopologicalSpace M] [T2Space M]
    [ChartedSpace ThreeManifoldModel M]
    ...
    { detailedPayload := detailed
      ...
      fullPackageEvolutionFamily :=
        stationaryZeroAnalyticFoundationDetailedAssemblyPayload_fullPackageEvolutionFamily
          detailed
      rawFullStatementPayloadFamily := detailed.fullStatementPayloadFamily }
  have fullPackageEvolutionPayload :
      ∃ n : ℕ∞ω,
      ∃ flow : RicciFlowData ThreeManifoldModelWithCorners n M,
      ∃ package :
        RicciFlowAnalyticFoundationPackage
...
/--
An inhabited detailed stationary-zero assembly payload selects one complete
analytic consumer, keeps every selected family definitionally tied to the
detailed assembly witness, and exposes the fixed-target complete analytic
endpoint through that exact selected consumer.  This is the maximal local
consumer route: downstream finite-extinction code can use the compact,
evolution, normalized package/evolution, and raw production endpoint without
losing the chosen analytic payload fields.
-/
theorem stationaryZeroAnalyticFoundation_nonemptyDetailedAssemblyPayload_selected_completeConsumerPayload_fields_and_fixedTarget_completeAnalyticEndpoint
    (payload :
      Nonempty StationaryZeroAnalyticFoundationDetailedAssemblyPayload.{u})
    (M : Type u) [TopologicalSpace M] [T2Space M]
    [ChartedSpace ThreeManifoldModel M]
...
/--
An inhabited detailed stationary-zero assembly payload exposes the analytic
package-layer and Ricci-flow analytic milestone fields through the same
selected complete consumer used for the fixed-target endpoint.  This is the
finite-extinction-facing projection: downstream surgery code can keep the
selected analytic consumer, the selected package-layer requirement, the
selected milestone requirement, the definitional links back to the detailed
payload, and the complete fixed-target analytic endpoint together.
-/
theorem stationaryZeroAnalyticFoundation_nonemptyDetailedAssemblyPayload_selected_requirements_and_fixedTarget_completeAnalyticEndpoint
    (payload :
      Nonempty StationaryZeroAnalyticFoundationDetailedAssemblyPayload.{u})
    (M : Type u) [TopologicalSpace M] [T2Space M]
    [ChartedSpace ThreeManifoldModel M]
...
/--
The selected stationary-zero analytic payload exposes the compact evolution
field group and the full fixed-target analytic endpoint from the same detailed
assembly witness.  This gives finite-extinction consumers a single route to
the package-flow identity, equation-boundary/evolution equations, and the
larger analytic endpoint used by downstream package assembly.
-/
theorem stationaryZeroAnalyticFoundation_nonemptyDetailedAssemblyPayload_selected_evolutionGroup_and_completeAnalyticEndpoint
    (payload :
      Nonempty StationaryZeroAnalyticFoundationDetailedAssemblyPayload.{u})
    (M : Type u) [TopologicalSpace M] [T2Space M]
    [ChartedSpace ThreeManifoldModel M]
    ...
    ∃ package :
      ...
      selected.detailedPayload = detailed ∧
        ...
        HasScalarCurvatureEvolutionEquation group.flow ∧
        HasCurvatureNormEvolutionInequality group.flow ∧
        HasCurvatureEvolutionEquations group.flow ∧
        (∃ n : ℕ∞ω,
        ∃ flow : RicciFlowData ThreeManifoldModelWithCorners n M,
        ∃ package :
          RicciFlowAnalyticFoundationPackage
          ...
          AnalyticFoundationSubobligationsPayload flow ∧
            ...
            HasCurvatureEvolutionEquations flow) := by
  rcases
    stationaryZeroAnalyticFoundation_nonemptyDetailedAssemblyPayload_selected_requirements_and_fixedTarget_evolutionGroup
      payload M with
    ⟨ selected
    , detailed
    , analyticRequirement
    ...
    , package
    , hDetailed
    , hAnalyticRequirement
    , hAnalyticMilestone
    , hAnalyticRequirementDetailed
    , hAnalyticMilestoneDetailed
    , hEvolutionFieldsFamily
    , hFullPackageEvolutionFamily
    , subobligations
    , packageFlow
    , statement
    , boundary
...
/--
The selected stationary-zero analytic payload exposes the compact evolution
field group, every selected family-field equality back to the detailed
assembly witness, and the full fixed-target analytic endpoint from the same
selected consumer.  This is the finite-extinction-facing analytic endpoint
that keeps the compact package-flow route synchronized with the raw production
family and the normalized full package/evolution family.
-/
theorem stationaryZeroAnalyticFoundation_nonemptyDetailedAssemblyPayload_selected_allFamilyFields_evolutionGroup_and_completeAnalyticEndpoint
    (payload :
      Nonempty StationaryZeroAnalyticFoundationDetailedAssemblyPayload.{u})
    (M : Type u) [TopologicalSpace M] [T2Space M]
    [ChartedSpace ThreeManifoldModel M]
...
/--
An inhabited detailed stationary-zero assembly payload also keeps the selected
consumer's family-field equalities synchronized with both fixed-target analytic
routes: the raw stationary-zero production witnesses and the normalized full
package/evolution endpoint.  This is the finite-extinction-facing projection
that lets downstream code consume the concrete metric/production data and the
package-level analytic endpoint from the same selected analytic payload.
-/
theorem stationaryZeroAnalyticFoundation_nonemptyDetailedAssemblyPayload_selected_allFamilyFields_rawProduction_and_completeAnalyticEndpoint
    (payload :
      Nonempty StationaryZeroAnalyticFoundationDetailedAssemblyPayload.{u})
    (M : Type u) [TopologicalSpace M] [T2Space M]
    [ChartedSpace ThreeManifoldModel M]
  ...
  exact
    ...
    , rfl
    , rfl
    , detailed.fullStatementPayloadFamily M
    , selected.fullPackageEvolutionFamily M
    ⟩
