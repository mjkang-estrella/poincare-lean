...
analytic statement extractors with the selected package flow.  The same endpoint
retains the concrete Levi-Civita connection field together with the short-time,
continuation, regularization, uniqueness, and terminal curvature-evolution
fields needed by finite-extinction consumers.
-/
theorem analyticProductionPackage_selected_statement_extractors_connectionField_and_curvature_endpoint
    {E : Type u} [NormedAddCommGroup E] [NormedSpace ℝ E]
    {H : Type v} [TopologicalSpace H]
    {I : ModelWithCorners ℝ E H} {n : ℕ∞ω}
    {M : Type w} [TopologicalSpace M] [ChartedSpace H M]
...
/--
A completed analytic-foundation package also exposes the longer
finite-extinction-facing control layer for its selected flow.  This keeps the
DeTurck pullback, short-time solution, maximal interval, Shi estimates, and
curvature-derivative bootstrap fields synchronized with the compact
continuation, uniqueness, and curvature-evolution tuple.
-/
theorem analyticProductionPackage_finiteExtinctionFacing_temporal_regularization_control_fields_of_package
    {E : Type u} [NormedAddCommGroup E] [NormedSpace ℝ E]
    {H : Type v} [TopologicalSpace H]
    {I : ModelWithCorners ℝ E H} {n : ℕ∞ω}
    {M : Type w} [TopologicalSpace M] [ChartedSpace H M]
...
/--
A completed analytic-foundation package exposes the finite-extinction-facing
control layer together with Hamilton's maximum principle.  This keeps the
selected-flow DeTurck/short-time route, continuation and blow-up alternatives,
regularization and derivative estimates, maximum-principle input, uniqueness,
and terminal curvature-evolution fields in one package-level endpoint.
-/
theorem analyticProductionPackage_finiteExtinctionFacing_maximumPrinciple_regularization_control_fields_of_package
    {E : Type u} [NormedAddCommGroup E] [NormedSpace ℝ E]
    {H : Type v} [TopologicalSpace H]
    {I : ModelWithCorners ℝ E H} {n : ℕ∞ω}
    {M : Type w} [TopologicalSpace M] [ChartedSpace H M]
...
/--
Package-field-facing form of the finite-extinction analytic control
certificate.  It opens the certificate together with the exact flow and
theorem-shaped statement selected by the analytic package, while retaining all
dependent control witnesses on the certificate flow and proving that this flow
is the package-selected one.
-/
theorem analyticProductionPackage_finiteExtinctionFacing_package_flow_statement_control_payload_of_package
    {E : Type u} [NormedAddCommGroup E] [NormedSpace ℝ E]
    {H : Type v} [TopologicalSpace H]
    {I : ModelWithCorners ℝ E H} {n : ℕ∞ω}
    {M : Type w} [TopologicalSpace M] [ChartedSpace H M]
    ...
    ∃ statement : RicciFlowAnalyticFoundationStatement I n M,
      flow = ricci_flow_data_of_analytic_foundation_package package ∧
        ...
        HasCurvatureEvolutionEquations certificate.flow := by
  rcases
    analyticProductionPackage_finiteExtinctionFacing_control_certificate_fields_of_package
      package with
    ⟨ certificate
    , hCertificateFlow
    ...
    , hCurvatureEvolution
    ⟩
  let flow : RicciFlowData I n M :=
    ricci_flow_data_of_analytic_foundation_package package
  let statement : RicciFlowAnalyticFoundationStatement I n M :=
...
/--
Package-plus-boundary finite-extinction analytic certificate: a completed
analytic package and a concrete equation-boundary package expose the selected
certificate flow/statement, named derivation/sub-obligation/equation fields,
temporal and regularity controls, maximum-principle and uniqueness controls,
curvature-evolution controls, and the pointwise Ricci-flow equation boundary
from one synchronized endpoint.
-/
theorem analyticProductionPackage_finiteExtinctionFacing_named_certificate_and_boundary_payload_fields_of_package
    {E : Type u} [NormedAddCommGroup E] [NormedSpace ℝ E]
    {H : Type v} [TopologicalSpace H]
    {I : ModelWithCorners ℝ E H} {n : ℕ∞ω}
    {M : Type w} [TopologicalSpace M] [ChartedSpace H M]
    ...
    ∃ equationEvidence :
      ...
      flow = ricci_flow_data_of_analytic_foundation_package package ∧
        ...
        HasCurvatureEvolutionEquations certificate.flow := by
  rcases
    analyticProductionPackage_finiteExtinctionFacing_named_certificate_payload_fields_of_package
      package with
    ⟨ certificate
    , flow
    , statement
...
/--
Package-plus-boundary analytic-boundary endpoint for finite-extinction
consumers.  This strengthens the named certificate/boundary payload by also
returning the assembled `AnalyticFoundationWithEquationBoundaryStatement` and
its boundary component transported to the certificate flow.
-/
theorem analyticProductionPackage_finiteExtinctionFacing_with_equation_boundary_statement_payload_of_package
    {E : Type u} [NormedAddCommGroup E] [NormedSpace ℝ E]
    {H : Type v} [TopologicalSpace H]
    {I : ModelWithCorners ℝ E H} {n : ℕ∞ω}
    {M : Type w} [TopologicalSpace M] [ChartedSpace H M]
    ...
    ∃ equationEvidence :
      ...
      flow = ricci_flow_data_of_analytic_foundation_package package ∧
        ...
        HasCurvatureEvolutionEquations certificate.flow := by
  rcases
    analyticProductionPackage_finiteExtinctionFacing_named_certificate_and_boundary_payload_fields_of_package
    ...
    , hCurvatureEvolution
    ⟩
  have hCertificatePackageFlow :
      certificate.flow =
        ricci_flow_data_of_analytic_foundation_package package :=
    hCertificateFlow.trans hFlow
  have hWithBoundaryPackage :
      AnalyticFoundationWithEquationBoundaryStatement
        (ricci_flow_data_of_analytic_foundation_package package) :=
    analytic_foundation_with_equation_boundary_of_package package boundary
  have hWithBoundaryCertificate :
      AnalyticFoundationWithEquationBoundaryStatement certificate.flow := by
    rw [hCertificatePackageFlow]
    exact hWithBoundaryPackage
  have hBoundaryCertificate :
      RicciFlowEquationBoundaryStatement certificate.flow := by
    rw [hCertificatePackageFlow]
    exact hBoundaryPackage
  exact
    ⟨ certificate
    , flow
    , statement
...
/--
Boundary-verification endpoint for finite-extinction analytic consumers.  In
addition to the certificate and transported equation-boundary statements, this
opens the concrete boundary verification, the boundary equation-interface
evidence, and the named pointwise equation payload that downstream curvature
and extinction estimates can reuse directly.
-/
theorem analyticProductionPackage_finiteExtinctionFacing_boundary_verification_payload_of_package
    {E : Type u} [NormedAddCommGroup E] [NormedSpace ℝ E]
    {H : Type v} [TopologicalSpace H]
    {I : ModelWithCorners ℝ E H} {n : ℕ∞ω}
    {M : Type w} [TopologicalSpace M] [ChartedSpace H M]
    ...
    ∃ boundaryPointwiseEquation :
      ...
      flow = ricci_flow_data_of_analytic_foundation_package package ∧
        ...
        certificate.derivationStatement = derivationStatement ∧
        certificate.subobligations = subobligations ∧
        certificate.equationEvidence = certificateEquationEvidence ∧
        boundaryVerification =
          ricci_flow_equation_verification_of_boundary_package boundary ∧
        boundaryEquationEvidence =
          equation_evidence_of_equation_boundary_package boundary ∧
        boundaryPointwiseEquation =
          pointwise_equation_payload_of_equation_boundary_package boundary ∧
        AnalyticFoundationWithEquationBoundaryStatement certificate.flow ∧
        AnalyticFoundationWithEquationBoundaryStatement
          (ricci_flow_data_of_analytic_foundation_package package) ∧
        RicciFlowEquationBoundaryStatement certificate.flow ∧
        ...
        HasCurvatureEvolutionEquations certificate.flow := by
  rcases
    analyticProductionPackage_finiteExtinctionFacing_with_equation_boundary_statement_payload_of_package
      package boundary with
    ⟨ certificate
    , flow
    , statement
    ...
    , hDerivationStatement
    , hSubobligations
    , hCertificateEquationEvidence
    , hWithBoundaryCertificate
    , hBoundaryCertificate
    , _hCertificateStatementSelected
    , _hStatementFlow
    , hBoundaryPackage
    , hDerivative
    , hEquationAtTime
...
/--
Certificate-flow boundary endpoint for finite-extinction analytic consumers.
The previous endpoint names the equation-boundary payload on the package
selected flow; this one transports the same pointwise Ricci-flow equation to
the certificate flow retained by the finite-extinction control certificate, so
downstream curvature estimates can use `certificate.flow` directly.
-/
theorem analyticProductionPackage_finiteExtinctionFacing_certificate_flow_boundary_equation_payload_of_package
    {E : Type u} [NormedAddCommGroup E] [NormedSpace ℝ E]
    {H : Type v} [TopologicalSpace H]
    {I : ModelWithCorners ℝ E H} {n : ℕ∞ω}
    {M : Type w} [TopologicalSpace M] [ChartedSpace H M]
    ...
    ∃ _boundaryPointwiseEquationOnCertificate :
      ...
      flow = ricci_flow_data_of_analytic_foundation_package package ∧
        ...
        HasCurvatureEvolutionEquations certificate.flow := by
  rcases
    analyticProductionPackage_finiteExtinctionFacing_boundary_verification_payload_of_package
      package boundary with
    ⟨ certificate
    , flow
    , statement
  ...
  have hCertificatePackageFlow :
      certificate.flow =
        ricci_flow_data_of_analytic_foundation_package package :=
    hCertificateFlow.trans hFlow
  have hBoundaryPointwiseEquationOnCertificate :
      ∀ (t : ℝ) (x : M) (v w : TangentSpace I x),
        metric_time_derivative_at_time_of_metric_derivative_field
          (metric_time_derivative_field_of_metric_derivative_data
            (metric_derivative_data_of_equation_boundary_package boundary))
    ...
    intro t x v w
    rw [hCertificatePackageFlow]
    exact boundaryPointwiseEquationOnPackage t x v w
  exact
    ⟨ certificate
    , flow
    , statement
    ...
    , boundaryEquationEvidence
    , boundaryPointwiseEquationOnPackage
    , hBoundaryPointwiseEquationOnCertificate
    , hFlow
    , hStatement
    , hCertificateFlow
...
/--
Combined certificate/package boundary-equation endpoint for finite-extinction
analytic consumers.  It keeps the transported pointwise equation on
`certificate.flow` together with the package-side derivative witness and
equation-at-time boundary fields, so downstream curvature estimates can move
between the certificate-selected flow and the package-selected flow without
reopening the larger analytic certificate payload.
-/
theorem analyticProductionPackage_finiteExtinctionFacing_certificate_and_package_boundary_equation_fields_of_package
    {E : Type u} [NormedAddCommGroup E] [NormedSpace ℝ E]
    {H : Type v} [TopologicalSpace H]
    {I : ModelWithCorners ℝ E H} {n : ℕ∞ω}
    {M : Type w} [TopologicalSpace M] [ChartedSpace H M]
...
/--
Opened certificate/package boundary-equation endpoint for finite-extinction
analytic consumers.  This combines the full finite-extinction-facing analytic
certificate fields with the package-side derivative and equation-at-time
boundary witnesses, while also transporting the pointwise Ricci-flow equation
to the certificate-selected flow.
-/
theorem analyticProductionPackage_finiteExtinctionFacing_opened_certificate_and_package_boundary_equation_fields_of_package
    {E : Type u} [NormedAddCommGroup E] [NormedSpace ℝ E]
    {H : Type v} [TopologicalSpace H]
    {I : ModelWithCorners ℝ E H} {n : ℕ∞ω}
    {M : Type w} [TopologicalSpace M] [ChartedSpace H M]
