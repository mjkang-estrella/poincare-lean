...
/--
A completed finite-extinction surgery package exposes the common selected flow
together with the construction package, Perelman control package, concrete
surgery-scale payload, and singularity-model blowup classification evidence.
-/
theorem surgery_perelman_scale_and_blowup_payload_of_finite_extinction_surgery_package
    {n : ℕ∞ω}
    {M : Type u} [TopologicalSpace M] [T2Space M]
    [ChartedSpace ThreeManifoldModel M] [SimplyConnectedSpace M]
    [CompactSpace M]
...
/--
Pointwise compact package-layer projection: assuming the finite-extinction
package layer, each target receives one selected flow carrying the construction
package, Perelman package, concrete surgery-scale payload, and blowup
classification evidence.
-/
theorem surgery_perelman_scale_and_blowup_payload_target_at_of_finiteExtinctionPackage_requirement
    (finiteExtinctionRequirement :
      dependencyPackageLayerRequirement.{u}
        DependencyPackageLayer.finiteExtinctionPackage)
    (M : Type u) [TopologicalSpace M] [T2Space M]
  ...
  exact
    ⟨ n
    , surgery_perelman_scale_and_blowup_payload_of_finite_extinction_surgery_package
        package
    ⟩
...
/--
Single assembly payload for the surgery/Perelman package layer projected from
the stronger finite-extinction package layer.  It keeps the package-layer
requirement, both surgery milestones, the named package-tied projection family,
and the selected-flow scale/blowup family in one proof object.
-/
structure SurgeryPerelmanAssemblyPayloadFromFiniteExtinction where
...
/--
The finite-extinction package-layer requirement constructs the single
surgery/Perelman assembly payload.
-/
def surgeryPerelmanAssemblyPayload_of_finiteExtinctionPackage_requirement
    (finiteExtinctionRequirement :
      dependencyPackageLayerRequirement.{u}
        DependencyPackageLayer.finiteExtinctionPackage) :
    SurgeryPerelmanAssemblyPayloadFromFiniteExtinction.{u} where
  surgeryPackageRequirement :=
    surgeryPackage_requirement_of_finiteExtinctionPackage_requirement
      finiteExtinctionRequirement
  ricciFlowWithSurgeryMilestone :=
...
/--
The assembly payload unpacks to the exact surgery/Perelman tuple already
projected from the finite-extinction package layer.
-/
theorem surgeryPerelmanAssemblyPayload_fields_of_finiteExtinctionPackage_requirement
    (finiteExtinctionRequirement :
      dependencyPackageLayerRequirement.{u}
        DependencyPackageLayer.finiteExtinctionPackage) :
    dependencyPackageLayerRequirement.{u}
...
/--
The detailed surgery/Perelman assembly payload exposes concrete selected-flow
data tied to the same finite-extinction package: the flow identity,
construction package, Perelman package, surgery-scale payload, blowup
classification, finite-extinction statement, and extinction witness.
-/
theorem surgeryPerelmanDetailedAssemblyPayload_selectedFlowExtinctionFamily
    (payload : SurgeryPerelmanDetailedAssemblyPayloadFromFiniteExtinction.{u}) :
    ∀ (M : Type u) [TopologicalSpace M] [T2Space M]
      [ChartedSpace ThreeManifoldModel M]
      [SimplyConnectedSpace M] [CompactSpace M]
      [IsManifold ThreeManifoldModelWithCorners 1 M],
...
/--
Construct the selected-flow finite-extinction family directly from the
finite-extinction package-layer requirement.
-/
theorem surgeryPerelman_selectedFlowExtinctionFamily_of_finiteExtinctionPackage_requirement
    (finiteExtinctionRequirement :
      dependencyPackageLayerRequirement.{u}
        DependencyPackageLayer.finiteExtinctionPackage) :
    ∀ (M : Type u) [TopologicalSpace M] [T2Space M]
...
/--
The finite-extinction package layer constructs the detailed surgery/Perelman
assembly payload and exposes the original finite-extinction requirement,
surgery package requirement, surgery/Perelman milestones, and selected-flow
finite-extinction family in one endpoint.
-/
theorem finiteExtinctionRequirement_detailedSurgeryPerelmanPayload_and_selectedFlowExtinctionFamily
    (finiteExtinctionRequirement :
      dependencyPackageLayerRequirement.{u}
        DependencyPackageLayer.finiteExtinctionPackage) :
    dependencyPackageLayerRequirement.{u}
...
/--
The terminal Ricci-flow-with-surgery payload discharges the full
construction-package route and exposes the aggregate surgery field it contains.
-/
theorem surgery_construction_package_and_ricci_flow_with_surgery_of_payload
    {n : ℕ∞ω}
    {M : Type u} [TopologicalSpace M] [ChartedSpace ThreeManifoldModel M]
    [IsManifold ThreeManifoldModelWithCorners 1 M]
    {flow : RicciFlowData ThreeManifoldModelWithCorners n M}
