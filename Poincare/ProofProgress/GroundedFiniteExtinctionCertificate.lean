...
/--
Grounded universal finite extinction keeps the selected complete consumer, the
checked final-certificate route, and both fixed-target derivation views tied to
the selected universal finite-extinction witness.  This is the non-frontier
endpoint for final assembly code that needs the target extinction witness from
the selected universal statement together with both the flow-first and
package-first derivation payloads.
-/
theorem groundedUniversalFiniteExtinction_selected_completeConsumerPayload_remaining_certificate_dualDerivationPayloads_and_universalWitness_of_grounded
    (smoothability :
      dependencyPackageLayerRequirement.{u}
        DependencyPackageLayer.smoothabilityPackage)
    (grounded : GroundedUniversalFiniteExtinctionStatement.{u})
...
/--
The fixed-target finite-extinction consumer also closes the theorem-shaped
Poincare statement through the same checked completion certificate.  This keeps
the selected universal witness, the remaining-dependency certificate, the
target completion criterion, and the final statement consequence synchronized
without reopening the broader family endpoints.
-/
theorem groundedUniversalFiniteExtinction_selected_completeConsumerPayload_remaining_certificate_targetWitness_completionCriterion_and_poincareStatement_of_grounded
    (smoothability :
      dependencyPackageLayerRequirement.{u}
        DependencyPackageLayer.smoothabilityPackage)
    (grounded : GroundedUniversalFiniteExtinctionStatement.{u})
    ...
    ∃ extinctionWitness : FiniteExtinctionByRicciFlowWithSurgery M,
      remaining.smoothability = smoothability ∧
        ...
        CompletionCriterionAtUniverse witness := by
  rcases
    groundedUniversalFiniteExtinction_selected_completeConsumerPayload_remaining_certificate_targetWitness_and_completionCriterion_of_grounded
...
/--
The fixed-target final-certificate consumer also retains the package-first
finite-extinction derivation payload for the same smooth target.  This keeps
the selected universal witness, checked certificate route, Poincare statement,
target completion criterion, and package/statement/derivation view synchronized
for downstream finite-extinction and final-assembly consumers.
-/
theorem groundedUniversalFiniteExtinction_selected_completeConsumerPayload_remaining_certificate_targetWitness_completionCriterion_poincareStatement_and_packageDerivationPayload_of_grounded
    (smoothability :
      dependencyPackageLayerRequirement.{u}
        DependencyPackageLayer.smoothabilityPackage)
    (grounded : GroundedUniversalFiniteExtinctionStatement.{u})
        ...
        ∃ _package' : FiniteExtinctionSurgeryPackage n' M,
          FiniteExtinctionStatement n' M ∧
            ...
            ∃ control' :
              ...
              HasFiniteExtinctionDerivation flow' surgery' control' := by
  rcases
    groundedUniversalFiniteExtinction_selected_completeConsumerPayload_remaining_certificate_targetWitness_completionCriterion_and_poincareStatement_of_grounded
      smoothability grounded topologyPackage M witness with
    ⟨ selected
    , remaining
    , certificate
...
/--
The fixed-target final-certificate consumer also retains the selected
Ricci-flow-with-surgery, Perelman-control, and finite-extinction milestone
fields.  This keeps the checked certificate, final Poincare statement,
target completion criterion, selected universal witness, and both
finite-extinction derivation views synchronized with the milestone fields used
by the dependency ledger.
-/
theorem groundedUniversalFiniteExtinction_selected_completeConsumerPayload_milestoneFields_remaining_certificate_targetWitness_completionCriterion_poincareStatement_and_packageDerivationPayload_of_grounded
    (smoothability :
      dependencyPackageLayerRequirement.{u}
        DependencyPackageLayer.smoothabilityPackage)
    (grounded : GroundedUniversalFiniteExtinctionStatement.{u})
        ...
        ∃ _package' : FiniteExtinctionSurgeryPackage n' M,
          FiniteExtinctionStatement n' M ∧
            ...
            ∃ control' :
              ...
              HasFiniteExtinctionDerivation flow' surgery' control' := by
  rcases
    groundedUniversalFiniteExtinction_selected_completeConsumerPayload_remaining_certificate_targetWitness_completionCriterion_poincareStatement_and_packageDerivationPayload_of_grounded
      smoothability grounded topologyPackage M witness with
    ⟨ selected
    , remaining
    , certificate
...
/--
The fixed-target grounded finite-extinction consumer retains every selected
target-family payload stored on the complete consumer: the compact
package/statement witness, the flow-package derivation payload, the full
flow/surgery/control statement payload, and the package-first derivation
payload.  This keeps the checked certificate route, final Poincare statement,
target completion criterion, selected universal witness, and all
finite-extinction package projections synchronized for downstream assembly.
-/
theorem groundedUniversalFiniteExtinction_selected_completeConsumerPayload_allTargetFamilies_remaining_certificate_targetWitness_completionCriterion_poincareStatement_of_grounded
    (smoothability :
      dependencyPackageLayerRequirement.{u}
        DependencyPackageLayer.smoothabilityPackage)
    (grounded : GroundedUniversalFiniteExtinctionStatement.{u})
...
/--
Grounded universal finite extinction can select the complete finite-extinction
consumer while keeping the exact detailed assembly payload that produced it.
The endpoint also retains the remaining-dependency certificate route and one
fixed-target flow/surgery/control/package/statement/derivation/extinction
payload from the same selected consumer, so final assembly can use the detailed
payload equalities and the concrete finite-extinction witness together.
-/
theorem groundedUniversalFiniteExtinction_selected_detailedPayload_remaining_certificate_route_and_fixedTarget_statementPayload_of_grounded
    (smoothability :
      dependencyPackageLayerRequirement.{u}
        DependencyPackageLayer.smoothabilityPackage)
    (grounded : GroundedUniversalFiniteExtinctionStatement.{u})
    ...
    [IsManifold ThreeManifoldModelWithCorners 1 M] :
    ∃ selected : GroundedUniversalFiniteExtinctionCompleteConsumerPayload.{u},
    ∃ detailed : GroundedUniversalFiniteExtinctionDetailedAssemblyPayload.{u},
    ∃ remaining : RemainingDependencyPackage.{u},
    ∃ certificate : PoincareCompletionCertificate.{u},
    ∃ n : ℕ∞ω,
    ∃ flow : RicciFlowData ThreeManifoldModelWithCorners n M,
    ...
    ∃ _derivation : HasFiniteExtinctionDerivation flow surgery control,
    ∃ extinctionWitness : FiniteExtinctionByRicciFlowWithSurgery M,
      detailed = groundedUniversalFiniteExtinctionDetailedAssemblyPayload
        grounded ∧
        selected.detailedPayload = detailed ∧
        selected.universalStatement = detailed.universalStatement ∧
        selected.finiteExtinctionPackageRequirement =
...
/--
The detailed grounded finite-extinction endpoint can retain all selected
target-family projections at the same fixed target.  This opens the compact
package/statement witness, the flow-package derivation view, the full statement
payload, and the package-first derivation view alongside the detailed payload
equalities and remaining-dependency certificate route.
-/
theorem groundedUniversalFiniteExtinction_selected_detailedPayload_allTargetFamilies_remaining_certificate_route_and_fixedTarget_statementPayload_of_grounded
    (smoothability :
      dependencyPackageLayerRequirement.{u}
        DependencyPackageLayer.smoothabilityPackage)
    (grounded : GroundedUniversalFiniteExtinctionStatement.{u})
    ...
    ∃ packageStatementDerivation :
      ...
      detailed = groundedUniversalFiniteExtinctionDetailedAssemblyPayload
        ...
        (∀ witness : Type u, CompletionCriterionAtUniverse witness) := by
  rcases
    groundedUniversalFiniteExtinction_selected_detailedPayload_remaining_certificate_route_and_fixedTarget_statementPayload_of_grounded
      smoothability grounded topologyPackage M with
    ⟨ selected
    , detailed
    , remaining
    ...
    , hCriteria
    ⟩
  let packageStatementWitness :=
    selected.packageStatementWitnessFamily M
  let flowPackage :=
    selected.flowPackageFamily M
  let statementPayload :=
    selected.statementPayloadFamily M
  let packageStatementDerivation :=
    selected.packageStatementDerivationFamily M
  exact
    ⟨ selected
    , detailed
...
/--
Compact final-certificate consumer form of the detailed grounded
finite-extinction endpoint.  It keeps the selected detailed payload and all
target-family projections while exposing the remaining-dependency checked
certificate as an inhabited certificate and specializing the completion
criteria to a requested universe witness.
-/
theorem groundedUniversalFiniteExtinction_selected_detailedPayload_allTargetFamilies_checkedCertificate_and_witnessCriterion_of_grounded
    (smoothability :
      dependencyPackageLayerRequirement.{u}
        DependencyPackageLayer.smoothabilityPackage)
    (grounded : GroundedUniversalFiniteExtinctionStatement.{u})
    ...
    ∃ packageStatementDerivation :
      ...
      detailed = groundedUniversalFiniteExtinctionDetailedAssemblyPayload
        ...
        CompletionCriterionAtUniverse witness := by
  rcases
    groundedUniversalFiniteExtinction_selected_detailedPayload_allTargetFamilies_remaining_certificate_route_and_fixedTarget_statementPayload_of_grounded
      smoothability grounded topologyPackage M with
    ⟨ selected
    , detailed
    , remaining
    ...
    , _hPackageStatementWitnessFamily
    , _hStatementPayloadFamily
    , _hFlowPackageFamily
    ...
    , hUniversal
    , hRequirement
    , _hRicciFlowMilestone
    , _hPerelmanMilestone
    , _hFiniteExtinctionMilestone
    , _hWitness
...
/--
Compact consumer form retaining both the concrete fixed-target finite-extinction
witnesses and all target-family projections while also exposing the checked
completion certificate at a requested universe witness.
-/
theorem groundedUniversalFiniteExtinction_selected_detailedPayload_concreteTarget_allTargetFamilies_checkedCertificate_and_witnessCriterion_of_grounded
    (smoothability :
      dependencyPackageLayerRequirement.{u}
        DependencyPackageLayer.smoothabilityPackage)
    (grounded : GroundedUniversalFiniteExtinctionStatement.{u})
...
/--
Detailed-field version of the compact grounded finite-extinction endpoint.  It
retains the concrete fixed-target flow/surgery/control/package witnesses, all
target-family projections, the checked certificate and witness criterion, and
the equalities identifying the selected consumer's target-family functions with
the detailed assembly payload fields that produced them.
-/
theorem groundedUniversalFiniteExtinction_selected_detailedPayload_familyFields_concreteTarget_checkedCertificate_and_witnessCriterion_of_grounded
    (smoothability :
      dependencyPackageLayerRequirement.{u}
        DependencyPackageLayer.smoothabilityPackage)
    (grounded : GroundedUniversalFiniteExtinctionStatement.{u})
...
/--
All-witness version of the detailed grounded finite-extinction endpoint.  It
retains the concrete fixed-target flow/surgery/control/package witnesses, all
target-family projections, the checked certificate as an inhabited certificate,
and the full universe-indexed completion-criteria family, so final-certificate
assembly does not need to choose a witness universe before consuming the
grounded finite-extinction payload.
-/
theorem groundedUniversalFiniteExtinction_selected_detailedPayload_familyFields_concreteTarget_checkedCertificate_and_completionCriteria_of_grounded
    (smoothability :
      dependencyPackageLayerRequirement.{u}
        DependencyPackageLayer.smoothabilityPackage)
    (grounded : GroundedUniversalFiniteExtinctionStatement.{u})
...
/--
Combined all-witness and fixed-witness version of the detailed grounded
finite-extinction endpoint.  It keeps the concrete fixed-target
flow/surgery/control/package witnesses and all target-family projections, names
the full completion-criteria family, and also names its specialization at a
requested witness universe.
-/
theorem groundedUniversalFiniteExtinction_selected_detailedPayload_familyFields_concreteTarget_checkedCertificate_completionCriteria_and_witnessCriterion_of_grounded
    (smoothability :
      dependencyPackageLayerRequirement.{u}
        DependencyPackageLayer.smoothabilityPackage)
    (grounded : GroundedUniversalFiniteExtinctionStatement.{u})
  ...
  rcases
    groundedUniversalFiniteExtinction_selected_detailedPayload_familyFields_concreteTarget_checkedCertificate_and_completionCriteria_of_grounded
      smoothability grounded topologyPackage M with
    ⟨ selected
    , detailed
    , remaining
...
/--
Uniform target-family form of the grounded finite-extinction certificate
endpoint.  A single grounded finite-extinction input supplies, for every smooth
compact simply connected target and every witness universe, the selected
consumer payload, detailed assembly payload, remaining dependency package,
checked completion certificate, target extinction witness, all-witness
completion-criteria family, and the named specialization at the requested
witness.
-/
theorem groundedUniversalFiniteExtinction_uniform_selected_certificate_completionCriteria_and_witnessCriterion_of_grounded
    (smoothability :
      dependencyPackageLayerRequirement.{u}
        DependencyPackageLayer.smoothabilityPackage)
    (grounded : GroundedUniversalFiniteExtinctionStatement.{u})
  ...
  rcases
    ...
    , _derivation
    , extinctionWitness
    , _packageStatementWitness
    , _flowPackage
    , _statementPayload
    , _packageStatementDerivation
    , allCriteria
    , witnessCriterion
    , hDetailed
    , hSelectedDetailed
    ...
    , hUniversal
    , hWitness
    , _hPackageStatementWitness
    , _hFlowPackage
    , _hStatementPayload
    , _hPackageStatementDerivation
    , _hPackageStatement
    , _hDerivation
    , hPoincare
    , hCertificatePayload
...
/--
Uniform concrete-surgery form of the grounded finite-extinction certificate
endpoint.  The collapsed certificate route still retains, for every target and
witness universe, the selected finite-extinction package index, Ricci-flow
data, surgery data, Perelman control, package statement, derivation witness,
target extinction witness, checked completion certificate, and fixed witness
criterion.
-/
theorem groundedUniversalFiniteExtinction_uniform_concrete_surgery_certificate_and_witnessCriterion_of_grounded
    (smoothability :
      dependencyPackageLayerRequirement.{u}
        DependencyPackageLayer.smoothabilityPackage)
    (grounded : GroundedUniversalFiniteExtinctionStatement.{u})
...
/--
The terminal equality route also exposes the canonical time-bound and
derivation equalities, together with their source witnesses.  This is the
stronger consumer-facing projection for downstream finite-extinction assembly:
it keeps the theorem-shaped statement, production certificate, terminal
finite-extinction witness, time bound, derivation, conclusion source, and all
canonical equalities in one endpoint.
-/

theorem grounded_universal_finite_extinction_statement_terminal_full_equality_payload
    (grounded : GroundedUniversalFiniteExtinctionStatement.{u})
    (M : Type u) [TopologicalSpace M] [T2Space M]
    [ChartedSpace ThreeManifoldModel M]
    [SimplyConnectedSpace M] [CompactSpace M]
...
/--
Theorem contract for
`grounded_universal_finite_extinction_statement_terminal_full_equality_payload`.
-/

theorem grounded_universal_finite_extinction_statement_terminal_full_equality_payload_eq :
    @Poincare.grounded_universal_finite_extinction_statement_terminal_full_equality_payload =
      @Poincare.grounded_universal_finite_extinction_statement_terminal_full_equality_payload :=
  rfl
