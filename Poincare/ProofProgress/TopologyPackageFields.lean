...
/--
The same compact two-puncture collapse certificate with the singleton
complement basepoint chosen internally from the topology package.  Downstream
two-puncture consumers now only need to provide the actual two-puncture
basepoint appearing in the low-homotopy groups.
-/
theorem twoPointComplement_lowHomotopyCollapse_payload_of_topology_package_at_basepoint
    (package : ExtinctionTopologyExtractionPackage.{u})
    (M : Type u) [TopologicalSpace M] [T2Space M]
    [ChartedSpace (EuclideanSpace ℝ (Fin 3)) M]
    [SimplyConnectedSpace M] [CompactSpace M]
  ...
  exact
    twoPointComplement_lowHomotopyCollapse_payload_of_topology_package
      package M extinction x hyx singleBasepoint twoBasepoint
...
/--
The topology package supplies one point-selected endpoint containing sphere
recognition, one-point compactification recognition, the complete
singleton/two-puncture complement-collapse payload, and the data-valued
two-puncture low-homotopy `Unique` payload for the chosen two-puncture
basepoint.
-/
theorem recognition_completeComplement_and_twoPointComplement_lowHomotopyUnique_payload_of_topology_package
    (package : ExtinctionTopologyExtractionPackage.{u})
    (M : Type u) [TopologicalSpace M] [T2Space M]
    [ChartedSpace (EuclideanSpace ℝ (Fin 3)) M]
    [SimplyConnectedSpace M] [CompactSpace M]
...
/--
The topology package exposes the extractor/complete-complement payload together
with data-valued singleton recognition and two-puncture low-homotopy
uniqueness at the same point-selected endpoint.
-/
theorem topology_package_extractor_completeComplement_singleton_and_twoPointComplement_payload
    (package : ExtinctionTopologyExtractionPackage.{u})
    (M : Type u) [TopologicalSpace M] [T2Space M]
    [ChartedSpace (EuclideanSpace ℝ (Fin 3)) M]
    [SimplyConnectedSpace M] [CompactSpace M]
