...
/--
Family endpoint combining the first topology-production data payload with the
recognition and puncture consequences of a completed topology package.  For
each finite-extinction target it names the certified decomposition datum,
synchronizes the data-built decomposition with both the statement projection
and the package-selected decomposition, and retains the sphere/one-point
recognition plus singleton and two-puncture complement collapse payloads.
-/
theorem topology_package_decomposition_data_recognition_and_puncture_payload_family_current_interface
    (package : ExtinctionTopologyExtractionPackage.{u})
    (decompositionData : ExtinctionTopologyDecompositionDataStatement.{u}) :
    ∀ (M : Type u) [TopologicalSpace M] [T2Space M]
      [ChartedSpace (EuclideanSpace ℝ (Fin 3)) M]
  ...
  rcases
    extinction_topology_decomposition_statement_named_data_payload_current_interface
      decompositionData M extinction with
    ⟨data, selectedDecomposition, hSelectedData, hStatementSelected⟩
  rcases topology_package_recognition_and_puncture_payload_family
      package M extinction with
    ⟨sphereRecognition, onePointRecognition, singletonFamily,
      twoPointFamily⟩
  refine
    ⟨ data
    , selectedDecomposition
...
/--
Fixed-target opened form of the topology-package decomposition endpoint.  It
turns the package-layer requirement and decomposition-data statement into a
single consumable certificate for the selected finite-extinction target: the
named package, certified decomposition datum, statement/package synchronization,
concrete sphere and one-point recognitions, and basepoint-specific singleton
and two-puncture complement collapse facts.
-/
theorem topologyPackage_requirement_fixedTarget_decomposition_recognition_and_puncture_payload_current_interface
    (topologyPackage :
      dependencyPackageLayerRequirement.{u}
        DependencyPackageLayer.topologyPackage)
    (decompositionData : ExtinctionTopologyDecompositionDataStatement.{u})
      ...
      package = topologyPackage ∧
        ...
        Subsingleton
          (FundamentalGroup (({x} ∪ {y})ᶜ : Set M)
            twoPointBasepoint) := by
  rcases
    topologyPackage_requirement_decomposition_data_recognition_and_puncture_payload_family_current_interface
      topologyPackage decompositionData M extinction with
    ⟨ package
    , data
    , selectedDecomposition
    , hPackage
  ...
  rcases hPackageHomeomorphism :
      homeomorphism_of_topology_package package M extinction with
    ⟨sphereHomeomorphism⟩
  rcases hOnePointRecognition :
      homeomorph_to_onePoint_threeSpace_of_topology_package
        package M extinction with
    ⟨onePointHomeomorphism⟩
  have singletonContractible : ContractibleSpace ({x}ᶜ : Set M) :=
    singletonFamily x
  have singletonSimplyConnected : SimplyConnectedSpace ({x}ᶜ : Set M) :=
    compl_singleton_simplyConnectedSpace_of_topology_package
      package M extinction x
  rcases
    compl_singleton_lowHomotopy_subsingleton_package_of_topology_package
    ...
    , singletonHomotopyPi1
    ⟩
  rcases twoPointFamily hyx twoPointBasepoint with
    ⟨twoPointSimplyConnected, twoPointFundamental⟩
  exact
    ⟨ package
    , data
    , selectedDecomposition
    , Nonempty.intro sphereHomeomorphism
    , sphereHomeomorphism
    , Nonempty.intro onePointHomeomorphism
    , onePointHomeomorphism
    , hPackage
    , hSelectedData
    , hStatementSelected
...
/--
Statement-witness form of the fixed-target topology-package endpoint.  This
keeps the package's theorem-shaped topology extraction, extinction-implies-
sphere, and lifted-homeomorphism derivation witnesses synchronized with the
same selected decomposition, concrete `ThreeSphere`/one-point homeomorphisms,
homeomorphism-derivation statement, and singleton/two-point puncture payload.
-/
theorem topologyPackage_requirement_fixedTarget_named_statement_witnesses_homeomorphism_derivation_and_puncture_payload_current_interface
    (topologyPackage :
      dependencyPackageLayerRequirement.{u}
        DependencyPackageLayer.topologyPackage)
    (decompositionData : ExtinctionTopologyDecompositionDataStatement.{u})
      ...
      package = topologyPackage ∧
        ...
        Subsingleton
          (FundamentalGroup (({x} ∪ {y})ᶜ : Set M)
            twoPointBasepoint) := by
  rcases
    topologyPackage_requirement_fixedTarget_decomposition_homeomorphism_derivation_and_puncture_payload_current_interface
      topologyPackage decompositionData M extinction x y hyx
      singletonBasepoint twoPointBasepoint with
    ⟨ package
    , data
...
/--
Complete-consumer derivation-puncture endpoint for the topology package layer.
This opens the consumer's retained target-family field itself, then specializes
that same family to one finite-extinction target so final-certificate code can
reuse the classification, recognition, homeomorphism-assembly, lifted
derivation, one-point, singleton, and two-point puncture payloads without
reconstructing the consumer.
-/
theorem topologyPackage_requirement_completeConsumer_derivationPunctureFamily_fixedTarget_payload
    (topologyPackage :
      dependencyPackageLayerRequirement.{u}
        DependencyPackageLayer.topologyPackage)
    (M : Type u) [TopologicalSpace M] [T2Space M]
    ...
    let package : ExtinctionTopologyExtractionPackage.{u} :=
      topologyPackage
    ∃ consumer : ExtinctionTopologyCompleteConsumerPayload.{u},
    ∃ selectedDerivationPunctureFamily :
      ∀ (M : Type u) [TopologicalSpace M] [T2Space M]
        [ChartedSpace (EuclideanSpace ℝ (Fin 3)) M]
        [SimplyConnectedSpace M] [CompactSpace M]
        (_extinction : FiniteExtinctionByRicciFlowWithSurgery M),
    ...
    ∃ homeomorphism : Nonempty (M ≃ₜ ThreeSphere),
      consumer.topologyPackage = package ∧
        ...
        consumer.liftedHomeomorphismDerivation =
          topology_extraction_lifted_homeomorphism_derivation_payload_of_topology_package
            package ∧
        consumer.derivationPunctureFamily =
          selectedDerivationPunctureFamily ∧
        selectedDerivationPunctureFamily =
          topology_package_extraction_derivation_and_puncture_payload_family
            package ∧
  ...
  let package : ExtinctionTopologyExtractionPackage.{u} :=
    topologyPackage
  let selectedDerivationPunctureFamily :
      ∀ (M : Type u) [TopologicalSpace M] [T2Space M]
        [ChartedSpace (EuclideanSpace ℝ (Fin 3)) M]
        [SimplyConnectedSpace M] [CompactSpace M]
        (_extinction : FiniteExtinctionByRicciFlowWithSurgery M),
  ...
  let consumer : ExtinctionTopologyCompleteConsumerPayload.{u} :=
    { topologyPackage := package
      ...
      liftedHomeomorphismDerivation :=
        topology_extraction_lifted_homeomorphism_derivation_payload_of_topology_package
          package
      derivationPunctureFamily := selectedDerivationPunctureFamily }
  rcases selectedDerivationPunctureFamily M extinction with
    ⟨ homeomorphism
    , hHomeomorphism
    , classification
    , simplyConnectedRecognition
